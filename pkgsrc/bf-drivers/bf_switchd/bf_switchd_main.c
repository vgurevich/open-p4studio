...
static void bf_switchd_parse_options(bf_switchd_context_t *ctx,
  ...
  while (1) {
    ...
    static struct option long_options[] = {
        ...
        {"skip-dma", no_argument, 0, OPT_SKIP_DMA},
        {"status-port", required_argument, 0, OPT_STS_PORT},
        {"bfrt-grpc-port", required_argument, 0, OPT_BFRT_GRPC_PORT},
    ...
    switch (c) {
      ...
      case OPT_SKIP_PORT_ADD:
        ...
        break;
      case OPT_SKIP_DMA:
        ctx->skip_dma_thread = true;
        break;
      case OPT_STS_PORT:
        ctx->dev_sts_thread = true;
        ctx->dev_sts_port = atoi(optarg);
        break;
      case OPT_BFRT_GRPC_PORT:
        ctx->bf_rt_grpc_port = atoi(optarg);
        break;
      case OPT_KERNEL_PKT:
        ctx->kernel_pkt = true;
        break;
...
int main(int argc, char *argv[]) {
  ...
  if (!switchd_main_ctx->kernel_pkt &&
      bf_switchd_is_kernel_pkt_module_present()) {
    printf("kernel mode packet driver present, forcing kernel_pkt option!\n");
    switchd_main_ctx->kernel_pkt = bf_switchd_is_kernel_pkt_module_present();
  } else if (switchd_main_ctx->kernel_pkt &&
             !bf_switchd_is_kernel_pkt_module_present()) {
    printf(
        "ERROR: Kernel mode packet processing enabled, but kernel mode packet "
        "driver not found\n");
    return -1;
  }
  ...
  if (switchd_main_ctx) {
    if (switchd_main_ctx->install_dir) free(switchd_main_ctx->install_dir);
    if (switchd_main_ctx->conf_file) free(switchd_main_ctx->conf_file);
    if (switchd_main_ctx->p4rt_server) free(switchd_main_ctx->p4rt_server);
