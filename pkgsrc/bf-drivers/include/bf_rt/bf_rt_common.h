...
typedef enum bf_rt_flag_offset_t {
  ...
  /** Flag used to allow callbacks with only entry handle (used with HLP/LLP
   * separation).
   */
  BF_RT_FORCE_SIMPLE_IDLE_TIME_CALLBACK = 4,
  /** Flag used to ignore error code NOT_FOUND on entry deletion.
   */
  BF_RT_IGNORE_NOT_FOUND = 5,
} bf_rt_flag_offset_t;
