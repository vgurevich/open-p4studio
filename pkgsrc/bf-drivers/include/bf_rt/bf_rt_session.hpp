...
class BfRtSession {
 public:
  ...
  /**
   * @brief Factory function to instantiate a session object
   *
   * @returns @c pointer to an allocated BfRtSession session object. The
   * session object is a pointer that cannot be used in transactions.
   * The caller needs to call free on allocated session in order to free memory.
   * Main purpose of this is to allow older and C style APIs working with
   * session_id to create the session object for use. Session created this way
   * cannot be used the same way normal BfRtSession object is.
   */
  static BfRtSession *sessionInstantiate(bf_rt_id_t session_id,
