...
/**
 * @brief Get Key Field mask, as defined in P4. It will be either the
 * user-defined mask, or it will have all the bits set to 1 according to the
 * field size. Mask argument size must be byte-matching field size calculated
 * as (field_width + 7) / 8.
 *
 * @param[in] table_hdl Table object
 * @param[in] field_id Field ID
 * @param[in] msize Size of mask array, must be matching field size.
 * @param[out] mask Mask of size of related field in bytes
 *
 * @return Status of the API call
 */
bf_status_t bf_rt_key_field_mask_get(const bf_rt_table_hdl *table_hdl,
                                     const bf_rt_id_t field_id,
                                     size_t msize,
                                     uint8_t *mask);
