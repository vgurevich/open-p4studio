...
/**
 * @brief Get value. Valid on fields of up to uint128_t array type.
 *   Each value is represented as uint8_t[16] and must be preallocated
 *   as such.
 *
 * @param[in] data_hdl          Data object handle
 * @param[in] field_id          Field ID
 * @param[in] val_sz            Size of the array. Must be multiply of 16.
 * @param[out] val              Pointer to the vector value to be filled in.
 *
 * @return Status of the API call
 */
bf_status_t bf_rt_data_field_get_value_u128_array(
    const bf_rt_table_data_hdl *data_hdl,
    const bf_rt_id_t field_id,
    size_t val_sz,
    uint8_t *val);
...
/**
 * @brief Get value array size. Valid on fields up to uint128_t array type
 *   Returned value describes how many values will be returned on call to
 *   bf_rt_data_field_get_value_u128_array, this means actual memory required
 *   is array_size * 16.
 *
 * @param[in] data_hdl          Data object handle
 * @param[in] field_id          Field ID
 ...
 * @return Status of the API call
 */
bf_status_t bf_rt_data_field_get_value_u128_array_size(
    const bf_rt_table_data_hdl *data_hdl,
    const bf_rt_id_t field_id,
    uint32_t *array_size);
