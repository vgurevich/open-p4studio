...
class BfRtTableData {
 public:
  ...
  /**
   * @brief Get value. Valid only on 128b register fields.
   *
   * @param[in] field_id Field ID
   * @param[out] value vector of uint8_t vectors of length 16 - one per pipe.
   *
   * @return Status of the API call
   */
  virtual bf_status_t getValue(
      const bf_rt_id_t &field_id,
      std::vector<std::vector<uint8_t>> *value) const = 0;
