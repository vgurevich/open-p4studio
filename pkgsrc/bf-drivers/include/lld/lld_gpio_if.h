...
#define BF_TOF3_IO_NUM_PIN_PAIR 8
...
/* We have separate macros for TF3 GPIO registers since its
 * MDIO block differs quite a bit */
#define GPIO_COMM_OFFSET_TOF3(reg) \
  (offsetof(tof3_reg,              \
            device_select.misc_all_regs.gpio_regs.gpio_common_regs.reg))

#define GPIO_PAIR_OFFSET_TOF3(idx, reg) \
  (offsetof(tof3_reg,                   \
            device_select.misc_all_regs.gpio_regs.gpio_pair_regs[idx].reg))
...
typedef enum {
  ...
  BF_TOF3_IO_PIN_PAIR_MAX = 7 /* keep this the last */
} bf_io_pin_pair_t;
...
typedef enum {
  ...
  BF_TOF3_GPIO_PIN_MAX = 15 /* keep this the last */
} bf_gpio_pin_t;
...
/* if a pair of pins are configured as GPIO, then, we program the
 * output levels before we (later on) set the direction as output
 * type */
...
/*
 *   Following MDIO APIs are non-blocking. Caller can initiate an
 * mdio transaction and get back later to check the result. After
 * initiating an mdio operation, if  the status (for write type
 * operation) or result (for read type operation) is not checked
 * before initiating next mdio transaction, then, the status or
 * result of the previous transaction would be unrecoverably erased.
 * If the previous transaction is incomplete when initiating  the
 * next transaction, the later one would return with appropriate
 * error code.
 */
...
/* Following MDIO transactions are blocking calls (sleep or busy
 * wait).
 */
...
/*
 * Following i2c APIs are non-blocking. Caller can initiate an i2c
 * transaction and get back later to check the result. After
 * initiating an i2c operation, if  the status (for write type
 * operation) or result (for read type operation) is not checked
 * before initiating next i2c transaction, then, the status or
 * result of the previous transaction would be unrecoverably erased.
 * If the previous transaction is incomplete when initiating  the
 * next transaction, the later one would return with appropriate
 * error code.
...
/* Following I2C transactions are the blocking versions of the calls
 * with a sleep of anticipated delay.
 */
/* reset the i2c bus */
...
/* Tofino carries out multi-byte operations using its internal
 * temporary buffers called statein and stateout buffers. Controller
 * takes the offset into this internal memory. we can remove
 * parameters <statein_addr> and <stateout_addr> from below APIs if
 * we do not want data to be preserved in the internal buffer for
 * any reason.  (does p4 program make use of this internal buffer
 * automatically??)
 */
