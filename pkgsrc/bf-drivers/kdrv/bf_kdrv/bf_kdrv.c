...
#define NEW_RHEL
  ...
  u32 *desc_mask_bits = &desc->masked;
  unsigned offset = desc->msi_attrib.entry_nr * PCI_MSIX_ENTRY_SIZE +
                    PCI_MSIX_ENTRY_VECTOR_CTRL;
  void __iomem *mask_base = desc->mask_base;
#elif LINUX_VERSION_CODE < KERNEL_VERSION(5, 17, 0) && !defined(NEW_RHEL)
  u32 mask_bits = desc->msix_ctrl;
  u32 *desc_mask_bits = &desc->msix_ctrl;
  unsigned offset = desc->msi_attrib.entry_nr * PCI_MSIX_ENTRY_SIZE +
                    PCI_MSIX_ENTRY_VECTOR_CTRL;
  void __iomem *mask_base = desc->mask_base;
#else
  ...
  if (mask_bits != *desc_mask_bits) {
    writel(mask_bits, mask_base + offset);
    readl(mask_base);
    *desc_mask_bits = mask_bits;
  }
}
...
static irqreturn_t bf_pci_irqhandler(int irq, struct bf_pci_dev *bfdev) {
  ...
  if (kpkt_dr_int_en && bfdev->info.irq != 0) {
    if (bfdev->mode == BF_INTR_MODE_LEGACY) {
      ...
      if ((bfdev->info.tof_type != BF_TOFINO_1) && bf_irq_is_tbus_msix(bfdev, irq)) {
        bf_kpkt_irqhandler(irq, bfdev->adapter_ptr);
      }
    }
  }
