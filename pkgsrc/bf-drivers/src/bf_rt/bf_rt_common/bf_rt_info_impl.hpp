...
class BfRtInfoImpl : public BfRtInfo {
 public:
  ...
  bf_status_t bfrtInfoGetLearns(
      std::vector<const BfRtLearn *> *learn_vec_ret) const override;
  bf_status_t bfrtLearnFromNameGet(std::string name,
                                   const BfRtLearn **learn_ret) const override;
  bf_status_t bfrtLearnFromIdGet(bf_rt_id_t id,
                                 const BfRtLearn **learn_ret) const override;
 ...
 private:
  ...
  void parseKeyType(const Cjson &node,
                    std::string &type,
                    size_t &width,
                    DataType *dataType,
                    uint64_t *default_value,
  ...
  std::unique_ptr<BfRtLearnField> parseLearnField(Cjson &learn_field_cjson,
                                                  size_t &offset);

  std::unique_ptr<BfRtLearn> parseLearn(const std::string &prog_name,
  ...
  // handles parsed from bf-rt/context jsons. We need to do this as the
  ...
  void parseKeyHelper(const Cjson *context_json_table_keys,
  ...
  // Learn Map
  std::map<bf_rt_id_t, std::string> learnIdMap;
  std::map<std::string, std::unique_ptr<BfRtLearn>> learnMap;
  std::map<std::string, const BfRtLearn *> fullLearnMap;
