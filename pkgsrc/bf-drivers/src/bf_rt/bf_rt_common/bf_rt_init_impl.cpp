...
std::map<std::pair<bf_dev_id_t, std::string>,
         ...
         PairHash>
    BfRtDevMgrImpl::bfRtDeviceStateMap;
...
bool BfRtDevMgrImpl::pkt_mgr_skip_ = false;
bool BfRtDevMgrImpl::mc_mgr_skip_ = false;
bool BfRtDevMgrImpl::port_mgr_skip_ = false;
bool BfRtDevMgrImpl::traffic_mgr_skip_ = false;

bf_status_t bf_rt_virtual_device_add(bf_dev_id_t dev_id,
                                     bf_dev_type_t dev_type,
                                     bf_device_profile_t *dev_profile,
                                     bf_dev_init_mode_t warm_init_mode) {
  auto &dev_mgr_obj = bfrt::BfRtDevMgr::getInstance();
  bf_dev_family_t dev_family = bf_dev_type_to_family(dev_type);
  return dev_mgr_obj.devMgrImpl()->bfRtDeviceAdd(
      dev_id, dev_family, dev_profile, NULL, warm_init_mode);
}
...
bf_status_t bf_rt_device_add(bf_dev_id_t dev_id,
  ...
  auto &dev_mgr_obj = bfrt::BfRtDevMgr::getInstance();
  return dev_mgr_obj.devMgrImpl()->bfRtDeviceAdd(
      dev_id, dev_family, dev_profile, dma_info, warm_init_mode);
}
...
bf_status_t BfRtInitImpl::bfRtModuleInit(bool pkt_mgr_skip,
                                         bool mc_mgr_skip,
                                         bool port_mgr_skip,
  ...
  callbacks.virtual_device_add = bf_rt_virtual_device_add;
  ...
  BfRtDevMgrImpl::bfRtDeviceConfigSet(
...
bf_status_t BfRtDevMgrImpl::bfRtInfoP4NamesGet(
  ...
  bf_status_t sts = BF_OBJECT_NOT_FOUND;
...
bf_status_t BfRtDevMgrImpl::bfRtDeviceAdd(
  ...
  switch (dev_family) {
    ...
    default:
      LOG_ERROR(
          "%s:%d Invalid device family, dev %d", __func__, __LINE__, dev_id);
  ...
  for (int i = 0; i < dev_profile->num_p4_programs; i++) {
    ...
    for (int j = 0; j < dev_profile->p4_programs[i].num_p4_pipelines; j++) {
      ...
      if (dev_profile->p4_programs[i].p4_pipelines[j].num_pipes_in_scope == 0) {
        ...
        uint32_t num_pipes;
        status = PipeMgrIntf::getInstance()->pipeMgrGetNumPipelines(dev_id,
                                                                    &num_pipes);
        if (status != BF_SUCCESS) {
          LOG_TRACE("%s:%d : Error : Failed to get number of pipes",
                    __func__,
                    __LINE__);
          return status;
        }
    ...
    // tables using keys
    status = duplicateEntryCheckEnable(
        bfRtInfoObjMap[std::pair<bf_dev_id_t, std::string>(dev_id, prog_name)]
            .get(),
        dev_id);
    if (status != BF_SUCCESS) {
    ...
    // shared_ptrs
    if (i != 0) {
      auto first_prog_state =
          bfRtDeviceStateMap[std::pair<bf_dev_id_t, std::string>(
              dev_id, first_p4_name)];
      this_prog_state->copyFixedObjects(*first_prog_state);
    }
  }
