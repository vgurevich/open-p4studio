...
bf_status_t PipeActionSpec::setValueCounterSpec(const BfRtTableDataField &field,
  ...
  status = counter_spec.setCounterDataFromValue(field_type, counter_val);
  if (status != BF_SUCCESS) {
    LOG_ERROR(
        "%s:%d ERROR : Not able to populate counter spec for field type %d",
        __func__,
        __LINE__,
...
bf_status_t PipeActionSpec::getValueCounterSpec(const BfRtTableDataField &field,
  ...
  status = counter_spec.getCounterData(field_type, &counter_val);
  if (status != BF_SUCCESS) {
    LOG_ERROR("%s:%d ERROR : Get counter data failed for field_type %d",
              __func__,
              __LINE__,
              static_cast<int>(field_type));
    return status;
...
bf_status_t PipeActionSpec::setValueRegisterSpec(
  ...
  uint64_t register_val[2] = {0, 0};
  bf_status_t status = BF_SUCCESS;

  if (value_ptr) {
    if (field_size == 128) {
      for (int idx = 0; idx < 16; idx++) {
        ((uint8_t *)register_val)[idx] = value_ptr[15 - idx];
      }
    } else {
      utils::BfRtTableFieldUtils::toHostOrderData(
          field, value_ptr, &register_val[0]);
    }
  } else {
    register_val[0] = value;
  }
  ...
  status = register_spec.populateStfulSpecFromData(&res_spec->data.stful);
  if (status != BF_SUCCESS) {
    LOG_ERROR("%s:%d Not able to copy the reg spec to action spec",
              __func__,
              __LINE__);
    return status;
  }
...
bf_status_t PipeActionSpec::getValueRegisterSpec(
    const BfRtTableDataField &field,
    std::vector<std::vector<uint8_t>> *value) const {
  const auto &bfrt_registers = register_spec.getRegisterVec();

  std::vector<uint8_t> u8v(16, 0);
  for (const auto &bfrt_register_data : bfrt_registers) {
    uint64_t val[2] = {0};
    bfrt_register_data.getData(field, val);
    std::memcpy(u8v.data(), val, 16);
    value->push_back(u8v);
  }
...
bf_status_t PipeActionSpec::setValueMeterSpec(const pipe_meter_spec_t &meter) {
  ...
  status = meter_spec.setMeterDataFromMeterSpec(meter);
  if (status != BF_SUCCESS) {
    LOG_ERROR("%s:%d ERROR : populating internal meter spec failed",
              __func__,
              __LINE__);
    return status;
  }
...
bf_status_t PipeActionSpec::setValueMeterSpec(const BfRtTableDataField &field,
  ...
  status = meter_spec.setMeterDataFromValue(field_type, meter_val);
  if (status != BF_SUCCESS) {
    LOG_ERROR("%s:%d ERROR : Set meter value failed for field_type %d",
              __func__,
              __LINE__,
              static_cast<int>(field_type));
    return status;
  ...
  switch (field_type) {
    ...
    default:
      LOG_ERROR(
          "%s:%d ERROR : Invalid field type %d while trying to set "
          "meter data",
          __func__,
          __LINE__,
          static_cast<int>(field_type));
...
bf_status_t PipeActionSpec::getValueMeterSpec(const BfRtTableDataField &field,
  ...
  status = meter_spec.getMeterDataFromValue(field_type, &meter_val);
  if (status != BF_SUCCESS) {
    LOG_ERROR("%s:%d ERROR : Get meter data failed for fiel type %d",
              __func__,
              __LINE__,
              static_cast<int>(field_type));
    return status;
...
bf_status_t PipeActionSpec::setValueLPFSpec(const pipe_lpf_spec_t &lpf) {
  ...
  status = lpf_spec.setLPFDataFromLPFSpec(&lpf);
  if (status != BF_SUCCESS) {
    LOG_ERROR("%s:%d ERROR : populating internal meter spec failed",
              __func__,
              __LINE__);
...
bf_status_t BfRtRegister::setData(const BfRtTableDataField &tableDataField,
  ...
  switch (field_size) {
    case 1:
      ...
      stful.bit = (value[0] & 0x1);
      break;
    case 8:
      if (hi_value) {
        stful.dbl_byte.hi = (value[0] & 0xff);
      } else if (lo_value) {
        stful.dbl_byte.lo = (value[0] & 0xff);
      } else {
        stful.byte = (value[0] & 0xff);
      }
      break;
    case 16:
      if (hi_value) {
        stful.dbl_half.hi = (value[0] & 0xffff);
      } else if (lo_value) {
        stful.dbl_half.lo = (value[0] & 0xffff);
      } else {
        stful.half = (value[0] & 0xffff);
      }
      break;
    case 32:
      if (hi_value) {
        stful.dbl_word.hi = (value[0] & 0xffffffff);
      } else if (lo_value) {
        stful.dbl_word.lo = (value[0] & 0xffffffff);
      } else {
        stful.word = (value[0] & 0xffffffff);
      }
      break;
    case 64:
      if (hi_value) {
        stful.dbl_dbl.hi = value[0];
      } else if (lo_value) {
        stful.dbl_dbl.lo = value[0];
      } else {
        stful.dbl = value[0];
      }
      break;
    case 128:
      BF_RT_ASSERT(hi_value == false);
      BF_RT_ASSERT(lo_value == false);
      stful.dbl_dbl.hi = value[1];
      stful.dbl_dbl.lo = value[0];
      break;
    default:
      LOG_ERROR(
          "%s:%d ERROR : Invalid field size encountered for field id %d,"
          " size %zu",
          __func__,
          __LINE__,
          tableDataField.getId(),
...
bf_status_t BfRtRegister::getData(const BfRtTableDataField &tableDataField,
                                  uint64_t value[2]) const {
  auto fieldTypes = tableDataField.getTypes();
  size_t field_size = tableDataField.getSize();
  ...
  switch (field_size) {
    ...
    case 8:
      if (hi_value) {
        value[0] = stful.dbl_byte.hi;
      } else if (lo_value) {
        value[0] = stful.dbl_byte.lo;
      } else {
        value[0] = stful.byte;
      }
      break;
    case 16:
      if (hi_value) {
        value[0] = stful.dbl_half.hi;
      } else if (lo_value) {
        value[0] = stful.dbl_half.lo;
      } else {
        value[0] = stful.half;
      }
      break;
    case 32:
      if (hi_value) {
        value[0] = stful.dbl_word.hi;
      } else if (lo_value) {
        value[0] = stful.dbl_word.lo;
      } else {
        value[0] = stful.word;
      }
      break;
    case 64: {
      if (hi_value) {
        value[0] = stful.dbl_dbl.hi;
      } else if (lo_value) {
        value[0] = stful.dbl_dbl.lo;
      } else {
        value[0] = stful.dbl;
      }
      break;
...
void RegisterSpecData::addRegisterToVec(
    const BfRtTableDataField &tableDataField, const uint64_t value[2]) {
  if (registers_.empty()) {
    BfRtRegister bfrt_register;
    registers_.push_back(bfrt_register);
  }

  registers_[0].setData(tableDataField, value);
}
...
bf_status_t RegisterSpecData::populateStfulSpecFromData(
    pipe_stful_mem_spec_t *stful_spec) const {
  if (!registers_.size()) {
...
bf_status_t CounterSpecData::setCounterDataFromValue(
    const DataFieldType &field_type, const uint64_t &value) {
  switch (field_type) {
    case DataFieldType::COUNTER_SPEC_BYTES:
...
bf_status_t CounterSpecData::getCounterData(const DataFieldType &field_type,
...
bf_status_t MeterSpecData::setMeterDataFromValue(const DataFieldType &fieldType,
                                                 const uint64_t &value) {
  switch (fieldType) {
    case METER_SPEC_CIR_PPS:
      setCIRPps(value);
    ...
    default:
      LOG_ERROR(
          "%s:%d ERROR : Invalid field type %d encountered for set meter data",
          __func__,
          __LINE__,
...
bf_status_t MeterSpecData::setMeterDataFromMeterSpec(
    const pipe_meter_spec_t &mspec) {
  if (mspec.cir.type == METER_RATE_TYPE_KBPS) {
    if (mspec.pir.type != METER_RATE_TYPE_KBPS) {
      LOG_ERROR(
          "%s:%d ERROR : Invalid meter rate type %d encountered while "
          "trying to set "
          "meter data",
          __func__,
          __LINE__,
    ...
    if (mspec.pir.type != METER_RATE_TYPE_PPS) {
      LOG_ERROR(
          "%s:%d ERROR : Invalid meter rate type %d encountered while "
          "trying to set "
          "meter data",
...
bf_status_t MeterSpecData::getMeterDataFromValue(const DataFieldType &fieldType,
                                                 uint64_t *value) const {
  switch (fieldType) {
    case METER_SPEC_CIR_PPS:
    ...
    default:
      LOG_ERROR(
          "%s:%d ERROR : Invalid field type %d while trying to get "
          "meter data",
          __func__,
          __LINE__,
          static_cast<int>(fieldType));
...
bf_status_t LPFSpecData::setLPFDataFromLPFSpec(
    const pipe_lpf_spec_t *lpf_spec) {
...
bf_status_t WREDSpecData::setWREDDataFromWREDSpec(
    const pipe_wred_spec_t *wred_spec) {
...
bf_status_t BfRtMatchActionTableData::getValue(
  ...
  this->getValue(field_id, &val);

  for (const auto &u8v : val) {
    uint64_t reg_val[2] = {0};
    std::memcpy(reg_val, u8v.data(), 16);
    value->push_back(reg_val[0]);
  }
  ...
  for (const auto &fieldType : types_vec) {
    switch (fieldType) {
      ...
      case (DataFieldType::REGISTER_SPEC_LO): {
        ...
        if (sts != BF_SUCCESS) {
          LOG_ERROR(
              "ERROR: %s:%d %s : Set register spec failed for field id %d "
              "action id "
              "%d",
              __func__,
              __LINE__,
      ...
      case (DataFieldType::METER_SPEC_PBS_KBITS): {
        ...
        if (sts != BF_SUCCESS) {
          LOG_ERROR(
              "ERROR: %s:%d %s : Set meter spec failed for field id %d "
              "action id "
              "%d",
              __func__,
              __LINE__,
    ...
    switch (fieldType) {
      ...
      case (DataFieldType::WRED_INDEX): {
        ...
        if (elem != resource_map.end()) {
          ...
          LOG_ERROR("%s:%d %s : Not supported config for field with id %d",
                    __func__,
                    __LINE__,
                    this->table_->table_name_get().c_str(),
                    tableDataField->getId());
...
BfRtSelectorTableData::BfRtSelectorTableData(
  ...
  for (const auto &field : {"$MAX_GROUP_SIZE", "$ADT_OFFSET"}) {
    bf_rt_id_t field_id;
    bf_status_t sts = tbl_obj->dataFieldIdGet(field, &field_id);
    if (sts != BF_SUCCESS) {
      // Offset field may be not present, just ignore
      if (strcmp(field, "$ADT_OFFSET") == 0) {
        this->adt_offset_ = 0xdeadbeef;
        continue;
      }
      LOG_ERROR("%s:%d ERROR Field Id for %s field not found",
                __func__,
                __LINE__,
                field);
      BF_RT_DBGCHK(0);
    ...
    if (fields.empty() ||
        (std::find(fields.begin(), fields.end(), field_id) != fields.end())) {
      ...
      sts = tbl_obj->defaultDataValueGet(
          field_id, 0 /* this->actionIdGet_() */, &default_value);
      if (sts != BF_SUCCESS) {
        // For some reason we were unable to get the default value from the
        // bfrt json. So just set it to 0
        LOG_ERROR("%s:%d ERROR Unable to get the default value for field %d %s",
                  __func__,
                  __LINE__,
                  field_id,
                  field);
      ...
      sts = this->setValueInternal(field_id, 0, default_value, nullptr);
      if (sts != BF_SUCCESS) {
        // Unable to set the default value
        LOG_ERROR("%s:%d ERROR Unable to set the default value for field %d %s",
                  __func__,
                  __LINE__,
                  field_id,
                  field);
...
bf_status_t BfRtSelectorTableData::setValueInternal(const bf_rt_id_t &field_id,
  ...
  if (field_type == DataFieldType::MAX_GROUP_SIZE) {
    max_grp_size_ = static_cast<uint32_t>(val);
  } else if (field_type == DataFieldType::ADT_OFFSET) {
    adt_offset_ = static_cast<uint32_t>(val);
  }
  return BF_SUCCESS;
}
...
bf_status_t BfRtCounterTableData::setValueInternal(const bf_rt_id_t &field_id,
  ...
  sts = getCounterSpecObj().setCounterDataFromValue(field_type, val);
  if (sts != BF_SUCCESS) {
    LOG_ERROR("%s:%d %s ERROR : Set counter data failed for field_id %d",
              __func__,
              __LINE__,
              this->table_->table_name_get().c_str(),
              field_id);
...
bf_status_t BfRtCounterTableData::getValueInternal(const bf_rt_id_t &field_id,
  ...
  sts = getCounterSpecObj().getCounterData(field_type, &counter);
  if (sts != BF_SUCCESS) {
    LOG_ERROR("%s:%d %s ERROR : Get counter data failed for field_id %d",
              __func__,
              __LINE__,
              this->table_->table_name_get().c_str(),
              field_id);
...
bf_status_t BfRtMeterTableData::setValueInternal(const bf_rt_id_t &field_id,
  ...
  sts = meter_spec_.setMeterDataFromValue(field_type, val);
  if (sts != BF_SUCCESS) {
    LOG_ERROR("%s:%d %s ERROR : Set meter value failed for field_id %d",
              __func__,
              __LINE__,
              this->table_->table_name_get().c_str(),
              field_id);
...
bf_status_t BfRtMeterTableData::getValueInternal(const bf_rt_id_t &field_id,
  ...
  sts = meter_spec_.getMeterDataFromValue(field_type, &meter);
  if (sts != BF_SUCCESS) {
    LOG_ERROR("%s:%d %s ERROR : Get value failed for field_id %d",
              __func__,
              __LINE__,
...
bf_status_t BfRtRegisterTableData::setValueInternal(const bf_rt_id_t &field_id,
  ...
  if (value_ptr && size == 16) {
    // 128 bit register
    for (int idx = 0; idx < 16; idx++) {
      ((uint8_t *)val)[idx] = value_ptr[15 - idx];
    }
  } else {
    DataFieldType field_type;
    // Here we pass an empty set because this can be used for any field type
  ...
  const BfRtTableDataField *tableDataField = nullptr;
  auto sts = this->table_->getDataField(field_id, &tableDataField);
  if (sts != BF_SUCCESS) {
    LOG_ERROR("%s:%d %s ERROR : Data field id %d not found",
              __func__,
              __LINE__,
  ...
  this->getRegisterSpecObj().addRegisterToVec(*tableDataField, val);
  return BF_SUCCESS;
}
...
bf_status_t BfRtRegisterTableData::getValue(
  ...
  for (const auto &bfrt_register_data : bfrt_registers) {
    uint64_t val[2] = {0};
    bfrt_register_data.getData(*tableDataField, val);
    std::memcpy(u8v.data(), val, 16);
    value->push_back(u8v);
  }
  return BF_SUCCESS;
