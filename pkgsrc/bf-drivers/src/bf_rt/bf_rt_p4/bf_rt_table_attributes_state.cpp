...
// The below function is responsible to free the spec
void bfRtIdleTmoExpiryTaskExecute(
  ...
  auto callback_cpp = hs ? std::get<2>(t) : std::get<1>(t);
  auto callback_c = hs ? std::get<4>(t) : std::get<3>(t);
  auto table = std::get<5>(t);
  auto cookie_actual = std::get<6>(t);
  ...
  if (callback_c) {
    callback_c(&dev_tgt,
               reinterpret_cast<const bf_rt_table_key_hdl *>(matchKey),
               cookie_actual);
  }
...
// Simple callback that does not fetch/expect match spec. The only
// identifier for entry will be mat_ent_hdl.
// Only c callback is supported and mat_ent_hdl is returned instead of key.
//
// This is used in the HLP/LLP separation mode with virtual devices.
// In such case the entry info is only available in HLP and LLP cannot
// derive this informaion - the registered callback_c will handle
// the mat_ent_hdl and pass it to the HLP driver to allow
// construction of the match_spec/entry key of the table
void bfRtIdleTmoExpiryInternalSimpleCb(bf_dev_id_t dev_id,
  ...
  if (callback_c)
    callback_c(&dev_tgt, (bf_rt_table_key_hdl *)&mat_ent_hdl, cookie_actual);
}
...
void bfRtIdleTmoExpiryInternalCb(bf_dev_id_t dev_id,
  ...
  /* Callback not configured, skip */
  if (callback_c == nullptr && callback_cpp == nullptr) return;
  ...
  auto attributes_aging_sp =
      std::make_shared<BfRtStateTableAttributesAging>(*attributes_aging_state);
...
pipe_status_t selUpdatePipeMgrInternalCb(pipe_sess_hdl_t sess_hdl,
  ...
  auto bf_status = sel_table->getGrpIdFromHndl(
      *session_obj, bf_rt_tgt, sel_grp_hdl, &sel_grp_id);
  ...
  bf_status = sel_table->getActMbrIdFromHndl(
      *session_obj, bf_rt_tgt, adt_ent_hdl, &act_mbr_id);
...
void BfRtStateTableAttributesAging::stateTableAttributesAgingReset() {
  ...
  idle_cb_c_ = nullptr;
  activate_cb_cpp_ = nullptr;
  activate_cb_c_ = nullptr;
  table_ = nullptr;
  cookie_ = nullptr;
}
