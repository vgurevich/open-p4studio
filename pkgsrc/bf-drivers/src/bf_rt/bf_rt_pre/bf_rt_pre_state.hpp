...
#ifndef _BF_RT_PRE_STATE_HPP
#define _BF_RT_PRE_STATE_HPP
...
class BfRtPREStateObj {
 public:
  ...
  bf_status_t statePREIdAdd(const PREStateIdType &id_type,
                            const bf_rt_id_t &id,
                            const bf_rt_id_t &hdl);

  bf_status_t statePREIdDel(const PREStateIdType &id_type,
                            const bf_rt_id_t &id);
  ...
  bf_status_t statePREIdGet(const PREStateIdType &id_type,
                            const bf_rt_id_t &hdl,
                            bf_rt_id_t *id) const;

  bf_status_t statePREHdlGet(const PREStateIdType &id_type,
                             const bf_rt_id_t &id,
                             bf_rt_id_t *hdl) const;

  bf_status_t statePREIdGetFirst(const PREStateIdType &id_type,
                                 bf_rt_id_t *first_id,
                                 bf_rt_id_t *first_hdl) const;

  bf_status_t statePREIdGetNext(const PREStateIdType &id_type,
                                const bf_rt_id_t &id,
                                bf_rt_id_t *next_id,
 ...
 private:
  ...
  std::map<bf_rt_id_t, bf_mc_node_hdl_t> node_id_to_hdl_map;
  // ECMP id to ECMP handle map
  std::map<bf_rt_id_t, bf_mc_ecmp_hdl_t> ecmp_id_to_hdl_map;

  // MGID handle to MGID map
  std::unordered_map<bf_mc_mgrp_hdl_t, bf_mc_grp_id_t> mgid_hdl_to_mgid_map;
  // Node handle to node id map
  std::unordered_map<bf_mc_node_hdl_t, bf_rt_id_t> node_hdl_to_id_map;
  // ECMP handle to ECMP id map
  std::unordered_map<bf_mc_ecmp_hdl_t, bf_rt_id_t> ecmp_hdl_to_id_map;
};

}  // namespace bfrt

#endif  // _BF_RT_PRE_STATE_HPP
