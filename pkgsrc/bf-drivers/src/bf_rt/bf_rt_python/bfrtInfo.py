...
class BfRtInfoError(Exception):
    def __init__(self, str_rep, info_init, sts, *args,**kwargs):
        self.sts = sts
...
class BfRtInfo:
    ...
    def __init__(self, cintf, name):
        ...
        if not sts == 0:
            err_msg = "BfRtInfo init handle failed for {}!".format(self.name)
            raise BfRtInfoError(err_msg, None, -1)
        sts = self._init_tables()
        if not sts == 0:
            err_msg = "BfRtInfo init tables failed for {}!".format(self.name)
            raise BfRtInfoError(err_msg, None, -1)
        sts = self._init_learns()
        if not sts == 0:
            err_msg = "BfRtInfo init learns failed for {}!".format(self.name)
            raise BfRtInfoError(err_msg, None, -1)
        sts = self._init_num_pipes()
        if not sts == 0:
            print("BfRtInfo init num pipeline failed for {}!".format(self.name))
            return -1
    ...
    def _init_num_pipes(self):
        pipes_num = c_int(-1)
        sts = self._cintf.get_driver().bf_rt_info_num_pipeline_info_get(self._handle, byref(pipes_num))
        if not sts == 0:
            print("CLI Error: get num pipeline info {} failed with status {}.".format(self.name, self._cintf.err_str(sts)))
            return sts
        self.num_pipes = pipes_num.value
        return 0
