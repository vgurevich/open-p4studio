...
class BfRtTable:
    ...
    class BfRtTableField:
        ...
        def _init_mask(self):
            ...
            if self.category == "key":
                d_type = self.table.data_type_map(self.data_type)
                if d_type == "BYTE_STREAM" or d_type == "UINT" or d_type == "INT":
                    arr_size = (self.size + 7) // 8;
                    arrtype = c_ubyte * arr_size
                    field_mask = arrtype()
                    sts = self.table._cintf.get_driver().bf_rt_key_field_mask_get(self.table._handle, self.id, arr_size, field_mask)
                    if not sts == 0:
                        print("CLI Error: get key field mask for {} failed. [{}]".format(self.name, self.table._cintf.err_str(sts)))
                    self.mask = 0
                    for j in range(0, arr_size):
                        self.mask |= field_mask[j] << (8 * (arr_size - 1 - j))
        ...
        def parse_input(self, method_name, value):
            ...
            if ((self.category == "key" and arg_key_type == "EXACT" and arg_data_type in ["UINT", "BYTE_STREAM"])
                or (self.category == "data" and arg_data_type in ["UINT", "BYTE_STREAM", "BOOL", "INT"])):
        ...
        def deparse_output(self, value):
            ...
            elif ((self.category == "key" and self.table.key_type_map(self.type) == "EXACT") or (self.category == "data" and self.table.data_type_map(self.data_type) in ["UINT", "BYTE_STREAM", "INT"])):
        ...
        def _stringify_int(self, value):
            ...
            # to handle dev.device_configuration ints in decimal form for human conveniance
            if self.name.decode('ascii') in ["num_pipes", "num_stages", "num_max_ports", "num_front_ports", "pcie_cpu_port"]:
                format_str = "{}"

            # to deal with dev.device_configuration timeouts values so they are in decimal form for human conveniance
            if self.name.decode('ascii') in ["lrt_dr_timeout_usec", "flow_learn_timeout_usec"]:
                format_str = "{}"
        ...
        def _stringify_cont(self, cont_val, cont_name, i_lvl=1):
            ...
            for el in cont_val:
                ...
                for f_name, f_val in sorted(el.items(),  key=lambda x: x[0]):
                    ...
                    # Check if this inner element is also a container.
                    # Do not call _stringify_cont() again if inner element is a list of integers.
                    if type(f_val) == list and all(isinstance(item, int) for item in f_val) == False:
    ...
    def table_type_map(table_type):
        if table_type == 0:
           ...
           return "TM_PORT_CFG"
            ...
            return "TM_L1_NODE_SCHED_CFG"
        elif table_type == 77:
            return "TM_L1_NODE_SCHED_SHAPING"
        elif table_type == 78:
            return "DEV_WARM_INIT"
        elif table_type == 79:
            return "INVLD"
        else:
            return "TODO"
    ...
    """
    @staticmethod
    def fill_c_byte_arr(content, size_bits):
        bytes_ = (size_bits + 7) // 8 # floor division
    ...
    @staticmethod
    def from_c_byte_arr(value, size_bits):
        bytes_ = (size_bits + 7) // 8 # floor division
        content = 0
        for i in range(0, bytes_):
    ...
    def _set_data_field(self, content, data_handle, data_fields, action = None):
        for name, info in data_fields.items():
            ...
            if self.data_type_map(info.data_type) == "UINT":
                if not info.is_ptr:
                    value = c_ulonglong(content[name])
                    ...
                    value, bytes_ = self.fill_c_byte_arr(content[name], info.size)
                    sts = self._cintf.get_driver().bf_rt_data_field_set_value_ptr(data_handle, info.id, value, bytes_)
            if self.data_type_map(info.data_type) == "INT":
                value = c_int64(content[name])
                sts = self._cintf.get_driver().bf_rt_data_field_set_int64(data_handle, info.id, value)
            ...
            if self.data_type_map(info.data_type) == "CONTAINER":
                ...
                for i in range(cont_list_len):
                    ...
                    if action:
                        sts =  self._cintf.get_driver().bf_rt_table_action_data_allocate_container(self._handle, info.id, self.actions[action]["id"], byref(inner_data_handles[i]))
                    else:
                    ...
                    if not sts == 0:
                        ...
                        return sts
                    if action:
                        sts = self._set_data_field(content[name][i], inner_data_handles[i], info._cont_data_fields, action)
                    else:
    ...
    def _set_data_fields(self, content, data_handle, action):
        ...
        return self._set_data_field(content, data_handle, data_fields, action)
        ...
        for name, info in data_fields.items():
            ...
            if self.data_type_map(info.data_type) == "BYTE_STREAM":
                if ('$bfrt_field_class', 'register_data') in info.annotations:
                    size = c_uint(0)
                    sts = self._cintf.get_driver().bf_rt_data_field_get_value_u128_array_size(data_handle, info.id, byref(size))
                    if sts == 0:
                        # u128 API requires 128bits allocated as 16 bytes per index
                        arrtype = c_ubyte * (16 * size.value)
                        value = arrtype()
                        sts = self._cintf.get_driver().bf_rt_data_field_get_value_u128_array(data_handle, info.id, size.value, value)
                        content[name] = []
                        # Decode value from bytes to variable
                        for i in range(0, size.value):
                            reg_val = 0
                            for j in range(0, 16):
                               reg_val |= value[i * 16 + j] << (8 * j)
                            content[name].append(reg_val)
                else:
                    value, bytes_ = self.fill_c_byte_arr(0, info.size)
            ...
            if self.data_type_map(info.data_type) == "INT":
                if not info.is_ptr:
                    value = c_int64(0)
                    sts = self._cintf.get_driver().bf_rt_data_field_get_int64(data_handle, info.id, byref(value))
                    content[name] = value.value
            if self.data_type_map(info.data_type) == "FLOAT":
                if not info.is_ptr:
                    value = c_float(0)
    ...
    def reset_entry(self, key_content, entry_handle=None):
        if entry_handle != None and "get_by_handle" not in self.supported_commands:
            raise BfRtTableError("{} Error: reseting entry by handle not supported.".format(self.name), self, -1)
        ...
        if entry_handle != None:
            ...
            sts = self._cintf.bf_rt_table_entry_key_get(self._handle,
                                                  ...
                                                  key_handle)
            if sts != 0:
                raise BfRtTableError("Error: table_entry_reset failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        else:
            key_handle = self._make_call_keys(key_content)
            if key_handle == -1:
                return -1
            entry_tgt = self._cintf.get_dev_tgt()
        sts = self._cintf.bf_rt_table_entry_reset(self._handle,
                                                self._cintf.get_session(),
                                                entry_tgt,
                                                key_handle)
        self._cintf.get_driver().bf_rt_table_key_deallocate(key_handle)
    ...
    def get_entry(self, key_content, from_hw=False, print_entry=True, key_handle=None, entry_handle=None):
        ...
        #Do not deallocate the key if it was passed to this function.
        #As the key will be deallocated by bfrt internal callback.
        if not is_key_set:
           self._cintf.get_driver().bf_rt_table_key_deallocate(key_handle)
        self._cintf.get_driver().bf_rt_table_data_deallocate(data_handle)
        return entry
    ...
    def get_capacity(self, from_hw=False):
        flags = 0
        if from_hw:
            flags = 1
        count = c_size_t(0)
        session = self._cintf.get_session()
        dev_target = self._cintf.get_dev_tgt()
        sts = self._cintf.bf_rt_table_size_get(self._handle, session, dev_target, byref(count), flags)
        if not sts == 0:
    ...
    def set_supported_attributes_to_supported_commands(self):
        ...
        for i in range(len(attributes_arr)):
            ...
            elif attributes_arr[i] == 1:
                self.supported_commands.append("dynamic_key_mask_set")
                self.supported_commands.append("dynamic_key_mask_get")
            ...
            elif attributes_arr[i] == 7:
                self.supported_commands.append("pre_device_config_set")
                self.supported_commands.append("pre_device_config_get")
    ...
    def pre_device_config_set(self, global_rid=None, port_protection=None, fast_failover=None, max_nodes_before_yield_count=None, max_nodes_threshold_node_count=None, max_nodes_threshold_node_port_lag_count=None):
        attr_hdl = self._cintf.handle_type()
        sts = self._cintf.get_driver().bf_rt_table_pre_device_config_attributes_allocate(self._handle, byref(attr_hdl))
        if not sts == 0:
            raise BfRtTableError("Error: bf_rt_table_pre_device_config_attributes_allocate failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        if global_rid != None:
            sts = self._cintf.get_driver().bf_rt_attributes_pre_global_rid_set(attr_hdl, c_uint(global_rid));
            if not sts == 0:
                self._attr_deallocate(attr_hdl)
                raise BfRtTableError("Error: bf_rt_attributes_pre_global_rid_set failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        if port_protection !=None:
            sts = self._cintf.get_driver().bf_rt_attributes_pre_port_protection_set(attr_hdl, c_bool(port_protection));
            if not sts == 0:
                self._attr_deallocate(attr_hdl)
                raise BfRtTableError("Error: bf_rt_attributes_pre_port_protection_set failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        if fast_failover !=None:
            sts = self._cintf.get_driver().bf_rt_attributes_pre_fast_failover_set(attr_hdl, c_bool(fast_failover));
            if not sts == 0:
                self._attr_deallocate(attr_hdl)
                raise BfRtTableError("Error: bf_rt_attributes_pre_fast_failover_set failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        if max_nodes_before_yield_count != None:
            sts = self._cintf.get_driver().bf_rt_attributes_pre_max_nodes_before_yield_set(attr_hdl, c_uint(max_nodes_before_yield_count));
            if not sts == 0:
                self._attr_deallocate(attr_hdl)
                raise BfRtTableError("Error: bf_rt_attributes_pre_max_nodes_before_yield_set failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        if max_nodes_threshold_node_count != None and max_nodes_threshold_node_port_lag_count != None:
            sts = self._cintf.get_driver().bf_rt_attributes_pre_max_nodes_threshold_set(attr_hdl, c_uint(max_nodes_threshold_node_count), c_uint(max_nodes_threshold_node_port_lag_count));
            if not sts == 0:
                self._attr_deallocate(attr_hdl)
                raise BfRtTableError("Error: bf_rt_attributes_pre_max_nodes_threshold_set failed on table {}. [{}]".format(self.name, self._cintf.err_str(sts)), self, sts)
        self._attr_set(attr_hdl)
        self._attr_deallocate(attr_hdl)
    ...
    def _wrap_idle_timeout_callback(self, callback):
        def callback_wrapper(target, handle, cookie):
            ...
            parser_id = target.contents.prsr_id
            entry = self.get_entry(key_content=None, print_entry = False, key_handle=handle)
            callback(dev_id, pipe_id, direction, parser_id, entry, False)
            return 0
        return self._cintf.idle_timeout_cb_type(callback_wrapper)

    def _wrap_ent_act_callback(self, callback):
        def callback_wrapper(target, handle, cookie):
            dev_id = target.contents.dev_id
            pipe_id = target.contents.pipe_id
            direction = target.contents.direction
            parser_id = target.contents.prsr_id
            entry = self.get_entry(key_content=None, print_entry = False, key_handle=handle)
            callback(dev_id, pipe_id, direction, parser_id, entry, True)
            return 0
        return self._cintf.idle_timeout_cb_type(callback_wrapper)
    ...
    def set_idle_table_notify_mode(self, enable, callback, interval, max_ttl, min_ttl):
        ...
        if not sts == 0:
            ...
            return -1
        # There is separate callback function to be registered per hit state (active vs idle),
        # but default bfrtcli handler is the same.
        self.idle_tmo_callback = self._wrap_idle_timeout_callback(callback)
        self.idle_act_callback = self._wrap_ent_act_callback(callback)
        sts = self._cintf.get_driver().bf_rt_attributes_idle_table_notify_mode_2way_set(attr_hdl,
                                                                                   c_bool(bool(enable)),
                                                                                   self.idle_tmo_callback,
                                                                                   self.idle_act_callback,
                                                                                   c_uint(interval),
                                                                                   c_uint(max_ttl),
                                                                                   c_uint(min_ttl),
                                                                                   c_void_p(0))
    ...
    def meter_byte_count_adjust_set(self, byte_count):
        ...
        sts = self._cintf.get_driver().bf_rt_attributes_meter_byte_count_adjust_set(attr_hdl, c_int(byte_count))
        if not sts == 0:
            self._attr_deallocate(attr_hdl)
    ...
    def dyn_key_mask_get(self):
        ...
        for f in fields:
            ...
            arrtype = c_ubyte * num_bytes.value
            bytes_ = arrtype()
            ...
            ret_val[field_name.value] = self.from_c_byte_arr(bytes_, num_bytes.value * 8)
        self._attr_deallocate(attr_hdl)
        return ret_val
    ...
    def parse_str_input(self, method_name, key_strs, data_strs, action=None):
        ...
        if key_strs != None:
            for name, input_ in key_strs.items():
                ...
                if not success:
                    print("ERROR: Can`t parse key in", method_name, "function. Invalid key:", input_)
                    return -1, {}
        ...
        if data_strs != None:
            ...
            for name, input_ in data_strs.items():
                ...
                if not success:
                    print("ERROR: Can`t parse data in", method_name, "function. Invalid value:", input_)
                    return {}, -1
