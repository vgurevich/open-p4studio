...
from bfrtLearn import BfRtLearn
from bfrtInfo import BfRtInfo
from bfrtInfo import BfRtInfoError
from bfrtTableEntry import TableEntry, target_check_and_set

# Fixed Tables created at child Nodes of the root 'bfrt' Node.
_bfrt_fixed_nodes = ["port", "pre", "mirror", "pktgen", "tf1", "tf2", "tf3"]



# Nodes that can be present in root and program nodes.
# prefix     to be used after program name node
# tbl_name   table full name defined in bf-rt.json
_bfrt_clone_nodes = [{'prefix':"pipe",'tbl_name':"tf2.snapshot.phv"}]
_bfrt_context = {}
promp_node = None
bfrt = None
...
class CIntfBFRT:
    ...
    def __init__(self, dev_id, table_cls, info_cls, learn_cls):
        self._dev_id = dev_id
    ...
    def _init_wrapper(self):
        ...
        def bf_rt_table_size_get(tbl_hdl, session, dev_tgt, count, flags):
            if self.gflags == True:
        ...
        def bf_rt_table_entry_reset(tbl_hdl, session, dev_tgt, key):
            if self.gflags == True:
                flags = c_uint64(0)
                return self._driver.bf_rt_table_entry_reset(tbl_hdl, session, dev_tgt, flags, key)
            else:
                return self._driver.bf_rt_table_entry_reset(tbl_hdl, session, dev_tgt, key)
        setattr(self, 'bf_rt_table_entry_reset', bf_rt_table_entry_reset)
    ...
    def _begin_transaction(self, atomic=False):
...
class BfDrvClientCallbacks(Structure):
    """
    This class represent a  bf_drv_client_handle_t structure. It is used for
    registering a device_add callback, that could later reload the bfrtcli
    when warm-init is triggered.
    """
    pass

BfDrvClientCallbacks.__slots__ = [
    'pkt_mgr_dev_add',
    'device_add',
    'virtual_device_add',
    'device_del',
    ...
    'err_intr_mode',
    'lock',
    'wait_for_swcfg_replay_end',
    'create_dma',
    'disable_input_pkts',
    'wait_for_flush',
    'core_reset',
    'unlock_reprogram_core',
    'config_complete',
    'enable_input_pkts',
    'error_cleanup',
    'warm_init_quick',
    'complete_hitless_hw_read',
    'complete_hitless_swcfg_replay',
    'compute_delta_changes',
    'push_delta_changes',
    'device_mode_virtual_dev_slave',
    'register_port_corr_action',
    'port_delta_push_done',
    'port_serdes_upgrade',
]

device_add_fn = CFUNCTYPE((c_int), (c_uint), (c_int), (c_void_p), (c_void_p), (c_uint32))

BfDrvClientCallbacks._fields_ = [
    ('pkt_mgr_dev_add',c_void_p),
    ('device_add', device_add_fn),
    ('virtual_device_add', c_void_p),
    ('device_del', c_void_p),
    ('device_log', c_void_p),
    ('device_restore', c_void_p),
    ('port_add', c_void_p),
    ('port_del', c_void_p),
    ('port_status', c_void_p),
    ('port_speed', c_void_p),
    ('port_admin_state', c_void_p),
    ('err_intr_mode', c_void_p),
    ('lock', c_void_p),
    ('wait_for_swcfg_replay_end', c_void_p),
    ('create_dma', c_void_p),
    ('disable_input_pkts', c_void_p),
    ('wait_for_flush', c_void_p),
    ('core_reset', c_void_p),
    ('unlock_reprogram_core', c_void_p),
    ('config_complete', c_void_p),
    ('enable_input_pkts', c_void_p),
    ('error_cleanup', c_void_p),
    ('warm_init_quick', c_void_p),
    ('complete_hitless_hw_read', c_void_p),
    ('complete_hitless_swcfg_replay', c_void_p),
    ('compute_delta_changes', c_void_p),
    ('push_delta_changes', c_void_p),
    ('device_mode_virtual_dev_slave', c_void_p),
    ('register_port_corr_action', c_void_p),
    ('port_delta_push_done', c_void_p),
    ('port_serdes_upgrade', c_void_p)]
...
class BFNode(BFContext):
    ...
    def _add_child(self, child):
        ...
        child_name = child._get_name()
        child_name = validate_p4_entity_name(child_name, self._children, "table_")
        exec("self.{} = child".format(child_name))
...
def _idle_table_notify_print(dev_id, pipe_id, direction, parser_id, entry, is_active):
    hit_state = "active" if is_active else "idle"
    print("Device id: {}\n"
          "Pipe id: {}\n"
          "Direction: {}\n"
          "Parser id: {}\n"
          "Hit state: {}\n".format(dev_id, pipe_id, direction, parser_id, hit_state))
    print(entry)
...
class BFLeaf(BFContext):
    ...
    def _field_mask_to_str(self, key_field):
        mask = str("None")
        if (key_field.mask is not None):
            mask = hex(key_field.mask)
        return mask

    @target_check_and_set
    def info(self, pipe=None, return_info=False, print_info=True, from_hw=False):
        res = {
            "table_name": self._name,
            "full_name": self._c_tbl.name,
            "type": self._c_tbl.table_type_map(self._c_tbl.get_type()),
            "usage": self._c_tbl.get_usage(from_hw),
            "capacity": self._c_tbl.get_capacity(from_hw),
            "key_fields": [],
            "data_fields": [],
        }
        if print_info:
            ...
            print("Key Fields:")
        key_rows = []
        headers = ["Name", "Type", "Size", "Required", "Read Only", "Mask"]
        for info in sorted(self._c_tbl.key_fields.values(), key=lambda x: x.id):
            ...
            field += [self._field_mask_to_str(info)]
            key_rows += [field]
        ...
        # we don't want Mask column to be listed in the data fields table
        headers.remove("Mask")
    ...
    def dump(self, table=False, pipe=None, gress_dir=None, prsr_id=None, json=False, from_hw=False, return_ents=False, print_zero=True):
        ...
        if "get_default" in self._c_tbl.supported_commands and (not (return_ents or json)):
            print("Default Entry:")
            try:
                d_ent = self.get_default(print_ent=False, from_hw=from_hw)
        ...
        if not table and not json and not return_ents:
            ...
            # dump() command executes by per pipe in case of asymmetric tables
            # because those tables don't support the pipe_id == PIPE_ALL
            pipe_count = 1
            symm_mode = self._c_tbl.get_symmetric_mode()
            if symm_mode == False:
                pipe_count = self._c_tbl._bfrt_info.num_pipes
            else: # True or -1 in case when entry_scope_attributes isn't supported
                self._cintf._set_pipe(0xffff)
            print("----- Table_Type: {} -----".format(table_type))
            for pipe_iter in range(pipe_count):
                if symm_mode == False:
                    print("----- Pipe_ID: {} -----".format(pipe_iter))
                    self._cintf._set_pipe(pipe_iter)
    ...
    def pre_device_config_set(self, global_rid=None, port_protection=None, fast_failover=None, max_nodes_before_yield_count=None, max_nodes_threshold_node_count=None, max_nodes_threshold_node_port_lag_count=None):
        self._c_tbl.pre_device_config_set(global_rid, port_protection, fast_failover, max_nodes_before_yield_count, max_nodes_threshold_node_count, max_nodes_threshold_node_port_lag_count)

    def pre_device_config_get(self):
        return self._c_tbl.pre_device_config_get()
...
{}
    ...
    def _build_names(self, names, p_names, cur_collide, ridx, names_prefix):
        collision = 0
        replaceables = [('$', ''), (' & ', '_'), ('-', '_'), (':', '_'), ('[', '_'),
                        (']', '_'), ('__', '_')]
        for name in names:
            p_name = name.decode('ascii')
            for x, y in replaceables:
                p_name = p_name.replace(x, y)
            cidx = len(p_name)
            for i in range(ridx):
            ...
            for name in p_names:
                p_names[name] = validate_p4_entity_name(p_names[name], p_names, names_prefix)
        return collision

    def _make_param_names(self, names, collide={}, ridx=1, names_prefix=""):
        ...
        while sts < 0:
            ...
            p_names = {}
            sts = self._build_names(names, p_names, cur_collide, ridx, names_prefix)
            if sts < 0:
                ridx += 1
    ...
    def _make_core_method_strs(self, method_name, key_fields={}, data_fields={}):
        ...
        key_types = []
        key_sizes = []
        key_pnames, collide, k_ridx = self._make_param_names(key_fields.keys(), names_prefix="key_")

        data_idx = []
        data_params = []
        data_types = []
        data_sizes = []
        data_pnames, _, d_ridx = self._make_param_names(data_fields.keys(), collide, k_ridx, names_prefix="data_")
        ...
        tries = 0
        while k_ridx != d_ridx:
            key_pnames, collide, k_ridx = self._make_param_names(key_fields.keys(), ridx=d_ridx, names_prefix="key_")
            data_pnames, _, d_ridx = self._make_param_names(data_fields.keys(), collide, k_ridx, names_prefix="data_")
            tries += 1
            if tries >= 50:
...
def validate_new_entry(new_entry, entry_list):
    result = True
    for item in entry_list:
        #Check if item is P4 table name
        if hasattr(item, '_c_tbl') and (getattr(item, "_name") == new_entry):
            result = False
    return result

def add_prefix_to_entry(entry, prefix, entry_list):
    def print_name_warn_(err_txt_, o_name_, n_name_):
        print("WARNING: The P4 " + prefix + " name '%s' is " %(o_name_) + err_txt_ + ". It is changed to '%s'." %(n_name_), file = sys.stderr)
    #
    p4_entry_name_res_ = prefix + entry
    counter = 1
    # Check if entry already exist in entry_list
    while not validate_new_entry(p4_entry_name_res_, entry_list):
        p4_entry_name_res_ = prefix + str(counter) + entry
        counter+=1
    #
    print_name_warn_("not a valid Python identifier", entry, p4_entry_name_res_)
    return p4_entry_name_res_

"""
P4 entity might be a reserved python keyword. We prefix to the name if that's the
case. In case of more conflicts, prefix_1_ and then prefix_2_ and so on.
"""
def validate_p4_entity_name(entity, entity_list, prefix):
    new_entity = entity
    if keyword.iskeyword(entity):
        new_entity =  add_prefix_to_entry(entity, prefix, entity_list)
    return new_entity
...
def make_deep_tree(p4_name, bf_rt_info, dev_node, cintf):
    ...
    for table_name, tbl_obj in bf_rt_info.tables.items():
        prefs = table_name.split('.')
        # If table should be present in both device and program node add it to
        # program node, but only after all other tables were already added and
        # node tree is complete
        for clone in _bfrt_clone_nodes:
            if clone['tbl_name'] == table_name:
                # Replace first node
                prefs[0] = clone['prefix']
                if p4_name != b"$SHARED":
                    p4_node = update_node_tree(p4_node, prefs, cintf)
                    BFLeaf(name=prefs[-1], c_tbl=tbl_obj, cintf=cintf, parent_node=p4_node)
    for learn_name, lrn_obj in bf_rt_info.learns.items():
        prefs = learn_name.split('.')
        p4_node = update_node_tree(p4_node, prefs, cintf)
        BFLrnLeaf(prefs[-1], lrn_obj, cintf, parent_node=p4_node)
    #
...
def populate_bfrt(dev_id_list):
    ...
    for dev_id in dev_id_list:
        cintf = CIntfBFRT(dev_id, BfRtTable, BfRtInfo, BfRtLearn)
        if cintf == -1:
            return -1
        if single_device:
    ...
    # This variable reload is needed in order to make bfrt warm-init feature
    # working. With out this line, the bfrt_python did not work after it was
    # reloaded by the callback.
...
def _wrap_bfrt_reload_callback(bfrt):
    def callback_wrapper(dev_id, dev_family, dev_profile, dma_info, warm_init_mode):
        bfrt()
        set_bfrt_parent_context()
        sts = bfrt.reload(bfrt.device_list)
        return sts
    return bfrt.bf_drv_register_cb_type(callback_wrapper)
...
def _register_reload_callback(bf_rt_drv_hdl):
    ...
    bfrt.bf_drv_register_cb_type = device_add_fn

    bfrt.bf_drv_client_callbacks = BfDrvClientCallbacks()
    bfrt.bf_drv_client_callbacks.device_add = _wrap_bfrt_reload_callback(bfrt)

    sts = bfrt._cintf.get_driver().bf_drv_client_register_callbacks(bf_rt_drv_hdl, byref(bfrt.bf_drv_client_callbacks), c_int(0)) # 0 - BF_CLIENT_PRIO_0
    if not sts == 0:
        print("bf_drv_client_register_callbacks failed [{}]".format(bfrt._cintf.err_str(sts)))
    return sts
...
def bfrt_input_transform(lines):
    new_lines = []
    for line in lines:
        if line == "?\n" or line == ".\n":
            ...
            new_lines.append("set_bfrt_parent_context()\n")
        elif line == ".?\n":
            if(_bfrt_context['cur_node'] == None):
                new_lines.append(f"?\n")
            else:
                path_to_cur_node = IPython.get_ipython().prompts.in_prompt_tokens()[0][1]
                new_lines.append(f"help({path_to_cur_node})\n")
        elif len(line) >= 3 and line[-2] == "?" and line[-3] != "?":
            # We must replace default handling of "?" with help call in order
            # to force same thread execution, otherwise class info can spawn in
            # separate thread and break usual shell flow of access, especially when
            # bfshell is used.
            new_lines.append("help("+line[0:-2]+")\n")
        elif line[0] == "?" and line[1] != "?":
            # Same thing here, just begining of the line case
            new_lines.append("help("+line[1:-1]+")\n")
        else:
...
def load_ipython_extension(ipython):
    setattr(sys.modules['__main__'], "_bfrt_context", _bfrt_context)
    setattr(sys.modules['__main__'], "set_bfrt_parent_context", set_bfrt_parent_context)
    setattr(sys.modules['__main__'], "bfrt", bfrt)
    if not bfrt_input_transform in ipython.input_transformers_cleanup:
        ipython.input_transformers_cleanup.append(bfrt_input_transform)
...
def unload_ipython_extension(ipython):
    if bfrt_input_transform in ipython.input_transformers_cleanup:
        ipython.input_transformers_cleanup.remove(bfrt_input_transform)
    pass

def bfrt_exit_handler():
    unload_ipython_extension(ipython_app.shell)
    for name in _bfrt_context['cur_context']:
        delattr(sys.modules['__main__'],name)
    delattr(sys.modules['__main__'], "_bfrt_context")
    delattr(sys.modules['__main__'], "set_bfrt_parent_context")
    delattr(sys.modules['__main__'], "bfrt")
    if bfrt._cintf is not None:
...
def start_bfrt(in_fd, out_fd, install_dir, dev_id_list, udf=None, interactive=False):
    # Workaround for ctrl + '\' case (when bfshell is started in a different terminal window)
    # Need to de-register the previous DVM add device callback
    global bf_rt_drv_hdl
    if bf_rt_drv_hdl != None:
        _deregister_reload_callback(bf_rt_drv_hdl)
    ...
    if ipython_app is None:
        ...
        # Need to set prompt after first initialization
        set_prompt()
    else:
    ...
    # Work around for ctrl + '\' case (when bfshell is started in a different terminal window)
    # Error messages observed in this case are as follows:

    # Traceback (most recent call last):
    # Error in sys.excepthook:

    # When you pipe the output of one program into another, the output produced by the writing program
    # gets backed up in a buffer and waits for the reading program to request that data from the buffer.
    # As long as the buffer is nonempty, any attempt to close the writing file object is supposed to fail
    # with an error.
    # The specific code that triggers the error is in the C language implementation of Python,
    # which is why you can't catch it with a try/except block. It runs after the contents of your
    # script has finished processing or interrupted. Basically, while Python is shutting itself down, it
    # attempts to close stdout, but that fails because there is still buffered output waiting to be read.
    # So Python tries to report this error as it would normally, but sys.excepthook has already been
    # removed as part of the finalization procedure, so that fails. Python then tries to print a message
    # to sys.stderr, but that has already been deallocated so again, it fails. The reason you see the
    # messages on the screen is that the Python code does contain a contingency fprintf to write out
    # some output to the file pointer directly, even if Python's output object doesn't exist.
    ...
    # function to sys.excepthook. In this case, we are overriding it to "do nothing" to avoid the messages
    # above as they are misleading to users and are not fatal.
