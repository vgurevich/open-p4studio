...
bf_status_t bf_rt_table_pre_device_config_attributes_allocate(
  ...
  std::unique_ptr<bfrt::BfRtTableAttributes> attr;
  auto status = table->attributeAllocate(
      bfrt::TableAttributesType::PRE_DEVICE_CONFIG, &attr);
  *tbl_attr_hdl_ret =
      reinterpret_cast<bf_rt_table_attributes_hdl *>(attr.release());
  return status;
}
...
bf_status_t bf_rt_table_dyn_key_mask_attributes_allocate(
  ...
  std::unique_ptr<bfrt::BfRtTableAttributes> attr;
  auto status = table->attributeAllocate(
      bfrt::TableAttributesType::DYNAMIC_KEY_MASK, &attr);
  *tbl_attr_hdl_ret =
      reinterpret_cast<bf_rt_table_attributes_hdl *>(attr.release());
  return status;
}
...
bf_status_t bf_rt_key_field_mask_get(const bf_rt_table_hdl *table_hdl,
  ...
  auto sts = table->keyFieldMaskGet(field_id, &m);
  if (sts != BF_SUCCESS) {
    return sts;
  }
  if (msize < m.size()) {
