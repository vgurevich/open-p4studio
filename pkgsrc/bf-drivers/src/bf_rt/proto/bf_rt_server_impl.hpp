...
struct bfRtGrpcServerFailException : public std::exception {
  const char *what() const throw() {
    return "Failed to start bfrt grpc server";
  }
};
...
class ConfigManager {
 public:
  ...
  Status warmInitEnd(const bf_dev_id_t &device_id);

 private:
  std::unique_ptr<DeviceForwardingConfig> device_forwarding_config_{nullptr};
...
class Notifications {
 public:
  Notifications(bool enable_learn,
                ...
                bool enable_entry_active,
                bool enable_selector_update_change)
      : enable_learn_(enable_learn),
        enable_idletimeout_(enable_idletimeout),
        enable_port_status_change_(enable_port_status_change),
        enable_entry_active_(enable_entry_active),
        enable_selector_update_change_(enable_selector_update_change){};
  bool enable_learn_ = false;
  bool enable_idletimeout_ = false;
  bool enable_port_status_change_ = false;
  bool enable_entry_active_ = false;
  bool enable_selector_update_change_ = false;
};
...
class ConnectionData {
 public:
  ...
  ~ConnectionData() {
    ...
    if (session_) session_->sessionCompleteOperations();
  }
  ...
  uint16_t get_ref_count() { return ref_count_; }
...
class BfRtServerErrorReporter {
 public:
  ...
  void push_back(const bfrt_proto::Error &status) {
    if (status.canonical_code()) {
      errors.emplace_back(index, status);
    }
    index++;
  }
