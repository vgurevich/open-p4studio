...
const std::string BINARY_FILE_NAME = "tofino.bin";
  ...
  for (const auto &config : request.config()) {
    ...
    for (const auto &profile : config.profiles()) {
      ...
      std::string binary_path =
          program_path + "/" + profile.profile_name() + "/" + BINARY_FILE_NAME;
      // pipe_scope
...
Status ConfigManager::warmInitEnd(const bf_dev_id_t &device_id) {
  switch (fwd_config_state_) {
    case ForwardingConfigState::INITIAL:
    case ForwardingConfigState::WARM_INIT_STARTED: {
      ...
      auto bf_status = bf_pal_device_warm_init_end(device_id);
...
void ConnectionData::insertTableIdInList(const bf_rt_id_t &table_id) {
  std::unique_lock<std::mutex> lk(operation_mtx);
  operation_cb_sync_list.insert(table_id);
  // Unlock the mutex before signalling the waiting thread
  lk.unlock();
  // Signal the waiting grpc thread
  operation_cv.notify_all();
}

bool ConnectionData::removeIfPresentTableIdFromList(const bf_rt_id_t &table_id,
                                                    long wait_time_sec) {
  // Wait until we hear about the completions of the operations
  std::unique_lock<std::mutex> lk(operation_mtx);
  return operation_cv.wait_for(lk, std::chrono::seconds(wait_time_sec), [&]() {
...
ServerData::ServerData(std::string server_name, std::string address)
  ...
  if (server_ == nullptr) {
    LOG_ERROR("%s:%d  %s Failed to start Server on %s",
              __func__,
              __LINE__,
              server_name_.c_str(),
              address.c_str());

    throw bfRtGrpcServerFailException();
  }
...
BfRtServer &BfRtServer::getInstance(std::unique_ptr<ServerData> server_data) {
  ...
  std::call_once(bfrt_server_once_flag, [&] {
...
Status BfRtServer::addConnection(const uint32_t &client_id,
  ...
  Notifications notifications(
      ...
      subscribe.notifications().enable_entry_active_notifications(),
      subscribe.notifications().enable_selector_update_change_notifications());
  ...
  if (connection_sp_it != client_id_to_connection_data_map.end()) {
    ...
    if (connection_sp->isIndependent()) {
      ...
      // We decrement ref_count if client changes from independent to subscribed
      connection_sp->decrement_ref_count();
      return Status();
    } else {
...
Status BfRtServer::addConnection(const uint32_t &client_id) {
  ...
  if (client_id_to_connection_data_map.find(client_id) !=
    ...
    // Client already exists. Just log increment connection_count and return
    // BF_ALREADY_EXISTS-status
    LOG_DBG("%s:%d Client %d already exists!", __func__, __LINE__, client_id);
  } else {
    // Add the connection to the map
    client_id_to_connection_data_map.emplace(
        client_id,
        std::make_shared<ConnectionData>(client_id,
                                         BfRtSession::sessionCreate()));
    LOG_DBG("%s:%d Client %d Connected!", __func__, __LINE__, client_id);
  }
  client_id_to_connection_data_map.at(client_id).get()->increment_ref_count();
  return Status();
}
...
Status setPortStatusNotify(
    const uint32_t &client_id,
    std::shared_ptr<bfrt::server::ConnectionData> &connection_sp,
    const bfrt_proto::SetForwardingPipelineConfigRequest *request) {
  const BfRtInfo *bfrtinfo;
  std::vector<std::reference_wrapper<const std::string>> p4_names;
  bf_status_t sts = BfRtDevMgr::getInstance().bfRtInfoP4NamesGet(
      request->device_id(), p4_names);
  check_and_return(sts, "Failed to get p4 names");

  // use the first p4_name because all the fixed functions across different
  // programs on same device are the same and use the same backend and state
  sts = BfRtDevMgr::getInstance().bfRtInfoGet(
      request->device_id(), p4_names[0], &bfrtinfo);
  check_and_return(sts, "Failed to get bfrt info for p4 program");

  const BfRtTable *table;
  sts = bfrtinfo->bfrtTableFromNameGet("$PORT", &table);
  check_and_return(sts, "Failed to get bfrt table from p4 program $PORT");

  bf_rt_target_t target;
  target.dev_id = request->device_id();
  target.pipe_id = BF_DEV_PIPE_ALL;

  const auto session = connection_sp->sessionGet();
  auto grpc_status = bfrt::server::setAttributePortStatusChgNotify(
      client_id, *session, target, *table, connection_sp, false);
  grpc_check_and_return(grpc_status,
                        "Failed to set port status notification attribute");
  return grpc_status;
}
  ...
  switch (fwd_action) {
    ...
    case bfrt_proto::
      ...
      if (connection_sp->getNotifications().enable_port_status_change_) {
        // disable port status notification
        grpc_status = setPortStatusNotify(client_id, connection_sp, request);
        grpc_check_and_return(
            grpc_status,
            "Failed to deregister port status change notification");
      }
      // WARM_INIT_BEGIN
    ...
    case bfrt_proto::
      ...
      // WARM_INIT_END
      grpc_status =
          connection_sp->config_manager.warmInitEnd(request->device_id());
...
void BfRtServer::cleanupConnection(const uint32_t &client_id) {
  ...
  if (!connection_data->isIndependent()) {
    ...
    // For independent clients, we only remove it from map if ref_count == 0
    connection_data->decrement_ref_count();
    if (0 == connection_data->get_ref_count()) {
...
bf_status_t bf_rt_grpc_server_run(const char * /*program_name*/,
  ...
  if (local_only) {
    return bf_rt_grpc_server_run_with_addr(
        ("127.0.0.1:" + port_str.str()).c_str());
  } else {
    return bf_rt_grpc_server_run_with_addr(
        ("0.0.0.0:" + port_str.str()).c_str());
  }
}
