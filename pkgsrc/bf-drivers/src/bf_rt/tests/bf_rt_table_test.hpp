...
#ifndef _BF_RT_TABLE_TEST_HPP_
#define _BF_RT_TABLE_TEST_HPP_
...
#include "bf_rt_tm_mgr_intf_mock.hpp"
#include "bf_rt_mc_mgr_intf_mock.hpp"
#include <bf_rt_common/bf_rt_init_impl.cpp>
#include <bf_rt_common/bf_rt_info_impl.hpp>
#include <bf_rt_common/bf_rt_table_impl.hpp>
#include <bf_rt_p4/bf_rt_p4_table_data_impl.hpp>
#include "bf_rt_matchers.hpp"
#include "bf_rt_matchers_factory.hpp"
...
#define ASSERT_SUCCESS(status) \
  ASSERT_THAT(status, BfRtMatchersFactory::makeIsSuccessMatcher())
#define EXPECT_SUCCESS(status) \
  EXPECT_THAT(status, BfRtMatchersFactory::makeIsSuccessMatcher())
...
class MatchSpec {
 public:
  MatchSpec(const GenericKey &random_key) {
    ...
    const auto &table = static_cast<const BfRtTableObj &>(base_table);
    ...
    for (const auto &field_id : random_key.fieldIdListGet()) {
      ...
      const BfRtTableKeyField *table_key_field;
...
class ActionSpec {
 public:
  ActionSpec(const GenericData &single_random_data,
    ...
    const BfRtTable &table = single_random_data.tableGet();
      ...
      for (const auto &field_type : types_vec) {
        switch (field_type) {
          ...
          case (DataFieldType::ACTION_PARAM_OPTIMIZED_OUT): {
            ...
            // context
            // json, bfrt is going to send zeros for this field to pipe mgr.
            // Hence when
...
class BfRtTableTest
 ...
 public:
  ...
  void bfRtDeviceAdd() {
    ...
    std::string bfrt_json_file_name = std::get<0>(t);
    std::string bfrt_json_file_path =
        std::string(INSTALLDIR) +
        (program_name != "switch" ? "/tofinopd" : "") + std::string("/") +
        program_name + std::string("/") + bfrt_json_file_name;
    ...
    // to pass in to the device add call for bfrt. This is guaranteed to only
    ...
    // Add a dummy device (with id 0) to bfrt
    ...
    dev_profile.p4_programs[dev_id].bfrt_json_file =
        const_cast<char *>(bfrt_json_file_path.c_str());
    dev_profile.p4_programs[dev_id].num_p4_pipelines =
        context_json_file_paths.size();
    std::string temp = std::string(BFRT_SHARED_INSTALLDIR) + std::string("/");
    dev_profile.bfrt_non_p4_json_dir_path = const_cast<char *>(temp.c_str());
    ...
    bf_dev_family_t dev_family =
        BF_DEV_FAMILY_TOFINO;  // We really dont care for this parameter
    ASSERT_SUCCESS(bf_rt_device_add(
        dev_id, dev_family, &dev_profile, nullptr, BF_DEV_INIT_COLD));
  }

  void bfRtDeviceRemove() { ASSERT_SUCCESS(bf_rt_device_remove(dev_id)); }
  ...
  virtual void SetUp() {
    ...
    // Before we carry on with the test, add the device to bfrt
    bfRtDeviceAdd();

    // Once the device has been added, get and cache the bfrt info obj
    ASSERT_SUCCESS(
        BfRtDevMgr::getInstance().bfRtInfoGet(dev_id, prog_name, &bfrtInfo));
    ASSERT_NE(bfrtInfo, nullptr) << "Failed to open json files";

    static_cast<const BfRtInfoImpl *>(bfrtInfo)->bfrtInfoGetTables(&b_tables);

    // Setup default session and bfrt_target and the table read flag
    session = BfRtSession::sessionCreate();
    bfrt_tgt.dev_id = dev_id;
    bfrt_tgt.pipe_id = pipe_id;
    flag = BfRtTable::BfRtTableGetFlag::GET_FROM_SW;

    // Set the seed for the random generator
    auto seed = time(NULL);
    std::cout << "Seed used is : " << static_cast<uint64_t>(seed) << "\n";
    BfRtEntryGen::setRandomSeed(seed);
  }
  virtual void TearDown() {
    // Remove the device from bfrt
    bfRtDeviceRemove();
  ...
  virtual void initialTestSetup(const BfRtTable &table,
  ...
  BfRtSession &getDefaultSession() { return *session; }

  bf_rt_target_t &getDefaultBfRtTarget() { return bfrt_tgt; }

  void formTableKey_from_idx(const BfRtTable &table,
                             const int &index,
                             BfRtTableKey *table_key);

  void formTableData_from_idx(const BfRtTable &table,
                              const int &index,
                              BfRtTableData *table_data);

 protected:
  BfRtTable::BfRtTableGetFlag &getDefaultTableReadFlag() { return flag; }

  void getTablesOfAType(const BfRtTable::TableType &table_type_arg,
                        std::vector<BfRtTable *> *table_vec) {
    for (auto *table : b_tables) {
      BfRtTable::TableType t_type;
      static_cast<const BfRtTableObj *>(table)->tableTypeGet(&t_type);
  ...
  // object
  void tableDataSetHelper(const BfRtTable &table,
  ...
  // key object
  void tableKeySetHelper(const GenericKey &random_key, BfRtTableKey &table_key);
  // This will return a vector of random data objects by quering them from
  // the generator
  void tableRandomDataGet(const BfRtTable &table,
                          const BfRtEntryGen &entry_generator,
                          const uint32_t &num_entries,
                          std::vector<GenericData> *random_data);
  // This will return a vector of random key objects by quering them from
  // the generator
  void tableRandomKeyGet(const BfRtTable &table,
                         const BfRtEntryGen &entry_generator,
                         const uint32_t &num_entries,
                         std::vector<GenericKey> *random_keys);

  // This compares 2 key objects
  void compareKeyObjects(const BfRtTableKey &obj_1,
                         const BfRtTableKey &obj_2) const;
  // This compares 2 data objects
  void compareDataObjects(const BfRtTableData &obj_1,
                          const BfRtTableData &obj_2) const;

  const BfRtInfo *bfrtInfo{nullptr};
  std::vector<BfRtTable *> b_tables;
  MockIPipeMgrIntf *pipe_mgr_obj{nullptr};
  bf_dev_id_t dev_id{0};
  bf_dev_pipe_t pipe_id{0xffff};
  std::string prog_name;
  std::shared_ptr<BfRtSession> session;
  bf_rt_target_t bfrt_tgt;
  BfRtTable::BfRtTableGetFlag flag{BfRtTable::BfRtTableGetFlag::GET_FROM_SW};
  BfRtTable::TableType table_type{BfRtTable::TableType::INVALID};

  std::unordered_map<const BfRtTable *, std::unique_ptr<BfRtEntryGen>>
      entry_generator_map;
  std::vector<GenericKey> random_keys;
  std::vector<GenericData> random_data;
...
class BfRtMatchActionTableTest : public BfRtTableTest {
 protected:
  BfRtMatchActionTableTest() {}
  void SetUp() override {
    // Call SetUp of the parent
    BfRtTableTest::SetUp();
    // Get all the match action direct tables
    getTablesOfAType(BfRtTable::TableType::MATCH_DIRECT, &d_tables);
    // Set the table type
    table_type = BfRtTable::TableType::MATCH_DIRECT;
    // Setup the Generator objects for all the tables
    for (const auto *table : d_tables) {
      entry_generator_map.insert(
          std::make_pair(table, BfRtEntryGen::makeGenerator(table)));
    }
  }

  void initialTestSetup(const BfRtTable &table,
                        const BfRtEntryGen &entry_generator,
                        const uint32_t &num_entries) override;

  void getTTL(const BfRtTable &table,
              GenericData &single_random_data,
              uint64_t *ttl_val);
  bf_status_t tableEntryAdd(const uint32_t &num_entries,
                            const BfRtTable &table,
                            std::map<pipe_ent_hdl_t, int> *entry_hdls);
  bf_status_t tableEntryModSwapActionData(
      const uint32_t &num_entries,
      const BfRtTable &table,
      const std::map<pipe_ent_hdl_t, int> &entry_hdls);
  bf_status_t tableEntryModIterateFields(
  ...
  void tableEntryModSetExpForField(const BfRtTable &table,
                                   const pipe_ent_hdl_t &ent_hdl,
                                   const int &index,
  ...
  void tableEntryGetSetExpForEntry(const BfRtTable &table,
                                   const GenericData &single_random_data,
                                   const int &times);

  std::vector<BfRtTable *> d_tables;
};  // BfRtMatchActionTableTest
...
class BfRtActionTableTest : public BfRtTableTest {
 protected:
  BfRtActionTableTest() {}
  void SetUp() override {
    // Call SetUp of the parent
    ...
    getTablesOfAType(BfRtTable::TableType::ACTION_PROFILE, &d_tables);
    // Set the table type
    table_type = BfRtTable::TableType::ACTION_PROFILE;
    // Setup the Generator objects for all the tables
    for (const auto *table : d_tables) {
...
class BfRtSelectorTableTest : public BfRtTableTest {
 protected:
  BfRtSelectorTableTest() {}
  void SetUp() override {
    // Call SetUp of the parent
    BfRtTableTest::SetUp();
    // Get all the selector tables
    getTablesOfAType(BfRtTable::TableType::SELECTOR, &d_tables);
    // Set the table type
    table_type = BfRtTable::TableType::SELECTOR;
    // FIXME Fgure out this mess of const BfRtTable * vs. non const BfRtTable *
    for (const auto *sel_tbl : d_tables) {
      auto act_prof_tbl_id =
          static_cast<const BfRtTableObj *>(sel_tbl)->getActProfId();
      const BfRtTable *act_tbl = nullptr;
      // Get the action profile table linked with this selector table
      EXPECT_SUCCESS(bfrtInfo->bfrtTableFromIdGet(act_prof_tbl_id, &act_tbl));
    ...
    for (const auto *table : d_tables) {
      ...
      entry_generator_map.insert(
          std::make_pair(sel_to_act_tbl[table],
                         BfRtEntryGen::makeGenerator(sel_to_act_tbl[table])));
    }
  }
  ...
  void addEntriesInActProfTable(const BfRtTable &act_tbl,
  ...
  std::vector<BfRtTable *> d_tables;
  std::unordered_map<const BfRtTable * /*Selector table*/,
                     const BfRtTable * /*Action profile table*/>
      sel_to_act_tbl;
  std::unordered_map<bf_rt_id_t, pipe_adt_ent_hdl_t> mbr_id_to_adt_ent_hdl_map;

 protected:
  std::vector<pipe_adt_ent_hdl_t> getPipeAdtEntHdlsFromActMbrIds(
      const std::vector<bf_rt_id_t> &mbr_ids) const {
...
class BfRtTMQueueTableTest : public BfRtTableTest {
 public:
  BfRtTMQueueTableTest(){};
  ~BfRtTMQueueTableTest(){};
  void SetUp() override {
    // Call SetUp of the parent
    BfRtTableTest::SetUp();
    ...
    getTablesOfAType(BfRtTable::TableType::TM_QUEUE_CFG, &d_tables);
    // Set the table type
    table_type = BfRtTable::TableType::TM_QUEUE_CFG;
    // Setup the Generator objects for all the tables
    for (const auto *table : d_tables) {
      entry_generator_map.insert(
          std::make_pair(table, BfRtEntryGen::makeGenerator(table)));
  ...
  bf_status_t tableTest(const uint32_t &num_entries, const BfRtTable &table);
...
class BfRtPREMulticastNodeTableTest : public BfRtTableTest {
 public:
  BfRtPREMulticastNodeTableTest(){};
  ~BfRtPREMulticastNodeTableTest(){};
  void SetUp() override {
    // Call SetUp of the parent
    BfRtTableTest::SetUp();
    this->mc_mgr_obj = MockIMcMgrIntf::getInstance();
    getTablesOfAType(BfRtTable::TableType::PRE_NODE, &this->d_tables);
    // Set the table type
    this->table_type = BfRtTable::TableType::PRE_NODE;
    // Setup the Generator objects for all the tables
...
class BfRtTableScopeGuard {
 public:
  BfRtTableScopeGuard(BfRtTableTest &test_table,
                      const BfRtTable &table,
                      BfRtEntryGen &entry_gen,
                      const uint32_t &num_entries,
    ...
    for (auto ts : type_set) {
      switch (ts) {
        case TableAttributesType::IDLE_TABLE_RUNTIME: {
          std::unique_ptr<BfRtTableAttributes> tbl_attr;
          ...
          EXPECT_SUCCESS(
              table.tableAttributesSet(test_table.getDefaultSession(),
                                       test_table.getDefaultBfRtTarget(),
                                       *tbl_attr));
          break;
        }
        default:
 ...
 private:
  BfRtTableTest &test_table_;
  const BfRtTable &table_;
  BfRtEntryGen &entry_gen_;
