    ...
    with open(filename, "rb") as csv_file:
        ...
        for row in csv_reader:
            ...
            if row["Type"] in ["addressmap", "userdefined addressmap"] :
                ...
                if open_arg == 1:
                    print "};"
                    print ("cmd_arg_t " + open_arg_name + 
                           "_mem = { " + str(list_elts) + ", " + 
                           open_arg_name + "_mem_list };\n" )
                open_arg = 1
                open_arg_name = row["Identifier"]
                open_addr_map = row["Identifier"]
                print "cmd_arg_item_t " + open_arg_name + "_mem_list[] = {"
                list_elts = 0
            elif row["Type"] in csv_register_types:
                reg_width = int(row["Register Size"].replace(" bits",""),0)
                active_addr_map.objs.append(reg(row["Identifier"], array_size, int(row["Offset"],0), reg_width, active_addr_map))
                ...
                if array_size == (1,):
                    print ( "{ \"" + identifier + "\"" + 
                            ", NULL, " + hex(offset) + ", &" + open_arg_name + "__" + identifier + ", " +
                            hex(reg_width/8) + " }," )
                    list_elts = list_elts + 1
                elif len(array_size) == 1:
                    print ( "{ \"" + identifier + "\"" + 
                            ", NULL, " + hex(offset) + ", &" + open_arg_name + "__" + identifier + ", " +
                            hex((reg_width/8)*array_size[0]) + " }," )
                    list_elts = list_elts + 1
                elif len(array_size) == 2:
                    print ( "{ \"" + identifier + "\"" + 
                            ", NULL, " + hex(offset) + ", &" + open_arg_name + "__" + identifier + ", " +
                            hex((reg_width/8)*array_size[0]*array_size[1]) + " }," )
                    list_elts = list_elts + 1
                elif len(array_size) == 3:
                    print ( "{ \"" + identifier + "\"" + 
                            ", NULL, " + hex(offset) + ", &" + open_arg_name + "__" + identifier + ", " +
                            hex((reg_width/8)*array_size[0]*array_size[1]*array_size[2]) + " }," )
                    list_elts = list_elts + 1
        ...
        if open_arg == 1:
            print "};"
            print "cmd_arg_t " + open_arg_name + "_mem = { " + str(list_elts) + ", " + open_arg_name + "_mem_list };\n"
...
def build_reg_info(dir):
    full_csr_file = os.path.join(dir,"pipe_top_level.csv")
