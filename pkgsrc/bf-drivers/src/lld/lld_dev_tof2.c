...
static int read_clock_file(const char *fname, uint32_t *val) {
  FILE *pFile;
  pFile = fopen(fname, "r");
  if (pFile == NULL) return 0;
  char b[64] = {0};
  int n = fscanf(pFile, "%63s", b);
  fclose(pFile);
  if (n > 0) {
    /* If the number starts with "0x" treat is as if someone specified a PLL
     * value (with or without the bypass bit set). */
    if (!strncmp(b, "0x", 2) || !strncmp(b, "0X", 2)) {
      *val = strtoull(b, NULL, 16);
    }
    /* If the number contains a "." assume someone specified a frequence in
     * GHz. */
    else if (strchr(b, '.')) {
      double f = 0.0;
      sscanf(b, "%lf", &f);
      int x = (int)(f * 1000000000);
      *val = freq_to_pll_val(x);
    }
    /* Otherwise assume a frequence in Hz. */
    else {
      int x = strtoul(b, NULL, 0);
      *val = freq_to_pll_val(x);
    }
    printf("\nRead clock value of %d from %s\n", pll_val_to_freq(*val), fname);
    lld_log("Read clock value of %d from %s", pll_val_to_freq(*val), fname);
    return 1;
  }
  return 0;
}
static void read_core_clk_from_file(uint32_t *bps_val, uint32_t *pps_val) {
  uint32_t bps = 0, pps = 0;
  if (read_clock_file("/core_clk", &bps)) {
    *bps_val = bps;
  }
  if (read_clock_file("/pps_clk", &pps)) {
    *pps_val = pps;
  }
}
#endif /* __KERNEL__ */
 ...
 * freq_to_pll_val
 *
 * Helper function to program pps_pll and core_pll.
...
#ifndef __KERNEL__
  uint32_t bps = 0, pps = 0;
  read_core_clk_from_file(&bps, &pps);
...
/************************************************************
 * lld_dev_tof2_ts_inc_value_set
 *
 ************************************************************/
static void lld_dev_tof2_ts_inc_value_set(bf_dev_id_t dev_id,
                                          int core_freq,
                                          int pps_freq) {
#ifndef __KERNEL__
  ...
  setp_tof2_mbus_ts_4ns_inc_value_count(&ctrl_val, x);
  addr = offsetof(tof2_reg, device_select.mbc.mbc_mbus.ts_4ns_inc_value);
  lld_write_register(dev_id, addr, ctrl_val);
  lld_log("Set 4ns pulse config to 0x%x", x);

  /* Program the same thing for the global_ts_inc_value. */
  addr = offsetof(tof2_reg, device_select.mbc.mbc_mbus.global_ts_inc_value);
  lld_write_register(dev_id, addr, x);
  ...
  addr = offsetof(tof2_reg, device_select.tm_top.tm_psc_top.psc_common.scratch);
  ...
  lld_write_register(dev_id, addr, x);
  addr = offsetof(tof2_reg, device_select.tm_top.tm_psc_top.psc_common.scratch);
  lld_write_register(dev_id, addr, y);
#endif
}

/*************************************************
 * lld_dev_tof2_un_reset
 *
...
bf_status_t lld_dev_tof2_un_reset(bf_dev_id_t dev_id) {
  ...
  uint32_t soft_rst = tof2_reg_device_select_misc_regs_soft_reset_address;
  uint32_t soft_rst_val = 0;
  uint32_t dbg_rst = tof2_reg_device_select_misc_regs_dbg_rst_address;
  uint32_t dbg_rst_val = 0;
  uint32_t pps_pll = tof2_reg_device_select_misc_regs_pps_pll_ctrl0_address;
  uint32_t bps_pll = tof2_reg_device_select_misc_regs_core_pll_ctrl0_address;
  uint32_t mac0_pll = tof2_reg_device_select_misc_regs_mac0_pll_ctrl0_address;
  uint32_t mac1_pll = tof2_reg_device_select_misc_regs_mac1_pll_ctrl0_address;
  uint32_t pps_pll_val = 0x88ff1800;
  uint32_t bps_pll_val = 0x88d71800;
  uint32_t mac0_pll_val = 0x891f1801;
  uint32_t mac1_pll_val = 0x891f1801;
  uint32_t dummy;
  uint32_t pvt = tof2_reg_device_select_misc_regs_pvt_ctrl_address;
  ...
  /* Put PPS, BPS, MAC0, and MAC1 PLLs in bypass.  Put the PPS PLL in bypass
   * first and wait a bit.  If the chip was running at capacity this will
   * reduce the load before we assert the resets below. */
  lld_write_register(dev_id, pps_pll, pps_pll_val);
  lld_read_register(dev_id, pps_pll, &dummy);
  bf_sys_usleep(10);
  lld_write_register(dev_id, bps_pll, bps_pll_val);
  lld_write_register(dev_id, mac0_pll, mac0_pll_val);
  lld_write_register(dev_id, mac1_pll, mac1_pll_val);

  /* Dummy read to ensure posted writes to PLLs complete. */
  lld_read_register(dev_id, mac1_pll, &dummy);

  /* Wait for PLLs to be updated. */
  bf_sys_usleep(5);

  /* Put core, MAC, MBus, TV80, tiles, and PLLs are in soft reset. */
  lld_read_register(dev_id, soft_rst, &soft_rst_val);
  soft_rst_val |= 0x0F3FC000;
  lld_write_register(dev_id, soft_rst, soft_rst_val);

  /* Program PPS and BPS PLL values based on desired clock frequency. */
  rc = bps_pps_ctrl0_val(dev_id, &bps_pll_val, &pps_pll_val);
  if (rc != BF_SUCCESS) return rc;
  lld_write_register(dev_id, pps_pll, pps_pll_val);
  lld_write_register(dev_id, bps_pll, bps_pll_val);
  ...
  for (i = 0; i < max_polls; ++i) {
    lld_read_register(dev_id, dbg_rst, &dbg_rst_val);
  ...
  lld_dev_tof2_ts_inc_value_set(dev_id, core_clk_freq, pps_clk_freq);

  /* Set PVT into tempature monitoring mode. */
  lld_write_register(dev_id, pvt, 0x008101E3);

  return BF_SUCCESS;
}

/********************************************************************
 * lld_dev_tof2_set_pps_clock
 *
...
bf_status_t lld_dev_tof2_set_pps_clock(bf_dev_id_t dev_id, int freq) {
  bf_status_t rc;
  int i, bps_sku_freq, pps_sku_freq, max_polls = 1000;
  uint32_t pps_pll = tof2_reg_device_select_misc_regs_pps_pll_ctrl0_address;
  uint32_t bps_pll = tof2_reg_device_select_misc_regs_core_pll_ctrl0_address;
  ...
  lld_read_register(dev_id, pps_pll, &pps_pll_val);
  pps_pll_val |= 0x80000000;
  lld_write_register(dev_id, pps_pll, pps_pll_val);

  /* Read to ensure posted writes to PLLs complete. */
  lld_read_register(dev_id, bps_pll, &bps_pll_val);
  bf_sys_usleep(5);
...
bf_status_t lld_dev_tof2_get_pps_clock(bf_dev_id_t dev_id,
  ...
  lld_read_register(dev_id, pps_pll, pll_val);
  *freq = pll_val_to_freq(*pll_val);
  return BF_SUCCESS;
}

/********************************************************************
 * lld_dev_tof2_get_core_clock
 *
...
bf_status_t lld_dev_tof2_get_core_clock(bf_dev_id_t dev_id,
  ...
  lld_read_register(dev_id, bps_pll, pll_val);
  *freq = pll_val_to_freq(*pll_val);
  return BF_SUCCESS;
}

/********************************************************************
 * lld_dev_tof2_reset_core
 *
...
bf_status_t lld_dev_tof2_reset_core(bf_dev_id_t dev_id) {
  ...
  lld_read_register(dev_id, pps_pll, &pps_pll_val);
  lld_read_register(dev_id, bps_pll, &bps_pll_val);
  lld_write_register(dev_id, pps_pll, pps_pll_val | 0x80000000);
  lld_write_register(dev_id, bps_pll, bps_pll_val | 0x80000000);
  /* Dummy read to ensure posted writes to PLLs complete. */
  lld_read_register(dev_id, bps_pll, &dummy);
  bf_sys_usleep(5);

  /* Read register first to preserve other bits. */
  lld_read_register(
      dev_id, tof2_reg_device_select_misc_regs_soft_reset_address, &val);
  val |= core_reset;
  ...
  while (!errors_expected && !reset_done) {
    lld_read_register(
        dev_id, tof2_reg_device_select_misc_regs_dbg_rst_address, &val);
    reset_done = !(val & core_reset_status);
    polls++;
    if (!reset_done) {
      if (polls >= max_polls) {
        lld_log("Dev %d [Core Unreset] Failed (note: will fail on arch model)",
                dev_id);
        return BF_INVALID_ARG;
      }
      lld_log("Dev %d [Core Unreset] In progress: %08x", dev_id, val);
    }
  }
  lld_log("Dev %d [Core Unreset] Done after %d polls", dev_id, polls);
...
bf_status_t lld_dev_tof2_tlp_poison_set(bf_dev_id_t dev_id, bool en) {
  ...
  offset = offsetof(tof2_reg, device_select.pcie_bar01_regs.cpu_glb_ctrl);
  lld_subdev_read_register(dev_id, 0, offset, &cpu_glb_ctrl);
  setp_tof2_cpu_glb_ctrl_ena_pois(&cpu_glb_ctrl, (en ? 1 : 0));
  setp_tof2_cpu_glb_ctrl_data_pois(&cpu_glb_ctrl, 1);
