...
lld_err_t lld_ind_write_tof(bf_dev_id_t dev_id,
  ...
  uint32_t addr_reg_lo =
      tofino_device_select_pcie_bar01_regs_pcie_regs_cpu_ind_addr_low_address;
  uint32_t addr_reg_hi =
      tofino_device_select_pcie_bar01_regs_pcie_regs_cpu_ind_addr_high_address;
  uint32_t data_reg_lo =
      tofino_device_select_pcie_bar01_regs_pcie_regs_cpu_ind_data00_address;
  uint32_t data_reg_hi =
      tofino_device_select_pcie_bar01_regs_pcie_regs_cpu_ind_data11_address;

  data = (uint32_t)(ind_addr >> 32ull);
  lld_write_register(dev_id, addr_reg_hi, data);
  data = (uint32_t)(ind_addr & 0xffffffffull);
  lld_write_register(dev_id, addr_reg_lo, data);

  data = (uint32_t)(data_hi >> 32ull);
  lld_write_register(dev_id, data_reg_hi, data);
  data = (uint32_t)(data_hi & 0xffffffffull);
  lld_write_register(dev_id, data_reg_hi - 4, data);

  data = (uint32_t)(data_lo >> 32ull);
  lld_write_register(dev_id, data_reg_hi - 8, data);
  data = (uint32_t)(data_lo & 0xffffffffull);

  lld_write_register(dev_id, data_reg_lo, data);  // note: triggers write

  return LLD_OK;
}

/** \brief lld_ind_read_tof
...
lld_err_t lld_ind_read_tof(bf_dev_id_t dev_id,
  ...
  lld_read_register(dev_id, data_reg_lo + 0, &data00);  // note: triggers read
  lld_read_register(dev_id, data_reg_lo + 4, &data01);
  lld_read_register(dev_id, data_reg_lo + 8, &data10);
  lld_read_register(dev_id, data_reg_lo + 12, &data11);
