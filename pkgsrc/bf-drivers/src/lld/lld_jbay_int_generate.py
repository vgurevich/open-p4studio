...
"""
  JBay_Interrupt_List_*.csv file is generated by HW group.
  * shows the date/version.
  ...
  The (current) structure output follows a typedef defined in lld/lld_tof2_interrupt.c
  ...
  To run:
    python ./lld_tof2_int_generate.py 
"""
...
def add_entry_list1(row, f_list1):
    ...
    if label_handling[0] == 0:
        #add a new struct
        if name_pipe == "":
            f_list1.write("lld_tof2_blk_lvl_int_list_t %s_%s_sh_ints[] = {\n" % (name_top, name_station))
        else:
            if re.match('^MAU', name_station):
                f_list1.write("lld_tof2_blk_lvl_int_list_t %s_%s##pp##_%s##mm##_sh_ints[] = {\\\n" % (name_top, name_pipe, name_station))
            else:#PRSR DPRSR
                f_list1.write("lld_tof2_blk_lvl_int_list_t %s_%s##pp##_%s_sh_ints[] = {\\\n" % (name_top, name_pipe, name_station))
        label_handling[0] = 1
    #add an entry
    if name_pipe == "":
        if row["Tree1"] != "":
            tree1_tmp = str(row["Tree1"])
        f_list1.write("  {offsetof(tof2_reg, %s),\n" % (tree1_tmp))
...
def add_entry_list2(row, f_list2):
    ...
    if label_handling[1] == 0:
        #add a new struct
        if name_pipe == "":
            f_list2.write("lld_tof2_blk_lvl_int_list_t %s_%s_%d_blk_lvl_list[] = {\n" % (name_top, name_station, no_list2))
        else:
            if re.match('^MAU', name_station):
                f_list2.write("lld_tof2_blk_lvl_int_list_t %s_%s##pp##_%s##mm##_%d_blk_lvl_list[] = {\\\n" % (name_top, name_pipe, name_station, no_list2))
            else:#PRSR DPRSR
                f_list2.write("lld_tof2_blk_lvl_int_list_t %s_%s##pp##_%s_%d_blk_lvl_list[] = {\\\n" % (name_top, name_pipe, name_station, no_list2))
        label_handling[1] = 1
    #add an entry
    if name_pipe == "":
        if row["Tree2"] != "":
            tree2_tmp = str(row["Tree2"])
        f_list2.write("  {offsetof(tof2_reg, %s),\n" % (tree2_tmp))
...
def add_entry_node(row, f_node):
    ...
    if label_handling[2] == 0:
        #add a new struct
        if name_pipe == "":
            f_node.write("lld_tof2_blk_lvl_int_t %s_%s_%d_blk_lvl_int[] = {\n" % (name_top, name_station, no_node))
        else:
            if re.match('^MAU', name_station):
                f_node.write("lld_tof2_blk_lvl_int_t %s_%s##pp##_%s##mm##_%d_blk_lvl_int[] = {\\\n" % (name_top, name_pipe, name_station, no_node))
            else:
                f_node.write("lld_tof2_blk_lvl_int_t %s_%s##pp##_%s_%d_blk_lvl_int[] = {\\\n" % (name_top, name_pipe, name_station, no_node))
        label_handling[2] = 1
    #add an entry
    if name_pipe == "":
        f_node.write("  {offsetof(tof2_reg, %s),\n" % str(row["Status"]))
        if row["Enable0"] != "":
            f_node.write("   offsetof(tof2_reg, %s),\n" % str(row["Enable0"]))
        else:
            f_node.write("   -1,\n")
...
def csv_parse_int_list(f_list1, f_list2, f_node):
    ...
    tree1_tmp = ""
    tree2_tmp = ""
    with open("JBay_Interrupt_List.csv", 'r') as csv_file:
...
def add_top_list(row, numb, f_top, f_top_rep):
    ...
    if numb == 8:
        ...
        f_top.write("lld_tof2_blk_lvl_int_list_t Tof2_int_top[16][20] = {\n")
        f_top.write("  {\n")
    is_null = 1
        ...
        for row in csv_reader:
            ...
            if re.match('^[0-9]', row["Top"]):
                ...
                if re.match('pipe0', pipe_name):
                    f_top_rep.write("    {offsetof(tof2_reg, device_select.pcie_bar01_regs.shadow_int[%d]),\n" % top_array_no)
                    f_top_rep.write("     %s,\n" % hex(top_array_mask))
                ...
                if row["Tree1"] != "":
                    #list1
                    f_top.write("    {offsetof(tof2_reg, device_select.pcie_bar01_regs.shadow_int[%d]),\n" % top_array_no)
                    f_top.write("     %s,\n" % hex(top_array_mask))
                    f_top.write("     false,\n")
...
if __name__ == "__main__":
    with open("Tof2_Interrupt_Struct_list1.c", "w") as f_list1:
        with open("Tof2_Interrupt_Struct_list2.c", "w") as f_list2:
            with open("Tof2_Interrupt_Struct_node.c", "w") as f_node:
                csv_parse_int_list(f_list1, f_list2, f_node)
            f_node.close()
        f_list2.close()
    f_list1.close()
    with open("Tof2_Interrupt_Struct_top.c", "w") as f_top:
        with open("Tof2_Interrupt_Struct_top_rep.c", "w") as f_top_rep:
            csv_parse_int_top_list(f_top, f_top_rep)
        f_top_rep.close()
    f_top.close()
    #merge files into one file
    with open ("lld_tof2_interrupt_struct.h", 'w') as f:
        f.write("""
/* clang-format off */
/*******************************************************************************
 *
...
""")
        ...
        with open("Tof2_Interrupt_Struct_node.c") as f_node:
            for line in f_node:
                f.write(line)
        f_node.close()
        f.write("};\n")
        with open("Tof2_Interrupt_Struct_list2.c") as f_list2:
            for line in f_list2:
                f.write(line)
        f_list2.close()
        with open("Tof2_Interrupt_Struct_list1.c") as f_list1:
            for line in f_list1:
                f.write(line)
        f_list1.close()
        ...
        os.remove("Tof2_Interrupt_Struct_node.c")
        os.remove("Tof2_Interrupt_Struct_list2.c")
        os.remove("Tof2_Interrupt_Struct_list1.c")
        with open("Tof2_Interrupt_Struct_top.c") as f_top:
            for line in f_top:
                f.write(line)
        f_top.close()
        #f.write("  },\n  {\n")
        with open("Tof2_Interrupt_Struct_top_rep.c") as f_top:
            for line in f_top:
                f.write(line)
        f_top.close()
        f.write("  },\n\n")
        ...
        #special case, PBC
        for i in range(0, 4):
            f.write("    {offsetof(tof2_reg, device_select.pcie_bar01_regs.shadow_int[%d]),\n" % (15))
            hard_code = (504 + 2*i)
        ...
        f.write("  },\n};\n") #end of top list
        f.write("#endif // LLD_TOF2_INTERRUPT_STRUCT_INCLUDED")
        f.write("/* clang-format on */\n")
    f.close()
    os.remove("Tof2_Interrupt_Struct_top.c")
    os.remove("Tof2_Interrupt_Struct_top_rep.c")
