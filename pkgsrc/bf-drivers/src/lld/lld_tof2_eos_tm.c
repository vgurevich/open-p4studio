...
uint32_t m_pipe_mask = 0xF;
#define NumPipe 4
uint32_t psc_blk_dis[0x4] = {0};
uint32_t caa_blk_vld[0xC0] = {0};

extern void ucli_log(char *fmt, ...);
extern char *get_full_reg_path_name(bf_dev_id_t dev_id, uint32_t offset);

void tm_ig_int_chk(bf_dev_id_t dev_id) {
  uint32_t addr, data;
  // uint8_t clear = 0;
  uint32_t intr_stat;
  uint32_t intr_mask;
  ...
  addr = offsetof(tof2_reg, device_select.tm_top.tm_caa_top.intr.stat);
  lld_read_register(dev_id, addr, &data);
  intr_stat = data;
...
void tm_eg_int_chk(bf_dev_id_t dev_id, uint8_t clear) {
  uint32_t addr, data;
  uint32_t intr_stat;
  ...
  for (int pipe_num = 0; pipe_num < NumPipe; pipe_num++) {
    ...
    switch (pipe_num) {
      case 0:
        addr = offsetof(tof2_reg,
                        device_select.tm_top.tm_scha_top.sch[0].intr.stat);
        break;
      case 1:
        addr = offsetof(tof2_reg,
                        device_select.tm_top.tm_scha_top.sch[1].intr.stat);
        break;
      case 2:
        addr = offsetof(tof2_reg,
                        device_select.tm_top.tm_schb_top.sch[0].intr.stat);
        break;
      case 3:
        addr = offsetof(tof2_reg,
                        device_select.tm_top.tm_schb_top.sch[1].intr.stat);
        break;
      default:
        bf_sys_assert(0);
    }
...
void tm_wac_common_eos_check(bf_dev_id_t dev_id) {
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_wac_top_wac_common_wac_common_wac_ap_cnt_cell_array_count;  // TM_REGS->get_tm()->tm_wac_top_->wac_common_->wac_common_->wac_ap_cnt_cell_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    addr = offsetof(tof2_reg,
                    device_select.tm_top.tm_wac_top.wac_common.wac_common
                        .wac_ap_cnt_cell[i]);
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_wac_ap_cnt_cell_cnt(&data);
    // if
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_wac_hdr_cnt_cell_cnt(&data);
  // if
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_wac_dod_cnt_cell_cnt(&data);
  // if
  ...
  if (!ignore_ap_drop_st) {
    cnt_size =
        tof2_reg_device_select_tm_top_tm_wac_top_wac_common_wac_common_block_drop_st_drop_state_cell_array_count;  // TM_REGS->get_tm()->tm_wac_top_->wac_common_->wac_common_block_drop_st_->drop_state_cell_.size();
    for (int i = 0; i < (int)cnt_size; i++) {
      // TM_REGS->get_tm()->tm_wac_top_->wac_common_->wac_common_block_drop_st_->drop_state_cell_[i]->Read();
      addr = offsetof(tof2_reg,
                      device_select.tm_top.tm_wac_top.wac_common
                          .wac_common_block_drop_st.drop_state_cell[i]);
      lld_read_register(dev_id, addr, &data);
      fld = getp_tof2_wac_ap_drop_state_cell_drop_state(&data);
      // if
      // (TM_REGS->get_tm()->tm_wac_top_->wac_common_->wac_common_block_drop_st_->drop_state_cell_[i]->drop_state_
      // != 0) {
      if (fld != 0) {
        ucli_log(
            "[ERROR]::wac_common_block drop_state non-zero:%0d for idx:%0d\n",
            fld,
            i);
      }
    }
  ...
  addr = offsetof(tof2_reg,
                  device_select.tm_top.tm_wac_top.wac_common.wac_common
                      .wac_wm_hdr_cnt_cell);
  lld_read_register(dev_id, addr, &data);
  uint32_t hdr_cnt_cell = getp_tof2_wac_wm_hdr_cnt_cell_cnt(&data);
  ...
  lld_read_register(dev_id, addr, &data);
  uint32_t hdr_limit_cell = getp_tof2_wac_hdr_limit_cell_limit(&data);
...
bool wac_glb_hdr_drop_cnt_chk;
  ...
  for (int pipe=0; pipe < NumPipe; pipe++) {
    ...
    for(int i=0; i<cnt_size; i++) {
      m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe)+"_ppg"+std::to_string(i)+"_drop_cnt";
  ...
  if(wac_port_drop_cnt_chk) {
    for (int pipe=0; pipe < NumPipe; pipe++) {
      ...
      for(int i=0; i<cnt_size; i++) {
        m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe)+"_port"+std::to_string(i)+"_drop_cnt";
  ...
  for (int pipe=0; pipe < NumPipe; pipe++) {
    ...
    if(wac_clr_drop_cnt_chk == 1) {
      ...
      for(int i=0; i<cnt_size; i++) {
        m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe)+"_ap"+std::to_string(i)+"_drop_cnt";
        m_sknobs_value = sknobs_get_value((char*)m_sknobs_name.c_str(), 0);
      ...
      for(int i=0; i<cnt_size; i++) {
        m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe)+"_yel"+std::to_string(i)+"_drop_cnt";
        m_sknobs_value = sknobs_get_value((char*)m_sknobs_name.c_str(), 0);
      ...
      for(int i=0; i<cnt_size; i++) {
        m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe)+"_red"+std::to_string(i)+"_drop_cnt";
        m_sknobs_value = sknobs_get_value((char*)m_sknobs_name.c_str(), 0);
  ...
  if(wac_glb_hdr_drop_cnt_chk) {
    for (int pipe=0; pipe < NumPipe; pipe++) {
      if(!((m_pipe_mask >> pipe) & 0x1)) { continue; }
      m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe)+"_hdr_drop_cnt";
      m_sknobs_value = sknobs_get_value((char*)m_sknobs_name.c_str(), 0);
...
uint32_t cnt_size;
  ...
  if (fuse_pipe_dis != 0 && (fuse_pipe_dis & (1 << pipe_num)) == 0) {
     m_sknobs_name = "tof2.tm.wac"+std::to_string(pipe_num)+"_ctr_drop_fuse_pipe";
     m_sknobs_value = sknobs_get_value((char*)m_sknobs_name.c_str(), 0);
...
void tm_wac_buf_full_eos_check(bf_dev_id_t dev_id) {
  uint32_t addr, data;
  ...
  for (int pipe_num = 0; pipe_num < 4; pipe_num++) {
    ...
    lld_read_register(dev_id, addr, &data);
    uint32_t c48_lo =
        getp_tof2_cnt48_inc_wac_drop_buf_full_0_2_ctr48_31_0(&data);
    lld_read_register(dev_id, addr + 4, &data);
    uint32_t c48_hi =
        getp_tof2_cnt48_inc_wac_drop_buf_full_1_2_ctr48_47_32(&data);
...
void tm_caa_eos_check(bf_dev_id_t dev_id) {
  uint32_t addr, data, fld;
  uint32_t cnt_size;
  uint32_t timeout_en = 0;
  uint32_t exp_blk_usect = 4;  // default if timeout_en=0 (also default)

  // blk_usecnt
  cnt_size =
      tof2_reg_device_select_tm_top_tm_caa_top_epipe_array_count;  // TM_REGS->get_tm()->tm_caa_top_->epipe_.size();
  addr = offsetof(tof2_reg, device_select.tm_top.tm_caa_top.ctrl);
  lld_read_register(dev_id, addr, &data);
  timeout_en = getp_tof2_caa_ctrl_r_timeout_en(&data);
  if (timeout_en) {
    exp_blk_usect = 0;
  }
  ...
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_caa_epipe_blocks_usecnt_r_value(&data);
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_caa_top_block_array_count;  // TM_REGS->get_tm()->tm_caa_top_->block_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    // TM_REGS->get_tm()->tm_caa_top_->block_[i]->addr_usecnt_->Read();
    addr = offsetof(tof2_reg,
                    device_select.tm_top.tm_caa_top.block[i].addr_usecnt);
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_caa_block_addr_usecnt_r_value(&data);
    if (i < 191) {
    ...
    } else if ((caa_blk_vld[i / 32] >> (i % 32)) & 1) {
      // if(TM_REGS->get_tm()->tm_caa_top_->block_[i]->addr_usecnt_->value_ !=
      // 2) {
      if (fld != 2) {
...
// CLC Common
void tm_clc_common_eos_check(bf_dev_id_t dev_id) {
  uint32_t addr, data;
  uint32_t cnt_size;
  ...
  // qlc_pt_ct_cnt pipe0
  cnt_size =
      tof2_reg_device_select_tm_top_tm_clc_top_clc_common_qclc_pt_ct_cnt_pipe0_array_count;  // TM_REGS->get_tm()->tm_clc_top_->clc_common_->qclc_pt_ct_cnt_pipe0_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    // TM_REGS->get_tm()->tm_clc_top_->clc_common_->qclc_pt_ct_cnt_pipe0_[i]->Read();
    addr = offsetof(
        tof2_reg,
        device_select.tm_top.tm_clc_top.clc_common.qclc_pt_ct_cnt_pipe0[i]);
    lld_read_register(dev_id, addr, &data);
    uint32_t cnt0 = getp_tof2_qclc_pt_ct_cnt_cnt0(&data);
    uint32_t cnt1 = getp_tof2_qclc_pt_ct_cnt_cnt1(&data);
    uint32_t cnt2 = getp_tof2_qclc_pt_ct_cnt_cnt2(&data);
    uint32_t cnt3 = getp_tof2_qclc_pt_ct_cnt_cnt3(&data);
    ...
    if (cnt0 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe0[%0d] non-zero cnt0_: %0d\n", i, cnt0);
    }
    // if
    ...
    if (cnt1 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe0[%0d] non-zero cnt1_: %0d\n", i, cnt1);
    }
    // if
    ...
    if (cnt2 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe0[%0d] non-zero cnt2_: %0d\n", i, cnt2);
    }
    // if
    ...
    if (cnt3 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe0[%0d] non-zero cnt3_: %0d\n", i, cnt3);
    }
  }
  // qlc_pt_ct_cnt pipe1
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    addr = offsetof(
        tof2_reg,
        device_select.tm_top.tm_clc_top.clc_common.qclc_pt_ct_cnt_pipe1[i]);
    lld_read_register(dev_id, addr, &data);
    uint32_t cnt0 = getp_tof2_qclc_pt_ct_cnt_cnt0(&data);
    uint32_t cnt1 = getp_tof2_qclc_pt_ct_cnt_cnt1(&data);
    uint32_t cnt2 = getp_tof2_qclc_pt_ct_cnt_cnt2(&data);
    ...
    if (cnt0 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe1[%0d] non-zero cnt0_: %0d\n", i, cnt0);
    }
    // if
    ...
    if (cnt1 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe1[%0d] non-zero cnt1_: %0d\n", i, cnt1);
    }
    // if
    ...
    if (cnt2 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe1[%0d] non-zero cnt2_: %0d\n", i, cnt2);
    }
    // if
    ...
    if (cnt3 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe1[%0d] non-zero cnt3_: %0d\n", i, cnt3);
    }
  }
  // qlc_pt_ct_cnt pipe2
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    addr = offsetof(
        tof2_reg,
        device_select.tm_top.tm_clc_top.clc_common.qclc_pt_ct_cnt_pipe2[i]);
    lld_read_register(0, addr, &data);
    uint32_t cnt0 = getp_tof2_qclc_pt_ct_cnt_cnt0(&data);
    uint32_t cnt1 = getp_tof2_qclc_pt_ct_cnt_cnt1(&data);
    uint32_t cnt2 = getp_tof2_qclc_pt_ct_cnt_cnt2(&data);
    uint32_t cnt3 = getp_tof2_qclc_pt_ct_cnt_cnt3(&data);
    ...
    if (cnt0 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe2[%0d] non-zero cnt0_: %0d\n", i, cnt0);
    }
    // if
    ...
    if (cnt1 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe2[%0d] non-zero cnt1_: %0d\n", i, cnt1);
    }
    // if
    ...
    if (cnt2 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe2[%0d] non-zero cnt2_: %0d\n", i, cnt2);
    }
    // if
    ...
    if (cnt3 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe2[%0d] non-zero cnt3_: %0d\n", i, cnt3);
    }
  }
  // qlc_pt_ct_cnt pipe3
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    addr = offsetof(
        tof2_reg,
        device_select.tm_top.tm_clc_top.clc_common.qclc_pt_ct_cnt_pipe3[i]);
    lld_read_register(0, addr, &data);
    uint32_t cnt0 = getp_tof2_qclc_pt_ct_cnt_cnt0(&data);
    uint32_t cnt1 = getp_tof2_qclc_pt_ct_cnt_cnt1(&data);
    uint32_t cnt2 = getp_tof2_qclc_pt_ct_cnt_cnt2(&data);
    ...
    if (cnt0 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe3[%0d] non-zero cnt0_: %0d\n", i, cnt0);
    }
    // if
    ...
    if (cnt1 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe3[%0d] non-zero cnt1_: %0d\n", i, cnt1);
    }
    // if
    ...
    if (cnt2 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe3[%0d] non-zero cnt2_: %0d\n", i, cnt2);
    }
    // if
    ...
    if (cnt3 != 0) {
      ucli_log(
          "[ERROR]::qclc_pt_ct_cnt_pipe3[%0d] non-zero cnt3_: %0d\n", i, cnt3);
    }
  }
  ...
  addr =
      offsetof(tof2_reg, device_select.tm_top.tm_clc_top.clc_common.ct_tot_cnt);
  lld_read_register(0, addr, &data);
  uint32_t uc_tot_cnt = getp_tof2_qclc_pt_tot_cnt_uc_tot_cnt(&data);
  uint32_t mc_tot_cnt = getp_tof2_qclc_pt_tot_cnt_mc_tot_cnt(&data);

  // if (TM_REGS->get_tm()->tm_clc_top_->clc_common_->ct_tot_cnt_->mc_tot_cnt_!=
  // 0) {
  if (mc_tot_cnt != 0) {
    ucli_log("[ERROR]::mc_ct_cnt non-zero cnt: %0d\n", mc_tot_cnt);
  }
  // Check uc_tot_cnt
  // if (TM_REGS->get_tm()->tm_clc_top_->clc_common_->ct_tot_cnt_->uc_tot_cnt_!=
  // 0) {
  if (uc_tot_cnt != 0) {
...
void tm_clc_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  uint32_t cnt_size;
  ucli_log("[TM_EOS]:: CLC Pipe%0d Checks Starting...\n", pipe_num);
  // egress_port_ct_state
  cnt_size =
      tof2_reg_device_select_tm_top_tm_clc_top_clc_egress_port_ct_state_array_count;  // TM_REGS->get_clc_pipe_rspec(pipe_num)->egress_port_ct_state_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    // TM_REGS->get_clc_pipe_rspec(pipe_num)->egress_port_ct_state_[i]->Read();
    addr = offsetof(
        tof2_reg,
        device_select.tm_top.tm_clc_top.clc[pipe_num].egress_port_ct_state[i]);
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_port_ct_state_ct_state(&data);
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_clc_top_clc_ingress_port_ct_state_array_count;  // TM_REGS->get_clc_pipe_rspec(pipe_num)->ingress_port_ct_state_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    // TM_REGS->get_clc_pipe_rspec(pipe_num)->ingress_port_ct_state_[i]->Read();
    addr = offsetof(
        tof2_reg,
        device_select.tm_top.tm_clc_top.clc[pipe_num].ingress_port_ct_state[i]);
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_port_ct_state_ct_state(&data);
...
// PEX Pipe
void tm_pex_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  uint32_t cnt_size;
  // uint32_t lim;
  // uint32_t wm_comp;
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_pex_top_pex_pt_state_array_count;  // TM_REGS->get_pex_pipe_rspec(pipe_num)->pt_state_.size();
  ...
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    fld = getp_tof2_qpex_pt_state_state0(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state1(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state2(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state3(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state4(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state5(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state6(&data);
    ...
    fld = getp_tof2_qpex_pt_state_state7(&data);
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_pex_top_pex_q_empty_array_count;  // tm_regmodel_db::get_register_model()->get_pex_pipe_rspec(pipe_num)->q_empty_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    fld = getp_tof2_qpex_q_empty_empty(&data);
    if (i % 3 == 2) {
      // if (TM_REGS->get_pex_pipe_rspec(pipe_num)->q_empty_[i]->empty_ !=
      // 0x800000ff) {
      if (fld != 0x800000ff) {
        ucli_log(
            "[ERROR]::pex q_empty not 0x800000ff for pipe:%0d, i:%0d, "
            "empty_state: 0x%0x\n",
            pipe_num,
            i,
            fld);
  ...
  if (1) {  // cfg_manager_db::get_cfg_manager()->get_backdoor() ==
    ...
    fld = getp_tof2_pex_debug_register_debug_sts(&data);
    // if(TM_REGS->get_pex_pipe_rspec(pipe_num)->pex_debug_register_->debug_sts_
    // != 0) {
    // Note: bit 23 is a "fifo empty" bit which gets set at end of run when
    //       there is no traffic
    fld &= 0x7fffff;
    if (fld != 0) {
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_pex_top_pex_pt_epb_cred_array_count;  // tm_regmodel_db::get_register_model()->get_tm()->tm_pex_top_->pex_[pipe_num]->pt_epb_cred_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    // TM_REGS->get_tm()->tm_pex_top_->pex_[pipe_num]->pt_epb_cred_[i]->Read();
    addr = offsetof(
        tof2_reg, device_select.tm_top.tm_pex_top.pex[pipe_num].pt_epb_cred[i]);
    lld_read_register(0, addr, &data);
    uint32_t data2, cred0, cred1, cred2, cred3, cred4, cred5, cred6, cred7;
    lld_read_register(0, addr + 4, &data2);
    cred0 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_0_2_cred0(&data);
    cred1 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_0_2_cred1(&data);
    cred2 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_0_2_cred2(&data);
    cred3 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_0_2_cred3(&data);
    cred4 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_1_2_cred4(&data);
    cred5 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_1_2_cred5(&data);
    cred6 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_1_2_cred6(&data);
    cred7 = getp_tof2_qpex_epb_cred_cnt_pt_epb_cred_1_2_cred7(&data);
    ...
    ucli_log(
        ...
        cred3);
    ucli_log(
        "[TM_EOS]:: PEX Pipe%0d pt_epb_credit MAC:%0d, cred4: %0d, cred5:%0d, "
        "cred6:%0d, cred7:%0d\n",
        pipe_num,
        i,
        cred4,
...
void tm_qac_common_eos_check(bf_dev_id_t dev_id) {
  uint32_t addr, data, fld;
  ...
  for (int fifo = 0; fifo < 4; fifo++) {
    ...
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_qac_pre_fifo_cnt_pkt_cnt_pkt(&data);
    ...
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_qac_pre_fifo_cnt_cell_cnt_pkt(&data);
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_qac_dod_cnt_cell_cnt(&data);
  ...
  addr = offsetof(
      ...
      device_select.tm_top.tm_qac_top.qac_common.qac_common.qac_dod_cnt_cell);
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_qac_mcct_cnt_pkt_cnt_pkt(&data);
...
void tm_qac_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_qac_top_qac_pipe_qac_reg_queue_drop_state_array_count;  // TM_REGS->get_qac_pipe_rspec(pipe_num)->qac_reg_->queue_drop_state_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_queue_drop_state_st(&data);
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_qac_top_qac_pipe_qac_reg_queue_drop_yel_state_array_count;  // TM_REGS->get_qac_pipe_rspec(pipe_num)->qac_reg_->queue_drop_yel_state_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_qac_top_qac_pipe_qac_reg_queue_drop_red_state_array_count;  // TM_REGS->get_qac_pipe_rspec(pipe_num)->qac_reg_->queue_drop_red_state_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_qac_top_qac_pipe_qac_reg_port_drop_state_array_count;  // TM_REGS->get_qac_pipe_rspec(pipe_num)->qac_reg_->port_drop_state_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_port_drop_state_st(&data);
...
void tm_psc_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_psc_ph_used_ph_used(&data);
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_psc_epipe_enable_r_value(&data);
  ...
  if (fld == 1) {
    ...
    lld_write_register(dev_id, addr, 0);
  }
}

void tm_psc_common_eos_check(bf_dev_id_t dev_id) {
  uint32_t addr, data, fld;
  ...
  lld_read_register(dev_id, addr, &bv[0]);
  lld_read_register(dev_id, addr + 4, &bv[1]);
  lld_read_register(dev_id, addr + 8, &bv[2]);
  lld_read_register(dev_id, addr + 12, &bv[3]);
  ...
  for (int i = 0; i < (int)cnt_size; i++) {
    ...
    addr =
        offsetof(tof2_reg, device_select.tm_top.tm_psc_top.psc_common.block[i]);
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_psc_block_state_r_state(&data);
    ...
    owner = getp_tof2_psc_block_state_r_owner(&data);
    fld = getp_tof2_psc_block_state_r_free(&data);
    ...
    blk_vld_exp = !((psc_blk_dis[i / 32] >> (i % 32)) & 1);
    ...
    usecnt = getp_tof2_psc_block_addr_usecnt_r_value(&data);
    if (usecnt != 0) {
  ...
  for (int pipe_num = 0; pipe_num < NumPipe; pipe_num++) {
    ...
    if (fld == 1) {
      ...
      if ((psc_blk_used[pipe_num] != 0) & (psc_blk_used[pipe_num] != 2)) {
        ucli_log("[ERROR]::PSC Block%0d is holding %0d blocks (!=2)!\n",
                 pipe_num,
...
void tm_qlc_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  uint32_t cnt_size;
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_qlc_dis_cred_qlc_dis_cred(&data);

  uint32_t m_sknobs_value = 32;
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_qlc_top_qlc_dis_qlen_array_count;  // TM_REGS->get_qlc_pipe_rspec(pipe_num)->dis_qlen_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    // TM_REGS->get_qlc_pipe_rspec(pipe_num)->dis_qlen_[i]->Read();
    addr = offsetof(tof2_reg,
                    device_select.tm_top.tm_qlc_top.qlc[pipe_num].dis_qlen[i]);
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_qlc_dis_qlen_cnt(&data);
...
void tm_sch_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  uint32_t cnt_size;
  // Check SCH Count
  ...
  // port pex credit
  cnt_size =
      tof2_reg_device_select_tm_top_tm_schb_top_sch_port_pex_status_mem_array_count;  // TM_REGS->get_sch_pipe_rspec(pipe_num)->port_pex_status_mem_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    uint32_t sch;
    // TM_REGS->get_sch_pipe_rspec(pipe_num)->port_pex_status_mem_[i]->Read();
    if (pipe_num < 2) {
      sch = pipe_num;  // 0 or 1
      addr = offsetof(
          tof2_reg,
          device_select.tm_top.tm_scha_top.sch[sch].port_pex_status_mem[i]);
    } else {
      sch = (pipe_num - 2);  // 0 or 1
      addr = offsetof(
          tof2_reg,
          device_select.tm_top.tm_schb_top.sch[sch].port_pex_status_mem[i]);
    }
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_sch_port_pex_status_r_pri_curr_credits(&data);
  ...
  // mac pex credit
  cnt_size =
      tof2_reg_device_select_tm_top_tm_scha_top_sch_mac_pex_status_mem_array_count;  // TM_REGS->get_sch_pipe_rspec(pipe_num)->mac_pex_status_mem_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
    uint32_t sch;
    ...
    if (pipe_num < 2) {
      ...
      addr = offsetof(
          tof2_reg,
          device_select.tm_top.tm_scha_top.sch[sch].mac_pex_status_mem[i]);
    } else {
      sch = (pipe_num - 2);  // 0 or 1
      addr = offsetof(
          tof2_reg,
          device_select.tm_top.tm_schb_top.sch[sch].mac_pex_status_mem[i]);
    }
    lld_read_register(dev_id, addr, &data);
    fld = getp_tof2_sch_mac_pex_status_r_pri_curr_credits(&data);
...
void tm_pre_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  uint32_t addr, data, fld;
  uint32_t cnt_size;
  // Check PRE Count
  ...
  cnt_size =
      tof2_reg_device_select_tm_top_tm_pre_top_pre_table_ph_count_array_count;  // TM_REGS->get_pre_pipe_rspec(pipe_num)->table_ph_count_.size();
  for (int i = 0; i < (int)cnt_size; i++) {
  ...
  lld_read_register(dev_id, addr, &data);
  fld = getp_tof2_pre_fifo_credit_log_l0_fifo_credit(&data);
  ...
  fld = getp_tof2_pre_fifo_credit_log_l1_fifo_credit(&data);
  ...
  fld = getp_tof2_pre_fifo_credit_log_l1l2_fifo_credit(&data);
  ...
  fld = getp_tof2_pre_fifo_credit_log_l2_fifo_credit(&data);
  ...
  fld = getp_tof2_pre_fifo_credit_log_l2_sel_fifo_credit(&data);
  ...
  fld = getp_tof2_pre_fifo_credit_log_l2_port_fifo_credit(&data);
  // if
...
void tm_prc_pipe_eos_check(uint8_t pipe_num__) {
  uint32_t cnt_size;
...
void tm_ig_pipe_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  //   tm_wac_pipe_eos_check(pipe_num); // mem
  tm_clc_pipe_eos_check(dev_id, pipe_num);
  tm_pex_pipe_eos_check(dev_id, pipe_num);
}

// IG EOS
void tm_ig_eos_check_all(bf_dev_id_t dev_id, uint32_t phys_pipe_msk) {
  tm_wac_common_eos_check(dev_id);
  tm_caa_eos_check(dev_id);
  tm_clc_common_eos_check(dev_id);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    tm_ig_pipe_eos_check(dev_id, i);
  }
  tm_wac_buf_full_eos_check(dev_id);
  tm_ig_int_chk(dev_id);
  ucli_log("[TM_EOS]:: IG Check Done!\n");
}

// EOS Check for Egress Modules
void tm_eg_eos_check(bf_dev_id_t dev_id, uint8_t pipe_num) {
  tm_qac_pipe_eos_check(dev_id, pipe_num);
  tm_psc_pipe_eos_check(dev_id, pipe_num);
  tm_qlc_pipe_eos_check(dev_id, pipe_num);
  tm_pre_pipe_eos_check(dev_id, pipe_num);
  tm_prc_pipe_eos_check(pipe_num);
  // FIX-ME, SCH PORT-PEX Status Mem
  // needs to move to Indirect
  tm_sch_pipe_eos_check(dev_id, pipe_num);
}

// EG EOS
void tm_eg_eos_check_all(bf_dev_id_t dev_id, uint32_t phys_pipe_msk) {
  tm_qac_common_eos_check(dev_id);
  tm_psc_common_eos_check(dev_id);
  for (int i = 0; i < 4; i++) {
    if (!((phys_pipe_msk >> i) & 0x1)) {
      continue;
    }
    tm_eg_eos_check(dev_id, i);
  }
  tm_eg_int_chk(dev_id, 1);
  ucli_log("[TM_EOS]:: EG Check Done!\n");
}

/********************************************************************
 * lld_tof2_eos_tm_audit
 *
...
void lld_tof2_eos_tm_audit(bf_dev_id_t dev_id, uint32_t phys_pipe_msk) {
  ...
  tm_ig_eos_check_all(dev_id, phys_pipe_msk);
  tm_eg_eos_check_all(dev_id, phys_pipe_msk);
  tm_wac_common_eos_check(dev_id);
