...
grpc::Status to_grpc_status(const DeviceMgr::Status &from) {
  ...
  bf_sys_assert(conversion_status.ok());
  return to;
}
...
using StreamChannelReaderWriter =
    grpc::ServerReaderWriter<p4v1::StreamMessageResponse,
                             p4v1::StreamMessageRequest>;
...
class Connection {
 ...
 private:
  Connection(ConnectionId::Id connection_id,
...
class DeviceState {
 public:
  ...
  explicit DeviceState(DeviceMgr::device_id_t device_id_)
  ...
  Status add_connection(Connection *connection) {
    ...
    if (!p.second) {
      return Status(StatusCode::INVALID_ARGUMENT, "Election id already exists");
    }
  ...
  Status update_connection(Connection *connection,
    ...
    bf_sys_assert(connection_it != connections.end());
  ...
  void process_stream_message_request(
    ...
    bf_sys_assert(request.update_case() !=
                  p4v1::StreamMessageRequest::kArbitration);
    std::lock_guard<std::mutex> lock(m);
...
class Devices {
 ...
 private:
  ...
  mutable std::mutex m{};
  std::unordered_map<DeviceMgr::device_id_t, std::unique_ptr<DeviceState> >
      device_map{};
};
...
class P4RuntimeServiceImpl : public p4v1::P4Runtime::Service {
 private:
  Status Write(ServerContext * /*context*/,
    ...
    // TODO: if there are no connections, we accept all Write requests
  ...
  Status SetForwardingPipelineConfig(
    ...
    auto device = Devices::get(request->device_id());
    // TODO: if there are no connections, we accept all requests with
    ...
    auto status =
        device_mgr->pipeline_config_set(request->action(), request->config());
    device_mgr->stream_message_response_register_cb(stream_message_response_cb,
                                                    NULL);
    return to_grpc_status(status);
  }
  ...
  Status GetForwardingPipelineConfig(
    ...
    auto device_mgr = Devices::get(request->device_id())->get_p4_mgr();
    if (device_mgr == nullptr) return no_pipeline_config_status();
    auto status = device_mgr->pipeline_config_get(request->response_type(),
                                                  rep->mutable_config());
    return to_grpc_status(status);
  }
  ...
  Status StreamChannel(ServerContext *context,
    ...
    struct ConnectionStatus {
      explicit ConnectionStatus(ServerContext *context_) : context(context_) {}
      ~ConnectionStatus() {
    ...
    while (stream->Read(&request)) {
      switch (request.update_case()) {
        case p4v1::StreamMessageRequest::kArbitration: {
          ...
          // TODO: a lot of existing code will break if 0 is not
          // valid anymore
          // if (election_id == 0) {
          //   return Status(StatusCode::INVALID_ARGUMENT,
          ...
          if (connection == nullptr) {
            connection_status.connection =
                Connection::make(election_id, stream, context);
  ...
  class P4RuntimeLogger : public LogWriterIface {
    void write(Severity severity, const char *msg) override {
      auto severity_map = [&severity]() {
        switch (severity) {
          // bf_sys' DEBUG level corresponds to PI's TRACE level
          case Severity::TRACE:
            return BF_LOG_DBG;
          case Severity::DEBUG:
            return BF_LOG_INFO;
      ...
      bf_sys_log_and_trace(BF_MOD_P4RT, severity_map(), "%s", msg);
    }
  };
...
void BfP4RtServerRunAddr(const char *server_address) {
  DeviceMgr::init(256 /* num devices, not used anymore */);

#ifdef PI_ENABLED
  // When the p4lang DeviceMgr implementation is used, we want to get some
  // useful log messages in the bf_drivers logs, which we can do by registering
  // our own logger implementation.
  register_pi_logger();
#endif
  ...
  builder.AddListeningPort(server_data->server_address,
...
void BfP4RtServerRun() { BfP4RtServerRunAddr("0.0.0.0:50051"); }

int BfP4RtServerGetPort() { return server_data->server_port; }
...
void BfP4RtServerWait() { server_data->server->Wait(); }

void BfP4RtServerShutdown() { server_data->server->Shutdown(); }
...
void BfP4RtServerCleanup() { delete server_data; }
}
