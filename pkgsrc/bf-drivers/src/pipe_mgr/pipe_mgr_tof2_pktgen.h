...
#ifndef _PIPE_MGR_TOF2_PKTGEN_H
#define _PIPE_MGR_TOF2_PKTGEN_H

#define PIPE_MGR_TOF2_PKTGEN_APP_CNT 16
#define PIPE_MGR_TOF2_PKTGEN_PKT_LEN_MIN 69

/* Tofino2 */
struct pipe_mgr_tof2_pg_app_ctx {
  uint32_t app_ctrl;
...
struct pipe_mgr_tof2_pg_app_cfg {
  /* Indexed by logical pipe. */
  /* Active config. */
  struct pipe_mgr_tof2_pg_app_ctx **a;
  /* Backup config (in case of txn abort). */
  struct pipe_mgr_tof2_pg_app_ctx **b;
  bool b_valid;
};

struct pipe_mgr_tof2_pfc_ctx {
  uint32_t pfc_hdr[4];
  uint32_t pfc_timer;
  uint32_t pfc_max_pkt_size;
};

typedef struct pipe_mgr_tof2_pg_dev_ctx {
  /* eth_cpu port ctrl */
  uint32_t port_ctrl;
  ...
  /* pfc */
  struct pipe_mgr_tof2_pfc_ctx *pfc;
  ...
  struct pipe_mgr_tof2_pg_app_cfg app;
  struct pkt_buffer_shadow_t *pkt_buffer_shadow;
} pipe_mgr_tof2_pg_dev_ctx;
...
bf_status_t pipe_mgr_tof2_pkt_buffer_shadow_mem_get(bf_dev_target_t dev_tgt,
                                                    ...
                                                    uint8_t *buf,
                                                    uint32_t size);
bf_status_t pipe_mgr_tof2_pkt_buffer_write_from_shadow(bf_session_hdl_t shdl,
                                                       bf_dev_target_t dev_tgt);
