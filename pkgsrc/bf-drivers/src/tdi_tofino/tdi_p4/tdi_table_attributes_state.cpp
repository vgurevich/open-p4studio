...
#include "tdi_table_attributes_state.hpp"
...
// match_spec is now "owned" by the tdiIdleTmoExpiryTaskExecute.
// The bloew function is responsible to free the spec
void tdiIdleTmoExpiryTaskExecute(
    bf_dev_id_t dev_id,
    pipe_mat_ent_hdl_t entry_hdl,
    pipe_tbl_match_spec_t *pipe_match_spec,
    std::shared_ptr<StateTableAttributesAging> attributes_aging_sp) {
  ...
  auto max_data_bytes = mat_context_info->maxDataSzGet();
  ...
  // construct a TableKey object from the pipe_match_spec
  std::unique_ptr<tdi::TableKey> newKey;
  auto status = table->keyAllocate(&newKey);
  if (status != TDI_SUCCESS) {
    LOG_ERROR("%s:%d Failed to allocate key", __func__, __LINE__);
    return;
  }
  // dynamic downcast to MatchActionKey
  auto matchKey = dynamic_cast<MatchActionKey *>(newKey.get());
  if (matchKey == nullptr) {
    LOG_ERROR(
  ...
  // entry as such
  tdi::tna::tofino::Target dev_tgt(
      dev_id, TNA_DEV_PIPE_ALL, TNA_DIRECTION_INGRESS, PIPE_MGR_PVS_PARSER_ALL);
  dev_target_t pipe_dev_tgt;
  dev_tgt.getTargetVals(&pipe_dev_tgt, nullptr, nullptr);
  ...
  if (entry_scope == TDI_TOFINO_ATTRIBUTES_ENTRY_SCOPE_ALL_PIPELINES) {
    dev_tgt.setValue(static_cast<tdi_target_e>(TDI_TNA_TARGET_PIPE_ID),
                     TNA_DEV_PIPE_ALL);
  } else {
    dev_tgt.setValue(static_cast<tdi_target_e>(TDI_TNA_TARGET_PIPE_ID),
                     pipe_mgr->pipeGetHdlPipe(entry_hdl));
  }
...
void tdiIdleTmoExpiryInternalCb(bf_dev_id_t dev_id,
  ...
  auto attributes_aging_state =
      static_cast<StateTableAttributesAging *>(cookie);
  ...
  auto attributes_aging_sp = std::make_shared<StateTableAttributesAging>(
      enabled, callback_cpp, callback_c, table, cookie_actual);

  // Retrieve the thread pool for this table.
  TdiThreadPool *idle_tmo_thread_pool = nullptr;
  switch (table_type) {
    case TDI_TOFINO_TABLE_TYPE_MATCH_DIRECT:
      idle_tmo_thread_pool = (static_cast<const MatchActionDirect *>(table))
                                 ->idletimeCbTdiThreadPoolGet();
      break;
    case TDI_TOFINO_TABLE_TYPE_MATCH_INDIRECT:
    case TDI_TOFINO_TABLE_TYPE_MATCH_INDIRECT_SELECTOR:
      idle_tmo_thread_pool = (static_cast<const MatchActionIndirect *>(table))
                                 ->idletimeCbTdiThreadPoolGet();
      break;
    default:
      LOG_ERROR(
  ...
  idle_tmo_thread_pool->submitTask(tdiIdleTmoExpiryTaskExecute,
                                   dev_id,
                                   entry_hdl,
...
pipe_status_t selUpdatePipeMgrInternalCb(pipe_sess_hdl_t sess_hdl,
                                         ...
                                         bool is_add) {
  auto cb_state = static_cast<StateSelUpdateCb *>(cookie);
  ...
  if (sess_hdl != session_obj->handleGet(static_cast<tdi_mgr_type_e>(
  ...
  tdi::tna::tofino::Target tdi_tgt(dev_tgt.device_id,
                                   dev_tgt.dev_pipe_id,
                                   TNA_DIRECTION_INGRESS,
                                   PIPE_MGR_PVS_PARSER_ALL);

  // From sel_grp_hdl get sel grp id
  auto sel_table = static_cast<const Selector *>(table);
  auto bf_status = sel_table->getGrpIdFromHndl(
      *session_obj, tdi_tgt, sel_grp_hdl, &sel_grp_id);
  if (bf_status != TDI_SUCCESS) {
    LOG_ERROR("%s:%d %s Group Id not found for group hndl %d",
              __func__,
              __LINE__,
              sel_table->tableInfoGet()->nameGet().c_str(),
              sel_grp_hdl);
  ...
  bf_status = sel_table->getActMbrIdFromHndl(
      *session_obj, tdi_tgt, adt_ent_hdl, &act_mbr_id);
  ...
  if (c_callback_fn) {
    c_callback_fn(reinterpret_cast<const tdi_session_hdl *>(session_obj.get()),
                  reinterpret_cast<const tdi_target_hdl *>(&tdi_tgt),
                  original_cookie,
                  sel_grp_id,
                  act_mbr_id,
                  logical_entry_index,
...
void StateTableAttributesAging::stateTableAttributesAgingSet(
    bool enabled,
  ...
  table_ = const_cast<tdi::Table *>(table);
  cookie_ = cookie;
}

void StateTableAttributesAging::stateTableAttributesAgingReset() {
  enabled_ = false;
...
std::tuple<bool,
           ...
           const tdi::Table *,
           void *>
StateTableAttributesAging::stateTableAttributesAgingGet() {
  return std::make_tuple(enabled_, callback_cpp_, callback_c_, table_, cookie_);
}

void StateSelUpdateCb::reset() {
  enable_ = false;
  sel_table_ = nullptr;
  cookie_ = nullptr;
  return;
