...
int tdiPortStatusChgInternalCb(bf_dev_id_t dev_id,
  ...
  if (matchKey == nullptr) {
    LOG_ERROR("%s:%d The key allocated wasn't portKey", __func__, __LINE__);
    return -1;
  }
  ...
  if (callback_c != nullptr) {
    // NOTE:: PortCfgTableKey objects are not required
    // to free at clientside c_callback.As the Internal cb
