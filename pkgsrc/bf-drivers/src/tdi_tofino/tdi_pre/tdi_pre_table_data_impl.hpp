...
#ifndef _TDI_PRE_TABLE_DATA_IMPL_HPP
#define _TDI_PRE_TABLE_DATA_IMPL_HPP
...
class TdiPREMGIDTableData : public TdiTableDataObj {
 public:
  TdiPREMGIDTableData(const TdiTableObj *tbl_obj) : TdiTableDataObj(tbl_obj){};

  ~TdiPREMGIDTableData() = default;

  tdi_status_t setValue(const tdi_id_t &field_id,
                        const std::vector<tdi_id_t> &arr) override final;
  ...
  tdi_status_t getValue(const tdi_id_t &field_id,
                        std::vector<tdi_id_t> *arr) const override final;

  tdi_status_t reset() override final;

  void setMulticastNodeIds(const std::vector<tdi_id_t> &id) {
  ...
  void setMulticastNodeL1XIds(const std::vector<tdi_id_t> &id) {
    node_l1_xids_ = id;
  }

  void setMulticastECMPIds(const std::vector<tdi_id_t> &id) {
  ...
  void setMulticastECMPL1XIds(const std::vector<tdi_id_t> &id) {
    ecmp_l1_xids_ = id;
  }

  const std::vector<tdi_id_t> &getMulticastNodeIds() const {
  ...
  const std::vector<tdi_id_t> &getMulticastNodeL1XIds() const {
    return node_l1_xids_;
  }

  const std::vector<tdi_id_t> &getMulticastECMPIds() const {
  ...
  const std::vector<tdi_id_t> &getMulticastECMPL1XIds() const {
 ...
 private:
  ...
  std::vector<tdi_id_t> multicast_node_ids_;
  std::vector<bool> node_l1_xids_valid_;
  std::vector<tdi_id_t> node_l1_xids_;
  std::vector<tdi_id_t> multicast_ecmp_ids_;
  std::vector<bool> ecmp_l1_xids_valid_;
  std::vector<tdi_id_t> ecmp_l1_xids_;
};

class TdiPREMulticastNodeTableData : public TdiTableDataObj {
 public:
  TdiPREMulticastNodeTableData(const TdiTableObj *tbl_obj)
      : TdiTableDataObj(tbl_obj){};

  ~TdiPREMulticastNodeTableData() = default;
  ...
  void setPortBitmap(const uint8_t *bitmap) {
    ...
    for (int i = 0; i < TDI_PORT_COUNT; i++) {
      bf_dev_port_t p = BIT_IDX_TO_DEV_PORT(i);
      int has_port = 0;
      TDI_MC_PORT_MAP_GET(port_bitmap_, p, has_port);
      if (has_port) {
        dev_port_list_.push_back(p);
      }
    }
  }
  void setMulticastDevPorts(const std::vector<tdi_id_t> &arr) {
    dev_port_list_ = arr;
    std::memset(port_bitmap_, 0, sizeof(bf_mc_port_map_t));
    for (auto const &i : dev_port_list_) {
      TDI_MC_PORT_MAP_SET(port_bitmap_, i);
    }
  }
  ...
  void setLAGBitmap(const uint8_t *bitmap) {
    ...
    lag_id_list_.clear();
    for (int i = 0; i < TDI_LAG_COUNT; i++) {
      int has_lag = 0;
      TDI_MC_LAG_MAP_GET(lag_bitmap_, i, has_lag);
      if (has_lag) {
        lag_id_list_.push_back(i);
      }
    }
  }
  void setMulticastLAGIDs(const std::vector<tdi_id_t> &arr) {
    lag_id_list_ = arr;
    std::memset(lag_bitmap_, 0, sizeof(bf_mc_lag_map_t));
    for (auto const &i : lag_id_list_) {
      TDI_MC_LAG_MAP_SET(lag_bitmap_, i);
    }
  }
  ...
  const std::vector<tdi_id_t> &getMulticastDevPorts() const {
    return dev_port_list_;
  }

  const std::vector<tdi_id_t> &getMulticastLAGIDs() const {
    return lag_id_list_;
  }
 ...
 private:
  ...
  std::vector<tdi_id_t> dev_port_list_;
  std::vector<tdi_id_t> lag_id_list_;
};

class TdiPREECMPTableData : public TdiTableDataObj {
 public:
  TdiPREECMPTableData(const TdiTableObj *tbl_obj) : TdiTableDataObj(tbl_obj){};

  ~TdiPREECMPTableData() = default;
...
class TdiPRELAGTableData : public TdiTableDataObj {
 public:
  TdiPRELAGTableData(const TdiTableObj *tbl_obj) : TdiTableDataObj(tbl_obj){};

  ~TdiPRELAGTableData() = default;
...
class TdiPREMulticastPruneTableData : public TdiTableDataObj {
 public:
  TdiPREMulticastPruneTableData(const TdiTableObj *tbl_obj)
      : TdiTableDataObj(tbl_obj){};

  ~TdiPREMulticastPruneTableData() = default;
...
class TdiPREMulticastPortTableData : public TdiTableDataObj {
 public:
  TdiPREMulticastPortTableData(const TdiTableObj *tbl_obj,
                               const std::vector<tdi_id_t> &fields)
      : TdiTableDataObj(tbl_obj) {
    set_active_fields(fields);
  }

  TdiPREMulticastPortTableData() = default;
  ...
  const std::set<tdi_id_t> &getActiveDataFields() const { return activeFields; }
 ...
 private:
  ...
  bool checkFieldActive(const tdi_id_t &field_id) const;
  std::unordered_map<tdi_id_t, uint64_t> u64FieldData;
  std::unordered_map<tdi_id_t, bool> boolFieldData;
  std::set<tdi_id_t> activeFields;
};

}  // namespace tdi

#endif  // _TDI_PRE_TABLE_DATA_IMPL_HPP
