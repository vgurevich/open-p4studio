...
TEST_P(TdiInfoTest, TestProgramPipelineProfiles) {
  PipelineProfInfoVec prof_vec;
  auto status = tdiInfo->tdiInfoPipelineInfoGet(&prof_vec);
...
TEST_P(TdiFixedInfoTest, TestTableDataFieldStringChoices) {
  std::vector<const TdiTable *> table_vec;
  ASSERT_EQ(tdiFixedInfo->tdiInfoGetTables(&table_vec), TDI_SUCCESS);
  for (const auto *table : table_vec) {
    tdi_id_t table_id;
    std::string table_name;
    ASSERT_EQ(table->tableIdGet(&table_id), TDI_SUCCESS);
    ASSERT_EQ(table->tableNameGet(&table_name), TDI_SUCCESS);

    if (/* table_name == "tm.port.group_cfg" || */ /* Without a key. */
        table_name == "tm.port.group") {
      // Check if key fields has been parsed corectly
      tdi_id_t key_field_id;
      ASSERT_EQ(table->keyFieldIdGet("pg_id", &key_field_id), TDI_SUCCESS);
    ...
    if (table_name == "tm.port.group_cfg") {
      ...
      ASSERT_EQ(table->dataFieldIdGet("pg_queues", &field_id), TDI_SUCCESS);
      // Try to get choices for a field without choices (should fail).
      EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                TDI_INVALID_ARG)
          << "Falsely giving choices for field " << field_id
    ...
    if (table_name == "tm.port.group") {
      ...
      //---
      ASSERT_EQ(table->actionIdGet("seq", &action_id), TDI_SUCCESS);
      ...
      //---
      ASSERT_EQ(table->actionIdGet("even", &action_id), TDI_SUCCESS);
      // Fields without choices
      for (const auto &fld_n : vf_names_common) {
        ASSERT_EQ(table->dataFieldIdGet(fld_n, action_id, &field_id),
                  TDI_SUCCESS);
      ...
      //---
      ASSERT_EQ(table->actionIdGet("crop", &action_id), TDI_SUCCESS);
      // Fields without choices
      for (const auto &fld_n : vf_names_common) {
        ASSERT_EQ(table->dataFieldIdGet(fld_n, action_id, &field_id),
                  TDI_SUCCESS);
      ...
      //---
      ASSERT_EQ(table->actionIdGet("map", &action_id), TDI_SUCCESS);
      // Fields without choices
      for (const auto &fld_n : vf_names_common) {
        ASSERT_EQ(table->dataFieldIdGet(fld_n, action_id, &field_id),
                  TDI_SUCCESS);
      ...
      if (std::find(j.begin(), j.end(), "tdi_tm_tf2.json") != j.end()) {
        pg_ports = 8;  // TF2
      }
    ...
    if (table_name == "tm.queue.cfg" || table_name == "tm.queue.map" ||
      ...
      ASSERT_EQ(table->keyFieldIdGet("pg_queue", &key_field_id), TDI_SUCCESS);
    ...
    if (table_name == "tm.queue.cfg") {
      ...
      ASSERT_EQ(table->dataFieldIdGet("pfc_cos", &field_id), TDI_SUCCESS);
      // Try to get choices for a field without choices (should fail).
      EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                TDI_INVALID_ARG)
          << "Falsely giving choices for field " << field_id
    ...
    if (table_name == "tm.queue.map") {
      ...
      for (const auto &fld_n : vf_names) {
        ASSERT_EQ(table->dataFieldIdGet(fld_n, &field_id), TDI_SUCCESS);
        // Try to get choices for a field without choices (should fail).
        EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                  TDI_INVALID_ARG)
            << "Falsely giving choices for field " << field_id
    ...
    if (table_name == "tm.queue.color") {
      ...
      auto j = GetParam();
      if (std::find(j.begin(), j.end(), "tdi_tm_tf2.json") != j.end() ||
          std::find(j.begin(), j.end(), "tdi_tm_tf3.json") != j.end()) {
    ...
    if (table_name == "tm.queue.buffer") {
      ...
      //---
      ASSERT_EQ(table->actionIdGet("shared_pool", &action_id), TDI_SUCCESS);
      ...
      //---
      ASSERT_EQ(table->actionIdGet("buffer_only", &action_id), TDI_SUCCESS);
    ...
    if (table_name == "tm.ppg") {
      ...
      tdi_id_t key_field_id;
      ASSERT_EQ(table->keyFieldIdGet("$dev_port", &key_field_id), TDI_SUCCESS);
      ASSERT_EQ(key_field_id, 1);

      ASSERT_EQ(table->keyFieldIdGet("$ppg_id", &key_field_id), TDI_SUCCESS);
      ASSERT_EQ(key_field_id, 2);

      // Verify if Pool Id choices for data field app_pool_id match
      std::vector<std::reference_wrapper<const std::string>> pool_choices;
      std::set<std::string> exp_pool_choices{"TDI_TM_IG_APP_POOL_0",
                                             "TDI_TM_IG_APP_POOL_1",
                                             "TDI_TM_IG_APP_POOL_2",
                                             "TDI_TM_IG_APP_POOL_3"};
      ...
      std::set<std::string> exp_baf_choices{"TDI_TM_PPG_BAF_1_POINT_5_PERCENT",
                                            "TDI_TM_PPG_BAF_3_PERCENT",
                                            "TDI_TM_PPG_BAF_6_PERCENT",
                                            "TDI_TM_PPG_BAF_11_PERCENT",
                                            "TDI_TM_PPG_BAF_20_PERCENT",
                                            "TDI_TM_PPG_BAF_33_PERCENT",
                                            "TDI_TM_PPG_BAF_50_PERCENT",
                                            "TDI_TM_PPG_BAF_66_PERCENT",
                                            "TDI_TM_PPG_BAF_80_PERCENT",
                                            "TDI_TM_PPG_BAF_DISABLE"};
      field_id = 9;
    ...
    if (table_name == "$PORT") {
      {
        std::vector<std::reference_wrapper<const std::string>> choices;
        std::set<std::string> exp_choices{"TDI_SPEED_NONE",
                                          "TDI_SPEED_1G",
                                          "TDI_SPEED_10G",
                                          "TDI_SPEED_25G",
                                          "TDI_SPEED_40G",
        ...
        tdi_id_t field_id;
        ASSERT_EQ(table->dataFieldIdGet("$N_LANES", &field_id), TDI_SUCCESS);
        ...
        ASSERT_EQ(table->dataFieldIdGet("$SPEED", &field_id), TDI_SUCCESS);
        EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                  TDI_SUCCESS)
            << "Unable to get allowed choices for field " << field_id << "\n";
      ...
      {
        std::vector<std::reference_wrapper<const std::string>> choices;
        std::set<std::string> exp_choices{"TDI_FEC_TYP_NONE",
                                          "TDI_FEC_TYP_FIRECODE",
                                          "TDI_FEC_TYP_REED_SOLOMON",
                                          "TDI_FEC_TYP_FC",
                                          "TDI_FEC_TYP_RS"};

        // Get the choices correctly
        tdi_id_t field_id;
        ASSERT_EQ(table->dataFieldIdGet("$FEC", &field_id), TDI_SUCCESS);
        EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                  TDI_SUCCESS)
            << "Unable to get allowed choices for field " << field_id << "\n";
      ...
      {
        std::vector<std::reference_wrapper<const std::string>> choices;
        std::set<std::string> exp_choices{"TDI_LPBK_NONE",
                                          "TDI_LPBK_MAC_NEAR",
                                          "TDI_LPBK_MAC_FAR",
                                          "TDI_LPBK_PCS_NEAR",
                                          "TDI_LPBK_SERDES_NEAR",
                                          "TDI_LPBK_SERDES_FAR"};
...

      {
        ...
        // Get the choices correctly
        tdi_id_t field_id;
        ASSERT_EQ(table->dataFieldIdGet("$PORT_DIR", &field_id), TDI_SUCCESS);
        EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                  TDI_SUCCESS)
            << "Unable to get allowed choices for field " << field_id << "\n";
      ...
      {
        std::vector<std::reference_wrapper<const std::string>> choices;
        std::set<std::string> exp_choices{"TDI_MEDIA_TYPE_COPPER",
                                          "TDI_MEDIA_TYPE_OPTICAL"};

        // Get the choices correctly
        tdi_id_t field_id;
        ASSERT_EQ(table->dataFieldIdGet("$MEDIA_TYPE", &field_id), TDI_SUCCESS);
        EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                  TDI_SUCCESS)
            << "Unable to get allowed choices for field " << field_id << "\n";
...
TEST_P(TdiInfoTest, TestTableKeyFieldStringChoices) {
  if (program_name != "tna_idletimeout") {
    return;
  }
  std::vector<const TdiTable *> table_vec;
  ASSERT_EQ(tdiInfo->tdiInfoGetTables(&table_vec), TDI_SUCCESS);
  for (const auto *table : table_vec) {
    tdi_id_t table_id;
    std::string table_name;
    ASSERT_EQ(table->tableIdGet(&table_id), TDI_SUCCESS);
    ...
    if (table_name == "pipe0.$SNAPSHOT_EGRESS_LIVENESS") {
      ...
      // Since this table does not have choices published in the tdi json
...
TEST_P(TdiInfoTest, TestTableDataFieldStringChoices) {
  ...
  for (const auto *table : table_vec) {
    ...
    if (program_name == "tna_idletimeout") {
      if (table_name == "SwitchIngress.dmac") {
        ...
        tdi_id_t field_id;
        ASSERT_EQ(table->dataFieldIdGet("$ENTRY_TTL", &field_id), TDI_SUCCESS);
        // Try to get choices for a field which is not a string. So this should
        // fail
        EXPECT_EQ(table->dataFieldAllowedChoicesGet(field_id, &choices),
                  TDI_INVALID_ARG)
...
// Testing whether Tdi can handle non fully resolved names if sufficiently
// unique
TEST_P(TdiInfoTest, TestNameUniqueness) {
  if (program_name == "tna_32q_2pipe") {
    const TdiTable *table;
    // Looking just for forward table should fail
    EXPECT_EQ(tdiInfo->tdiTableFromNameGet("forward", &table),
              TDI_OBJECT_NOT_FOUND);
    // Looking for SwitchEgress_a.forward or SwitchEgress_b.forward
    // should pass
    EXPECT_EQ(tdiInfo->tdiTableFromNameGet(
    ...
    const TdiLearn *learn;
    // FIXME Commenting out the following until the time when the corresponding
    // changes in the compiler are merged
    // EXPECT_EQ(tdiInfo->tdiLearnFromNameGet(
    ...
    EXPECT_EQ(tdiInfo->tdiTableFromNameGet("forward", &table), TDI_SUCCESS);
    // Looking for pipe.SwitchIngress.forward should pass
    EXPECT_EQ(tdiInfo->tdiTableFromNameGet("SwitchIngress.forward", &table),
              TDI_SUCCESS);
  } else {
    return;
  }
...
// correctly
TEST_P(TdiInfoTest, TestTableDependencies) {
  std::vector<const TdiTable *> table_vec;
  ASSERT_EQ(tdiInfo->tdiInfoGetTables(&table_vec), TDI_SUCCESS);
  for (const auto *table : table_vec) {
    tdi_id_t table_id;
    ...
    if (program_name == "tna_action_selector") {
      ...
      {
        std::vector<tdi_id_t> depends_on_list;
        ASSERT_EQ(tdiInfo->tdiInfoTablesThisTableDependsOnGet(table_id,
                                                              &depends_on_list),
                  TDI_SUCCESS);
        if (table_name == "SwitchIngress.forward") {
          ...
          for (const auto &iter : depends_on_list) {
            const TdiTable *temp_table;
            EXPECT_EQ(tdiInfo->tdiTableFromIdGet(iter, &temp_table),
                      TDI_SUCCESS);
            std::string temp_table_name;
            EXPECT_EQ(temp_table->tableNameGet(&temp_table_name), TDI_SUCCESS);
          ...
          const TdiTable *temp_table;
          EXPECT_EQ(tdiInfo->tdiTableFromIdGet(depends_on_list[0], &temp_table),
                    TDI_SUCCESS);
          std::string temp_table_name;
          EXPECT_EQ(temp_table->tableNameGet(&temp_table_name), TDI_SUCCESS);
      ...
      {
        std::vector<tdi_id_t> tables_depend_on_this_table_list;
        EXPECT_EQ(tdiInfo->tdiInfoTablesDependentOnThisTableGet(
        ...
        if (table_name == "SwitchIngress.action_selector") {
          ...
          const TdiTable *temp_table;
          EXPECT_EQ(tdiInfo->tdiTableFromIdGet(
...
// was created for every tdi.json entry
TEST_P(TdiInfoTest, TestTableNumbersIsCorrect) {
  std::vector<const TdiTable *> vec;
  auto status = tdiInfo->tdiInfoGetTables(&vec);
  Cjson tables_cjson = (*tdi_root_cjson)["tables"];
  auto tdi_json_num = tables_cjson.array_size();

  uint32_t optimized_out = 0;
  // Loop over tdi.json and check for the table-objs
  for (const auto &table_cjson : tables_cjson.getCjsonChildVec()) {
    ...
    status = tdiInfo->tdiTableFromNameGet(table_name, &table);
    EXPECT_TRUE(status == TDI_SUCCESS || status == TDI_INVALID_ARG)
        << "Failed to get table from name " << table_name << " with status "
        << status << "\n";
    if (status == TDI_INVALID_ARG) {
      optimized_out++;
    }
    ...
    EXPECT_EQ(status, TDI_SUCCESS);
    EXPECT_EQ(table_name, name);
    // check using IDs too
    tdi_id_t table_id = (*table_cjson)["id"];
    status = tdiInfo->tdiTableFromIdGet(table_id, &table);
    EXPECT_EQ(status, TDI_SUCCESS);
  }
  EXPECT_EQ(tdi_json_num - optimized_out, vec.size())
      << "Number of tables parsed is wrong";
}

// Testing whether for a corresponding learn object
// was created for every tdi.json entry
TEST_P(TdiInfoTest, TestLearnObjectNumbersIsCorrect) {
  std::vector<const TdiLearn *> vec;
  auto status = tdiInfo->tdiInfoGetLearns(&vec);
  Cjson learn_cjson = (*tdi_root_cjson)["learn_filters"];

  auto tdi_json_num = learn_cjson.array_size();
  EXPECT_EQ(tdi_json_num, vec.size())
      << "Number of learn filters parsed is wrong";

  // Loop over tdi.json and check for the learn-objs
  for (const auto &learn_cjson_each : learn_cjson.getCjsonChildVec()) {
    std::string learn_name = (*learn_cjson_each)["name"];
    const TdiLearn *learn = nullptr;
    status = tdiInfo->tdiLearnFromNameGet(learn_name, &learn);
    EXPECT_TRUE(status == TDI_SUCCESS)
    ...
    // check using IDs too
    tdi_id_t learn_id = (*learn_cjson_each)["id"];
    status = tdiInfo->tdiLearnFromIdGet(learn_id, &learn);
    EXPECT_EQ(status, TDI_SUCCESS);
  }
}

// Testing for count of Action Profiles
TEST_P(TdiInfoTest, TestActionProfileCountIsCorrect) {
  std::vector<const TdiTable *> vec;
  auto status = tdiInfo->tdiInfoGetTables(&vec);
  ...
  for (const auto &table_cjson : tables_cjson.getCjsonChildVec()) {
    std::string table_name = (*table_cjson)["name"];
    const TdiTable *table_intf = nullptr;
    status = tdiInfo->tdiTableFromNameGet(table_name, &table_intf);
    if (status != TDI_SUCCESS) {
      continue;
    }
    const TdiTableObj *table = dynamic_cast<const TdiTableObj *>(table_intf);

    std::vector<tdi_id_t> action_id_list;
    ...
    if (!((*table_cjson)["action_specs"].exists())) {
      ...
      if (table_type == TdiTable::TableType::MATCH_DIRECT) {
        EXPECT_EQ(status, TDI_OBJECT_NOT_FOUND)
            << "Check failed for table " << table_name;
      } else {
        EXPECT_EQ(status, TDI_NOT_SUPPORTED)
            << "Check failed for table " << table_name;
      }
    } else {
...
TEST_P(TdiInfoTest, TestKeyFieldCountIsCorrect) {
  std::vector<const TdiTable *> vec;
  auto status = tdiInfo->tdiInfoGetTables(&vec);
  ...
  for (const auto &table_cjson : tables_cjson.getCjsonChildVec()) {
    ...
    std::vector<tdi_id_t> field_id_list;
    status = table->keyFieldIdListGet(&field_id_list);

    EXPECT_EQ(status, TDI_SUCCESS) << "Check failed for table " << table_name;
...
TEST_P(TdiInfoTest, TestDataFieldCountIsCorrect) {
  std::vector<const TdiTable *> vec;
  auto status = tdiInfo->tdiInfoGetTables(&vec);
  ...
  for (const auto &table_cjson : tables_cjson.getCjsonChildVec()) {
    ...
    for (auto const &action_spec :
      ...
      tdi_id_t action_id = (*action_spec)["id"];
      std::vector<tdi_id_t> field_id_list;
...
TEST_P(TdiInfoTest, TestTableOperationsParsingIsCorrect) {
  std::vector<const TdiTable *> vec;
  auto status = tdiInfo->tdiInfoGetTables(&vec);
  ...
  for (const auto &table_cjson : tables_cjson.getCjsonChildVec()) {
    ...
    switch (table_type) {
      case TdiTable::TableType::REGISTER: {
        EXPECT_NE(std::find(operations_v.begin(), operations_v.end(), "Sync"),
                  operations_v.end())
            << "Check failed for table " << table_name;
        std::unique_ptr<TdiTableOperations> table_ops;
      ...
      case TdiTable::TableType::COUNTER: {
        EXPECT_NE(std::find(operations_v.begin(), operations_v.end(), "Sync"),
                  operations_v.end())
            << "Check failed for table " << table_name;
        std::unique_ptr<TdiTableOperations> table_ops;
      ...
      case TdiTable::TableType::MATCH_DIRECT:
      case TdiTable::TableType::MATCH_INDIRECT:
      case TdiTable::TableType::MATCH_INDIRECT_SELECTOR: {
...
// correct.
TEST_P(TdiInfoTestActionProfile, TestTableActionProfileParamType) {
  ...
  for (auto const &action_param_pair : paramMap) {
    ...
    for (auto const &item : vec) {
      auto table = dynamic_cast<const TdiTableObj *>(item);
      // find action_id
      tdi_id_t action_id;
