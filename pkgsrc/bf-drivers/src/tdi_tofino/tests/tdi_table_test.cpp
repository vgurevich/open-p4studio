...
namespace {
bool checkDefaultOnly(const TdiTable &table, const TdiTableData &data) {
...
void TdiTableTest::tableRandomKeyGet(const TdiTable &table,
...
void TdiTableTest::tableRandomDataGet(
...
void TdiTableTest::tableKeySetHelper(const GenericKey &random_key,
...
void TdiTableTest::tableDataSetHelper(
    const TdiTable &table,
  ...
  for (const auto &field_id : interested_fields) {
    ...
    switch (data_type) {
      ...
      case DataType::INT_ARR: {
        std::vector<tdi_id_t> mbr_ids = generic_data_field.getValue_intarr();
...
void TdiTableTest::tableDataSetHelper(const TdiTable &table,
                                      const GenericData &single_random_data,
                                      TdiTableData &table_data) {
...
void TdiTableTest::tableExpectedMatchSpecGet(
    const TdiTable &table,
...
void TdiTableTest::tableExpectedActionSpecGet(
    const TdiTable &table,
...
void TdiTableTest::compareKeyObjects(const TdiTableKey &exp_obj,
                                     const TdiTableKey &rcv_obj) const {
  // Get the parent table from the key objects
  const TdiTable *table_1 = nullptr, *table_2 = nullptr;
  ...
  std::vector<tdi_id_t> list_1, list_2;
...
void TdiTableTest::compareDataObjects(const TdiTableData &exp_obj,
                                      const TdiTableData &rcv_obj) const {
  // Get and compare  action id from the table data objects
  tdi_id_t act_id_1 = 0, act_id_2 = 0;
  ...
  for (uint32_t i = 0; i < list_1.size(); i++) {
    ...
    switch (data_type_1) {
      case DataType::INT_ARR: {
        std::vector<tdi_id_t> val_1, val_2;
        EXPECT_SUCCESS(exp_obj.getValue(list_1[i], &val_1));
        EXPECT_SUCCESS(rcv_obj.getValue(list_2[i], &val_2));
...
void TdiMatchActionTableTest::initialTestSetup(
    const TdiTable &table,
    const TdiEntryGen &entry_generator,
    const uint32_t &num_entries) {
  // Call Base's initial setup
  TdiTableTest::initialTestSetup(table, entry_generator, num_entries);
...
void TdiMatchActionTableTest::getTTL(const TdiTable &table,
  ...
  *ttl_val = 0;
  for (const auto &field_id : single_random_data.fieldIdListGet()) {
    const auto &generic_data_field = single_random_data.dataFieldGet(field_id);
    if (single_random_data.actionIdGet()) {
      ASSERT_SUCCESS(static_cast<const TdiTableObj &>(table).getDataField(
...
tdi_status_t TdiMatchActionTableTest::tableEntryAdd(
    const uint32_t &num_entries,
    const TdiTable &table,
  ...
  for (uint32_t n = 0; n < num_entries; n++) {
    ...
    auto ms_matcher = TdiMatchersFactory::makeMatchSpecMatcher(
        exp_ms_vec[n]->getPipeMatchSpec());
    const pipe_action_spec_t *as = exp_as_vec[n]->getPipeActionSpec();
    auto as_matcher = TdiMatchersFactory::makeActionSpecMatcher(as);
      ...
      EXPECT_CALL(
          *pipe_mgr_obj,
          pipeMgrMatEntAdd(_,
                           ...
                           _,
                           ms_matcher,
                           static_cast<const TdiTableObj &>(table).getActFnHdl(
                               single_random_data.actionIdGet()),
                           as_matcher,
                           ttl_val,
                           _,
    ...
    sts = table.tableEntryAdd(
        getDefaultSession(), getDefaultTdiTarget(), *table_key, *table_data);
    if (!action_default_only && !is_const_table) {
      EXPECT_EQ(sts, TDI_SUCCESS)
      ...
      EXPECT_EQ(sts, TDI_INVALID_ARG)
          << "Table entry add succeeded when we "
             "expected it to fail for entry : "
          << n << " Expected status : " << TDI_INVALID_ARG
          << " Received status : " << sts << "\n";
    }
  }
  return TDI_SUCCESS;
...
void TdiMatchActionTableTest::tableEntryModSetExpForField(
    const TdiTable &table,
  ...
  if (direct_resource_found) {
    ...
    auto rs_matcher = TdiMatchersFactory::makeResourceSpecArrayMatcher(
  ...
  if (direct_counter_found) {
    ...
    auto cs_matcher = TdiMatchersFactory::makeCounterSpecMatcher(counter_spec);
    EXPECT_CALL(*pipe_mgr_obj,
...
void TdiMatchActionTableTest::tableEntryModIterateFieldsSetExpForFieldId(
    const TdiTable &table,
    const pipe_ent_hdl_t &ent_hdl,
    const int &index,
  ...
  const auto &single_random_data = random_data[index];
  if (single_random_data.actionIdGet()) {
    ASSERT_SUCCESS(static_cast<const TdiTableObj &>(table).dataFieldTypeGet(
  ...
  auto field_destination =
      TdiTableDataField::getDataFieldDestination(field_types);
  ...
  ASSERT_EQ(bf_status, TDI_SUCCESS);
  if (action_id) {
...
// pipe mgr APIs
tdi_status_t TdiMatchActionTableTest::tableEntryModIterateFields(
  ...
  // Do one keyAllocate and dataAllocate and reset across uses
  std::unique_ptr<TdiTableKey> table_key_mod;
  EXPECT_SUCCESS(table.keyAllocate(&table_key_mod));

  std::unique_ptr<TdiTableData> table_data_mod;
  ...
  for (const auto &iter : entry_hdls) {
    ...
    // Today tdi and the lower level pipe-mgr does not support only one
    ...
    std::map<fieldDestination, std::vector<tdi_id_t>> destination_types;
    for (const auto &field_id : single_random_data.fieldIdListGet()) {
      const TdiTableDataField *table_data_field = nullptr;
      if (single_random_data.actionIdGet()) {
        EXPECT_SUCCESS(static_cast<const TdiTableObj &>(table).getDataField(
            field_id, single_random_data.actionIdGet(), &table_data_field));
      } else {
        EXPECT_SUCCESS(static_cast<const TdiTableObj &>(table).getDataField(
            field_id, &table_data_field));
    ...
    for (const auto &it : destination_types) {
      ...
      // API version
      bool is_action_param = it.first == tdi::fieldDestination::ACTION_SPEC;
      ...
      for (const auto &field_id : it.second) {
        ...
        EXPECT_CALL(*pipe_mgr_obj, pipeMgrMatchSpecToEntHdl(_, _, _, _, _))
            .Times(1)
        ...
        // Finally call the tdi table entry modify API
...
void TdiTableTest::formTableKey_from_idx(const TdiTable &table,
                                         const int &index,
                                         TdiTableKey *table_key) {
  TDI_ASSERT(static_cast<uint32_t>(index) < random_keys.size());
...
void TdiTableTest::formTableData_from_idx(const TdiTable &table,
                                          const int &index,
                                          TdiTableData *table_data) {
  TDI_ASSERT(static_cast<uint32_t>(index) < random_data.size());
...
// 0 <-> (n-1), 1 <-> (n-2), ....
tdi_status_t TdiMatchActionTableTest::tableEntryModSwapActionData(
    const uint32_t &num_entries,
    const TdiTable &table,
  ...
  for (auto iter = entry_hdls.begin(); iter != entry_hdls.end(); iter++) {
    ...
    for (const auto &field_id : single_random_data.fieldIdListGet()) {
      std::set<DataFieldType> field_types;
      if (single_random_data.actionIdGet()) {
        EXPECT_SUCCESS(static_cast<const TdiTableObj &>(table).dataFieldTypeGet(
            field_id, single_random_data.actionIdGet(), &field_types));
      } else {
        EXPECT_SUCCESS(static_cast<const TdiTableObj &>(table).dataFieldTypeGet(
            field_id, &field_types));
...
void TdiMatchActionTableTest::tableEntryGetSetExpForEntry(
    const TdiTable &table,
    const GenericData &single_random_data,
    const int &times) {
  // TDI calls different APIs on pipe mgr backend depending on the types
  ...
  // Currently TDI allows 2 types of data allocates.
  ...
  // id, we expect TDI to always read the action spec from pipe mgr. Thus
...
TEST_P(TdiMatchActionTableTest, EntryAdd) {
  ...
  for (uint32_t n = 0; n < d_tables.size(); n++) {
    ...
    TdiTableScopeGuard scope_guard(
...
TEST_P(TdiMatchActionTableTest, EntryGet) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t n = 0; n < d_tables.size(); n++) {
    ...
    for (auto iter = entry_hdl_list.begin(); iter != entry_hdl_list.end();
      ...
      std::unique_ptr<TdiTableData> table_data_expected;
      if (single_random_data.actionIdGet()) {
...
TEST_P(TdiMatchActionTableTest, EntryGetIterator) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t iter = 0; iter < d_tables.size(); iter++) {
    ...
    TdiTableScopeGuard scope_guard(*this,
                                   table,
                                   entry_generator,
                                   num_entries,
                                   pipe_mgr_obj,
    ...
    std::unique_ptr<TdiTableKey> table_key_expected;
    ...
    std::unique_ptr<TdiTableKey> first_key;
    EXPECT_SUCCESS(table.keyAllocate(&first_key));
    std::unique_ptr<TdiTableData> first_data;
    ...
    std::vector<std::pair<TdiTableKey *, TdiTableData *>> next_entries;
    uint32_t next_entries_count = entry_hdl_list.size();
    std::vector<std::unique_ptr<TdiTableKey>> next_keys(next_entries_count);
    std::vector<std::unique_ptr<TdiTableData>> next_data(next_entries_count);
    ...
    EXPECT_CALL(*pipe_mgr_obj, pipeMgrMatchSpecToEntHdl(_, _, _, _, _))
        ...
        .RetiresOnSaturation();
    EXPECT_CALL(*pipe_mgr_obj,
                pipeMgrGetNextEntryHandles(
                    _, _, _, first_entry_hdl, next_entries_count, _))
        .Times(1)
        .WillOnce(Invoke(&pipe_mgr_obj->getMockIPipeMgrIntfHelper(),
                         &MockIPipeMgrIntfHelper::pipeMgrGetNextEntryHandles))
        .RetiresOnSaturation();
    // Set correct expectations on the relevant pipe mgr APIs
...
TEST_P(TdiMatchActionTableTest, EntryMod) {
  uint32_t num_entries = std::get<2>(GetParam());
...
TEST_P(TdiMatchActionTableTest, EntryDel) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t n = 0; n < d_tables.size(); n++) {
    ...
    for (uint32_t m = 0; m < num_entries; m++) {
      ...
      auto sts = table.tableEntryDel(
          getDefaultSession(), getDefaultTdiTarget(), *table_key);
      if (!is_const_table) {
        EXPECT_EQ(sts, TDI_SUCCESS)
        ...
        EXPECT_EQ(sts, TDI_INVALID_ARG)
            ...
            << m << " Expected status : " << TDI_INVALID_ARG
            << " Received status : " << sts << "\n";
      }
    }
  }
...
void TdiActionTableTest::initialTestSetup(const TdiTable &table,
                                          const TdiEntryGen &entry_generator,
                                          const uint32_t &num_entries) {
  // Call Base's initial setup
  TdiTableTest::initialTestSetup(table, entry_generator, num_entries);
...
tdi_status_t TdiActionTableTest::tableEntryAdd(
    const uint32_t &num_entries,
    const TdiTable &table,
    std::map<pipe_ent_hdl_t, int> *entry_hdls) {
  // - Allocate 'num_entries' number of key and data objects
...
TEST_P(TdiActionTableTest, EntryAdd) {
  uint32_t num_entries = std::get<2>(GetParam());
...
TEST_P(TdiActionTableTest, DISABLED_EntryGet) {
  uint32_t num_entries = std::get<2>(GetParam());
...
TEST_P(TdiActionTableTest, DISABLED_EntryGetIterator) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t n = 0; n < d_tables.size(); n++) {
    ...
    // TDI is free to return any of the installed entries as the first entry
...
TEST_P(TdiActionTableTest, DISABLED_EntryMod) {
  uint32_t num_entries = std::get<2>(GetParam());
...
TEST_P(TdiActionTableTest, DISABLED_EntryDel) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t n = 0; n < d_tables.size(); n++) {
    ...
    for (auto iter = entry_hdl_list.begin(); iter != entry_hdl_list.end();
      ...
      // Since TDI itself stores state for action tables, we need to ensure
      // that that state gets updated properly. Thus try to read the entry
      // and expect it to fail
      std::unique_ptr<TdiTableData> table_data_tmp;
      ...
      EXPECT_EQ(table.tableEntryGet(getDefaultSession(),
                                    ...
                                    *table_key,
                                    getDefaultTableReadFlag(),
                                    table_data_tmp.get()),
                TDI_OBJECT_NOT_FOUND);
    }
  }
}

void TdiSelectorTableTest::initialTestSetup(const TdiTable &table,
                                            const TdiEntryGen &entry_generator,
                                            const uint32_t &num_entries) {
...
void TdiSelectorTableTest::addEntriesInActProfTable(
    const TdiTable &act_tbl, std::unordered_set<uint32_t> *seen_mbr_ids) {
  ...
  for (auto iter = new_mbr_ids.begin(); iter != new_mbr_ids.end();
    ...
    // This is what is maintained by tdi
    mbr_id_to_adt_ent_hdl_map.insert(std::make_pair(*iter, adt_ent_hdl));
  }
}

tdi_status_t TdiSelectorTableTest::tableEntryAdd(
    const uint32_t &num_entries,
    const TdiTable &table,
  ...
  for (uint32_t n = 0; n < num_entries; n++) {
    ...
    EXPECT_CALL(*pipe_mgr_obj,
                pipeMgrSelGrpAdd(
                    _, _, _, _, single_random_data.getMaxGroupSize(), _, _))
        .Times(1)
        .WillOnce(DoAll(
            WithArgs<5>(Invoke(
    ...
    auto mbr_id_matcher =
        TdiMatchersFactory::makeGroupMbrIdMatcher(adt_ent_hdls);
    auto mbr_sts_matcher = TdiMatchersFactory::makeGroupMbrStsMatcher(mbr_sts);
    EXPECT_CALL(*pipe_mgr_obj,
...
TEST_P(TdiSelectorTableTest, DISABLED_EntryAdd) {
...
TEST_P(TdiSelectorTableTest, DISABLED_EntryGet) {
  const uint32_t num_entries = std::get<2>(GetParam());
...
TEST_P(TdiSelectorTableTest, DISABLED_EntryGetIterator) {
  const uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t iter = 0; iter < d_tables.size(); iter++) {
    ...
    EXPECT_CALL(*pipe_mgr_obj, pipeMgrSelGrpMbrsGet(_, _, _, _, _, _, _, _))
        .Times(1)
        .WillOnce(Invoke(&pipe_mgr_obj->getMockIPipeMgrIntfHelper(),
                         &MockIPipeMgrIntfHelper::pipeMgrSelGrpMbrsGet))
        .RetiresOnSaturation();
...
TEST_P(TdiSelectorTableTest, DISABLED_EntryMod) {
  const uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t iter = 0; iter < d_tables.size(); iter++) {
    ...
    for (auto iter_3 = group_hdl_list.begin(); iter_3 != group_hdl_list.end();
      ...
      auto mbr_sts_matcher =
          TdiMatchersFactory::makeGroupMbrStsMatcher(mbr_sts);
      EXPECT_CALL(*pipe_mgr_obj,
                  pipeMgrSelGrpMbrsSet(_,
                                       _,
                                       _,
...
TEST_P(TdiSelectorTableTest, DISABLED_EntryDel) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t iter = 0; iter < d_tables.size(); iter++) {
    ...
    for (auto iter_4 = group_hdl_list.begin(); iter_4 != group_hdl_list.end();
      ...
      // Since TDI itself stores state for selector tables, we need to ensure
      // that that state gets updated properly. Thus try to read the entry
      // and expect it to fail
...
//
//-----------------------------------------------------------------------
// TDI TM Queue table test
void TdiTMQueueTableTest::initialTestSetup(const TdiTable &table,
                                           const TdiEntryGen &entry_generator,
                                           const uint32_t &num_entries) {
...
tdi_status_t TdiTMQueueTableTest::tableTest(const uint32_t &num_entries,
                                            const TdiTable &table) {
  std::string table_name;
  table.tableNameGet(&table_name);
  std::unique_ptr<TdiTableKey> table_key;
...
// TODO: implement it
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(TdiTMQueueTableTest);

void TdiPREMulticastNodeTableTest::initialTestSetup(
    const TdiTable &table,
    const TdiEntryGen &entry_generator,
    const uint32_t &num_entries) {
  // Clear out all the vectors
...
TEST_P(TdiPREMulticastNodeTableTest, EntryAllocateGet) {
  uint32_t num_entries = std::get<2>(GetParam());
  ...
  for (uint32_t n = 0; n < this->d_tables.size(); n++) {
    ...
    std::unique_ptr<TdiTableKey> tb_key;
    std::unique_ptr<TdiTableData> tb_data;
