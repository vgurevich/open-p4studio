...
# The following method categorizes the ports in ports.json file as belonging to either of the pipes
swports_by_pipe = [[] for _ in range(num_pipes)]
for port in swports:
...
class HitlessTnaPortMetadataNegative(HitlessBaseTestNegative):
    ...
    def post_hitless_validation(self):
        ...
        '''
        self.igr_to_egr_port_map = {}
        for i, j in six.iteritems(self.replayed_igr_to_egr_port_map):
            self.igr_to_egr_port_map[i] = j

        self.pm_dict = {}
        for i, j in six.iteritems(self.replayed_pm_dict):
            self.pm_dict[i] = j

        self.exm_dict = {}
        for i, j in six.iteritems(self.replayed_exm_dict):
            self.exm_dict[i] = j
...
class HitlessTnaLpmAlpmNegative(HitlessBaseTestNegative):
    ...
    def post_hitless_validation(self):
        ...
        self.key_list = []
        self.data_list = []
        for i, j in six.iteritems(self.replayed_alpm_dict):
            self.alpm_dict[i] = j
...
class HitlessTnaTernaryMatchIndirectNegative(HitlessBaseTestNegative):
    ...
    def post_hitless_validation(self):
        ...
        '''
        self.action_table_dict = {}
        for i, j in six.iteritems(self.replayed_action_table_dict):
            self.action_table_dict[i] = j

        self.tcam_table_dict = {}
        for i, j in six.iteritems(self.replayed_tcam_table_dict):
...
class HitlessDynHashingNegative(HitlessBaseTestNegative):
    ...
    def setup_test_data(self):
        ...
        self.target = gc.Target(device_id=0, pipe_id=0xFFFF)
        slice_tuple_list = []
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.proto",   start_bit=0, length=8,  order=4))
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.sip",     start_bit=0, length=32, order=5))
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.dip",     start_bit=0, length=32, order=1))
        slice_tuple_list.append(HashFieldSlice(name="hdr.tcp.sPort",    start_bit=0, length=16, order=2))
        slice_tuple_list.append(HashFieldSlice(name="hdr.tcp.dPort",    start_bit=0, length=16, order=3))
    ...
    def post_hitless_validation(self):
        ...
        logger.info("Create some slices of the hash fields")
        slice_tuple_list = []
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.proto",   start_bit=0,  length=2,  order=4))
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.proto",   start_bit=2,  length=4,  order=5))
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.sip",     start_bit=0,  length=32, order=0))
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.dip",     start_bit=0,  length=15, order=1))
        slice_tuple_list.append(HashFieldSlice(name="hdr.ipv4.dip",     start_bit=16, length=15, order=2))
        slice_tuple_list.append(HashFieldSlice(name="hdr.tcp.sPort",    start_bit=0,  length=16, order=0))
        slice_tuple_list.append(HashFieldSlice(name="hdr.tcp.dPort",    start_bit=0,  length=16, order=3))
        self.data_list = [self.create_hash_data(self.hash_config_table, slice_tuple_list)]
        self.hash_config_table.default_entry_set(self.target, self.data_list[0])
    ...
    def get_entries_and_verify(self, from_hw = True):
        ...
        resp = self.hash_config_table.default_entry_get(self.target, {"from_hw": False})
        i = 0
        for data, key in resp:
...
class HitlessIndirectMeterTestNegative(HitlessBaseTestNegative):
    ...
    def post_hitless_validation(self):
        ...
        for key, data in six.iteritems(self.meter_data):
