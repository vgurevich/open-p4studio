...
class MultipleProgramWithReplayTest(MultiProgramTest):
    ...
    def runTest(self):
        ...
        time.sleep(3)
        try:
            self.a.port_table = self.a.bfrt_info.table_get("$PORT")
            self.a.port_table.attribute_port_status_change_set(target, enable=True)
...
class MultipleProgramToMultipleProgramTest(MultiProgramTest):
    ...
    def runTest(self):
        ...
        # For this test, the profile name should remain the same because the
        # prefix is already present in the table/learn names in bf-rt.json
        profile_name_to_put = profile_name_to_pick = "profile_a"
