...
control SwitchIngressDeparser_a(
    ...
    apply {
        if (ig_dprsr_md.digest_type == 0) {
            digest_b.pack({hdr.ipv4.dst_addr, hdr.ipv4.src_addr});
        } else if (ig_dprsr_md.digest_type == 1) {
            digest_a.pack({hdr.ethernet.dst_addr, hdr.ethernet.src_addr});
        }
...
control SwitchIngress_a(
    ...
    action dmac_miss(bit<3> dg_type) {
        ig_dprsr_md.digest_type = dg_type;
    }
    table learning {
        key = {
            hdr.ethernet.dst_addr : exact;
