...
class DirectCounterModifyNoReset(BfRuntimeTest):
    """@brief Tests modification of single entry in match table with direct counters while
       showcasing reset_stats flag, which allows to modify entry without reseting counter
       value. Table has actions available (with and without action data) in order to test
       both cases.
    """
    ...
    def verifyCounter(self, exp_cnt_pkts, exp_cnt_bytes):
        for data, key in self.table.entry_get(self.target,
                                                 [],
                                                 flags={"from_hw": True}):
            data_dict = data.to_dict()
            assert exp_cnt_pkts == data_dict["$COUNTER_SPEC_PKTS"]
            assert exp_cnt_bytes == data_dict["$COUNTER_SPEC_BYTES"]

    def runTest(self):
        ig_port = swports[0]
        eg_port = swports[1]
        dmac = '10:22:33:44:55:66'

        # Get bfrt_info and set it as part of the test
        bfrt_info = self.interface.bfrt_info_get("tna_counter")

        self.table = bfrt_info.table_get("SwitchIngress.forward_or_drop")
        self.table.info.key_field_annotation_add("hdr.ethernet.dst_addr", "mac")

        pkt = testutils.simple_tcp_packet(pktlen=pkt_len, eth_dst=dmac)
        exp_pkt = pkt

        self.target = gc.Target(device_id=0, pipe_id=0xffff)
        key = self.table.make_key([gc.KeyTuple('hdr.ethernet.dst_addr', dmac)])
        data_port = self.table.make_data([gc.DataTuple('port', eg_port)],
                                         'SwitchIngress.set_port') 
        data_drop = self.table.make_data([], 'SwitchIngress.drop')
        self.table.entry_add(self.target, [key], [data_port])
        logger.info("expecting packet on port %d", eg_port)
        testutils.send_packet(self, ig_port, pkt)
        testutils.verify_packet(self, exp_pkt, eg_port)

        exp_cnt_pkts = 1;
        exp_cnt_bytes = pkt_len + 4;
        self.verifyCounter(exp_cnt_pkts, exp_cnt_bytes)

        # Modify entry and check counters few times
        test_data = [data_drop, data_port, data_drop, data_port, data_drop]
        flags = {'reset_stats':False}

        for data in test_data:
            self.table.entry_mod(self.target, [key], [data], flags = flags)

            testutils.send_packet(self, ig_port, pkt)

            # Everytime packet should be counted and counter should not get reset on modify.
            exp_cnt_pkts += 1;
            exp_cnt_bytes += pkt_len + 4;
            ...
            if data == data_port:
                ...
                try:
                    ...
                    # When running with the model and a small negative timeout
                    # it is possible the model takes longer to process a packet
                    # than the timeout of verify_no_other_packets.  This results
                    # in the test checking the counter result before the model
                    # has processed the packet.  Ensure that is not the reason
                    # for this failure by waiting a little longer and trying
                    # again.
                    time.sleep(3)
                    testutils.verify_no_other_packets(self)
                    self.verifyCounter(exp_cnt_pkts, exp_cnt_bytes)
