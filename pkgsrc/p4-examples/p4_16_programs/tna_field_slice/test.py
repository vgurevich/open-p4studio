...
def MultiSliceTestHelper(self, table_obj, exm_0_field_name, tcam_field_name, tcam_1_field_name, exm_1_field_name,
                         is_multi_field):
    """ @brief
    ...
    for entry_idx in range(num_entries):
        ...
        try:
            ...
            tcam_1_v6_addr = ipv6_dst_addr
            tcam_1_v6_addr_mask = ipv6_dst_addr_mask
            ...
            # ternary-1
            tcam_1_val = getIpv6AddrFieldSliceValue(tcam_1_field_name, tcam_1_v6_addr)
            tcam_1_mask = getIpv6AddrFieldSliceValue(tcam_1_field_name, tcam_1_v6_addr_mask)
            tcam_1_field_bit_width = getFieldSliceBitWidthFromName(tcam_1_field_name)
            tcam_1_field_byte_width = getFieldSliceByteWidthFromName(tcam_1_field_name)
            logger.debug("TCAM_1 value before scrambling " + hex(tcam_1_val))
            # Scramble bits if we want the pkt to miss in this iteration because of a mismatch on this field
            if should_tcam_1_match == False:
                # Flip any bit which we expect to match on
                index = tcam_1_field_bit_width
                for i in range(tcam_1_field_bit_width):
                    if ((tcam_1_mask >> i) & 0x01) == 1:
                        index = i
                if index == tcam_1_field_bit_width:
                    # indicates that the generated  mask is all zeros. Thus ternary field is always going to match
                    # Hence flip the flag accordingly
                    should_tcam_1_match = True
                else:
                    tcam_1_val = garbleBits(tcam_1_val, [index])
            # Always scramble bits which are don't care
            garble_bit_index.clear()
            for i in range(tcam_1_field_bit_width):
                bit_val = (tcam_1_mask >> i) & 1
                if bit_val == 0:
                    garble_bit_index.add(i)
            tcam_1_val = garbleBits(tcam_1_val, garble_bit_index)
            logger.debug("TCAM_1 value after scrambling " + hex(tcam_1_val))
            # We never scramble the tcam mask
            logger.debug("TCAM_1 mask " + hex(tcam_1_mask))
            ...
            table_key = table_obj.make_key(
                [gc.KeyTuple(self.port_field_name, ig_port),
                 ...
                 gc.KeyTuple(tcam_1_field_name, tcam_1_val, tcam_1_mask),
            ...
            resp = table_obj.entry_get(target, None, {"from_hw": True})
            # for tcam, the mask is applied on the value before programming it in the hardware
            # thus get the expected tcam val to be read from hw
            tcam_value_hw = tcam_val & tcam_mask
            tcam_1_value_hw = tcam_1_val & tcam_1_mask
            for data, key in resp:
                data_dict = data.to_dict()
                key_dict = key.to_dict()
                assert key_dict[self.port_field_name]['value'] == ig_port
                ...
                assert key_dict[tcam_1_field_name]['value'] == tcam_1_value_hw
                assert key_dict[tcam_1_field_name]['mask'] == tcam_1_mask
            ...
            logger.info("Should Tcam_1 match %d", should_tcam_1_match)
            logger.info("Should Exm_1 match %d", should_exm_match_1)
            if should_exm_match_0 == 1 and should_tcam_match == 1 and should_tcam_1_match == 1 and should_exm_match_1 == 1:
...
class SingleFieldMultiSliceTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        MultiSliceTestHelper(self, table_obj, exm_0_field_name="hdr.ipv6.dst_addr[41:5]",
                             tcam_field_name="hdr.ipv6.dst_addr[127:54]", tcam_1_field_name="hdr.ipv6.dst_addr[53:42]",
...
class SingleFieldMultiSliceAliasTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        MultiSliceTestHelper(self, table_obj, exm_0_field_name="ipv6.dstaddr_23_9_",
                             tcam_field_name="ipv6.dstaddr_57_28_", tcam_1_field_name="ipv6.dstaddr_100_70_",
...
class MultiFieldMultiSliceTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        MultiSliceTestHelper(self, table_obj, exm_0_field_name="hdr.ipv6.src_addr[84:50]",
                             tcam_field_name="hdr.ipv6.src_addr[43:0]", tcam_1_field_name="hdr.ipv6.dst_addr[127:16]",
...
class MultiFieldMultiSliceAliasTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        MultiSliceTestHelper(self, table_obj, exm_0_field_name="hdr.ipv6.src_addr[53:19]",
                             tcam_field_name="ipv6.srcaddr_106_80_", tcam_1_field_name="hdr.ipv6.dst_addr[81:60]",
