...
def verify_packet_list(test, port_ll, pkt_ll):
    ...
    while more_to_rx:
        ...
        if rcv_port is None:
            ...
            logger.info("Expected ports remaining: %s", port_ll)
        ...
        if found_port != True or found_pkt != True:
            logger.info("Unexpected Rx: port %d", rcv_port)
            logger.info("%s", format_packet(rcv_pkt))
            logger.info("Expected the following:")
            for port_list, pkt_list in zip(port_ll, pkt_ll):
                logger.info("  Ports: %s", sorted(port_list))
                for pkt in pkt_list:
                    logger.info("%s", format_packet(pkt))
    ...
    if rcv_port != None:
        logger.info("Extra Rx: port %d Packet %s", rcv_port, format_packet(rcv_pkt))
...
class LagGrp:
    ...
    def getMbrByHash(self, h, pkt_rid, node_rid, yid):
        global t
        # Tofino-2 and later will include the node's RID in the passed in hash
        # value by simply XORing the two together.  Note the hash value is only
        # 13 bits so we apply a mask to ensure it stays that way.
...
class MCTree:
    ...
    def reprogram(self):
        ...
        for l1 in self.nodes:
            l1.associate(self.bfrt_test, self.mgid_hdl, l1.xid)
        for grp, xid in self.ecmps:
    ...
    def get_ports(self, pkt_rid, pkt_xid, pkt_yid, pkt_hash1=0, pkt_hash2=0):
        ...
        for l1 in self.nodes:
            if l1.xid is not None and pkt_xid == l1.xid:
                continue
            ports = l1.getPorts(self.bfrt_test, pkt_rid, pkt_xid, pkt_yid, pkt_hash2)
    ...
    def print_tree(self):
        logger.info("Dev: %d MGID: 0x%x Num L1 Nodes: %d Num ECMPs: %d", self.dev, self.mgid, len(self.nodes), len(self.ecmps))
        for l1 in self.nodes:
            if l1.xid is not None:
                logger.info("  L1_Hdl: 0x%x RID: 0x%x XID: 0x%x Ports %s LAGS %s", l1.l1_hdl, l1.rid, l1.xid, l1.mbr_ports, l1.mbr_lags)
            else:
                logger.info("  L1_Hdl: 0x%x RID: 0x%x Ports %s LAGS %s", l1.l1_hdl, l1.rid, l1.mbr_ports, l1.mbr_lags)
        for grp, xid in self.ecmps:
            if xid is not None:
                logger.info("  ECMP Hdl: 0x%x XID: 0x%x", grp, xid)
            else:
                logger.info("  ECMP Hdl: 0x%x", grp)
...
class TestMulticastc2c(BfRuntimeTest):
    # Sets up unicast, Broadcast and multicast forwarding tables
    # with copy_to_cpu flag enable.
    # Tests basic unicast forwarding, broadcast and multicast
    ...
    def runTest(self):
        ...
        This test does the following:
          - Programs unicast forwarding rule based on DMAC.
            If DMAC matches, packet will be forwarded to the next port along
            with a copied packet to CPU. A copy will be forwarded to CPU.
          - Programs multicast forwarding rule based on multicast DMAC.
            ...
            multicast programming for the MGID. A copy will be forwarded to CPU.
          - For each MGID, creates a L1 node and adds random number of ports
            to the L1 node
          - Sends unicast packet to each port and verifies that it is
            correctly forwarded to group member ports and CPU port.
          - Sends multicast packet to each port and verifies that the packet
            gets flooded based on the assigned MGID to group member ports and
            CPU ports.
        '''
        setup_random()
        ...
        ifid_to_brid = {}
        # Dict to hold multicast mgid1 for each bridge id
        brid_to_mgid = {}
        ...
        for p in swports:
            port_to_ifid[p] = p
            ifids .add(p)
            logger.info("Our ports:%d", p)

        # Randomly select bridge ids equal to the number of ports (ifids)
        # from the range between 0 and 64K-1
        brids.update(random.sample(range(0, 0xFFFE), len(ifids)))
        ...
        ip_addr_list = [ipv4Addr_to_i32("224.100.0.0")]
        for i in range(1, len(brid_list)):
            ip_addr_list.append(ip_addr_list[i-1] + 1)
            ip_addr_str_list.append(i32_to_ipv4Addr(ip_addr_list[i]))
        ...
        try:
            ...
            logger.info("Adding SwitchIngress.ing_port table entries")
            ...
            logger.info("Adding SwitchIngress.ing_src_ifid table entries")
            for port_num in swports:
                ifid = port_to_ifid[port_num]
            ...
            logger.info("Adding MGID table entries")
            for brid in brids:
            ...
            # Store reverse of the MGIDs for the second table of MGIDs
            # Add IP entries
            logger.info("Adding SwitchIngress.ing_ipv4_mcast table entries")
            for i, port in enumerate(swports):
                ing_port = port
                ifid = port_to_ifid[ing_port]
                brid = ifid_to_brid[ifid]
                mgid1 = brid_to_mgid[brid]
                xid = (random.randint(1, 0x10000) & 0xFFFF)
                self.ing_ipv4_mcast_table.entry_add(
                    target,
                    [self.ing_ipv4_mcast_table.make_key([
                        client.KeyTuple('ig_md.vrf', brid),
                        client.KeyTuple('hdr.ipv4.src_addr', 0, 0),
                        client.KeyTuple('hdr.ipv4.dst_addr', ip_addr_str_list[i],
                                        '255.255.255.255'),
                        client.KeyTuple('$MATCH_PRIORITY', 1)])],
                    [self.ing_ipv4_mcast_table.make_data([client.DataTuple('xid', xid),
                                                          client.DataTuple('mgid1', mgid1),
                                                          client.DataTuple('mgid2', 0xFFFF)],
                                                         'SwitchIngress.mcast_route')]
                )

            # Allocate a single L1 node for each brid
            logger.info("Adding MC node table entries")
            l1_id = 1
            for brid in brids:
            ...
            for i in range(0, len(swports)):
                logger.info("Sending unicast on port %d expecting receive on port %s and cpu_port %s", swports[i],
                                        swports[(i + 1) % len(swports)],
                                        swports[1])
                rx_port_list = [swports[1], swports[(i + 1) % len(swports)]]
                pkt = build_rx_packet_list(rx_port_list, simple_tcp_packet(eth_dst='00:11:11:11:11:00',
                ...
                send_packet(self, swports[i], epkt)
                verify_packet_list(self, [rx_port_list], [pkt])

            for ing_port in swports:
                ifid = port_to_ifid[ing_port]
                brid = ifid_to_brid[ifid]
                rx_port_list = list(ports_in_tree[brid])

                logger.info("Sending multicast on port %d expecting receive on ports %s and cpu_port:%s",
                                                            ing_port, rx_port_list, swports[1])
                rx_port_list.append(swports[1])
            ...
            for i, ing_port in enumerate(swports):
                ifid = port_to_ifid[ing_port]
                brid = ifid_to_brid[ifid]
                rx_port_list = list(ports_in_tree[brid])

                logger.info("Sending multicast route on port %d expecting receive on ports %s and cpu_port:%s",
                                                            ing_port, rx_port_list, swports[1])
                rx_port_list.append(swports[1])
                rx_pkt_list = build_rx_packet_list(rx_port_list, simple_tcp_packet(eth_dst='01:00:5e:00:00:00',
                                                                                   eth_src='00:22:22:22:22:22',
                                                                                   ip_src='1.1.1.1',
                                                                                   ip_dst=ip_addr_str_list[i],
                                                                                   ip_tos=255,
                                                                                   ip_ttl=63,
                                                                                   ip_id=101))
                pkt = simple_tcp_packet(eth_dst='01:00:5e:00:00:00',
                                        eth_src='00:22:22:22:22:22',
                                        ip_src='1.1.1.1',
                                        ip_dst=ip_addr_str_list[i],
                                        ip_tos=255,
            ...
            logger.info("Disable port %d as CopyToCPU port", swports[1])
            self.port_table.entry_mod(
                target,
                [self.port_table.make_key([
                    client.KeyTuple('$DEV_PORT', swports[0])])],
                [self.port_table.make_data([
            ...
            self.ing_port_table.entry_del(target, [])
            self.ing_src_ifid_table.entry_del(target, [])
            self.ing_dmac_table.entry_del(target, [])
            self.ing_ipv4_mcast_table.entry_del(target, [])
...
class TestBasic(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        try:
            ...
            for i in range(0, len(swports)):
                logger.info("Sending unicast on port %d expecting receive on port %d", swports[i], swports[(i + 1) % len(swports)])
            ...
            for ing_port in swports:
                ...
                rx_pkt_list = build_rx_packet_list(rx_port_list, simple_tcp_packet(eth_dst='FF:FF:FF:FF:FF:FF',
                                                                                   ...
                                                                                   ip_ttl=64,
                                                                                   ip_id=101))
                logger.info("Sending multicast on port %d expecting receive on ports %s", ing_port, rx_port_list)
                pkt = simple_tcp_packet(eth_dst='FF:FF:FF:FF:FF:FF',
                                        eth_src='00:22:22:22:22:22',
                                        ip_src='1.1.1.1',
                                        ip_dst='10.0.0.1',
    ...
    def runTest(self):
        ...
        try:
            ...
            for i in range(0, len(ifid_list)):
                ...
                logger.info("Adding MC prune table entries %d", port_cnt)
            ...
            for port in port_list:
                ...
                logger.info("Sending multicast on port %d expecting receive on ports %s", port, rx_port_list)
            ...
            for port in port_list:
                ...
                for p in mbr_port_list:
                    rx_port_list.append(p)
                logger.info("Sending multicast on port %d expecting receive on ports (NO PRUNING) %s", port, rx_port_list)
                pkt = simple_tcp_packet(eth_dst='E4:83:A6:93:02:4B',
                                        eth_src='00:22:22:22:22:22',
                                        ip_src='1.1.1.1',
                                        ip_dst='10.0.0.1',
            ...
            for port in port_list:
                ...
                for p in mbr_port_list:
                    if p not in prune_list:
                        rx_port_list.append(p)
                logger.info("Sending multicast on port %d expecting receive on ports (GLOBAL PRUNE) %s", port, rx_port_list)
                pkt = simple_tcp_packet(eth_dst='E4:83:A6:93:02:4B',
                                        eth_src='00:22:22:22:22:22',
                                        ip_src='1.1.1.1',
                                        ip_dst='10.0.0.1',
        ...
        try:
            ...
            for port in test_ports:
                ...
                for ip in test_addrs:
                    ...
                    logger.info("Sending %s on port %d expecting receive on ports: %s", ip_addr_str_list[ip_addr_list.index(ip)], port, rx_port_list)
                    send_packet(self, port, pkt)
                    verify_packet_list(self, rx_port_list, rx_pkt_list)
...
class TestLag(BfRuntimeTest, pd_base_tests.ThriftInterfaceDataPlane):
    ...
    def runTest(self):
        ...
        if not support_hw_port_flap:
            hw_down_ports = []
        logger.info("HW down ports: %s", sorted(hw_down_ports))
        sw_down_ports = random.sample(port_list, len(port_list) // 2)
        logger.info("SW down ports: %s", sorted(sw_down_ports))
        ...
        try:
            ...
            while len(test_lags):
                if len(test_lags) > lags_per_pass:
                    ...
                    test_lags = []
                logger.info("Checking LAG IDs: %s", lag_ids)
                ...
                for step in range(1, 6):
                    ...
                    logger.info("  Sending on port %d expecting receive on ports:", ing_port)
                    for x, y in zip(rx_rid_list, rx_port_list):
                        logger.info("    RID 0x%x Ports %s", x, sorted(y))
                    send_packet(self, ing_port, pkt)
...
class TestAttributes(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        try:
            ...
            logger.info("Configuring global rid to 0x%x", global_rid)
            self.mgid_table.attribute_pre_device_config_set(target, global_rid=global_rid)
            logger.info("Enabling port protection")
            self.mgid_table.attribute_pre_device_config_set(target, port_protection_enable=port_protection)
            logger.info("Enabling fast failover")
            self.mgid_table.attribute_pre_device_config_set(target, fast_failover_enable=fast_failover)
            logger.info("Configuring max nodes before yield to %d", max_nodes_before_yield_count)
            self.mgid_table.attribute_pre_device_config_set(target,
                                                            max_nodes_before_yield=max_nodes_before_yield_count)
            logger.info("Configuring max node threshold node count to %d and port lag count to %d", max_node_threshold_node_count, max_node_threshold_node_port_lag_count)
            self.mgid_table.attribute_pre_device_config_set(target,
    ...
    def runTest(self):
        ...
        try:
            for port_index in range(len(swports)):
                ...
                logger.info("Setting port %d as the backup port for %d", backup_port, port)
                self.port_table.entry_add(
                    target,
                    [self.port_table.make_key([
                ...
                logger.info("Clearing backup port config for port %d", port)
                self.port_table.entry_mod(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Setting port %d as CopyToCPU port", port)
                self.port_table.entry_add(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Clearing port %d as CopyToCPU port", port)
                self.port_table.entry_mod(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Setting multicast forward state for port %d", port)
                self.port_table.entry_add(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Clearing multicast forward state for port %d", port)
                self.port_table.entry_mod(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Clearing multicast fast failover for port %d", port)
                self.port_table.entry_add(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Setting ALL multicast port config for port %d", port)
                self.port_table.entry_add(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
                ...
                logger.info("Clearing ALL multicast port config for port %d", port)
                self.port_table.entry_mod(
                    target,
                    [self.port_table.make_key([
                        client.KeyTuple('$DEV_PORT', port)])],
