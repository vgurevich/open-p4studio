...
def make_phase0_data(field1, field2, field3, field4):
    ...
    elif testutils.test_param_get("arch") == "tofino3":
        phase0data = phase0data << 64
    return phase0data
    ...
    def runTest(self):
        ...
        try:
            ...
            for i in range(num_entries):
                ...
                phase0data = 0
                while True:
                    igr_pipe = random.choice(pipes)
                    igr_local_port = random.randint(0, 63)

                    # For Tofino 3 only even ports exist
                    if testutils.test_param_get("arch") == "tofino3":
                        igr_local_port = int(igr_local_port / 2) * 2
            ...
            for key, value in list(phase0_igr_ports.items()):
                ...
                if recv_data != phase0data:
                    logger.info("Verifying %d entry for igr port %d", i, hex(igr_port))
                    logger.info("Exp data : %s : Rcv data : %s", hex(phase0data), hex(recv_data))
                    assert(0)
                i += 1
        finally:
        ...
        try:
            ...
            for data, key in resp:
                ...
                if recv_data != prog_data:
                    logger.info("Verifying %d entry for igr port %d", i, hex(igr_port))
                    logger.info("Exp data : %s : Rcv data : %s", hex(prog_data), hex(recv_data))
                    assert(0)
                i += 1
            # Verifying clean all
            self.port_metadata_table.entry_del(target)
