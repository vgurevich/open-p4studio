...
#include "../../s3/packet_utils.h"

#define MASK 0x000000ffu
#define PRINT_PACKET_INFO

DEFINE_HASHTABLE(dev_port_hash_table, 13);
DEFINE_HASHTABLE(intf_hash_table, 13);
DEFINE_HASHTABLE(bd_hash_table, 13);
DEFINE_HASHTABLE(rcode_hash_table, 13);

/* key should be untouched because it is more efficient due to the current key numbering */
#define BF_KNET_GET_HASH(key, hash_bits) (key & ((1 << hash_bits) -1))

/* CPU header size + Fabric header size - Ether type size (2)
 * Ether type is part of both CPU header and Fabric header */

#define BF_KNET_RX_FILETER_CPU_HEADER_OFFSET (sizeof(switch_ethernet_header_t) + \
                                              sizeof(switch_fabric_header_t) - 2)
...
__bf_knet_match_rx_pkt_hash(struct sk_buff *skb,
                            struct hlist_head *hash_table,
                            bf_knet_rx_filter_info_t *prev_match_filter,
                            uint8_t *data,
                            int lsize,
                            uint16_t key,
                            int hash_bits)
{
  ...
  bf_knet_rx_filter_spec_t *fspec = NULL;
  int idx;
  int match;

  hlist_for_each_entry_rcu(filter, &hash_table[BF_KNET_GET_HASH(key, hash_bits)], hash_bucket) {
    if (filter->rx_filter.spec.filter_size < lsize) {
      fspec = &filter->rx_filter.spec;
      if (prev_match_filter &&
          prev_match_filter->rx_filter.spec.priority <= fspec->priority) {
        return prev_match_filter;
      }
      ...
      for (idx = 0; idx < fspec->filter_size; idx++) {
        knet_debug(KERN_INFO, &skb->dev->dev,
                   "Byte[%d]: data:%u mask:%u, masked "
                    ...
                    fspec->filter[idx]);
        if ((data[idx] & fspec->mask[idx]) != fspec->filter[idx]) {
          knet_debug(KERN_INFO, &skb->dev->dev,
...
static inline bf_knet_rx_filter_info_t *
bf_knet_match_rx_pkt_hash(struct sk_buff *skb)
{
  switch_cpu_header_t *cpu_hdr;
  int lsize;
  uint8_t *data;
  bf_knet_rx_filter_info_t *match_filter;

  lsize = skb->len - skb->data_len;
  data = skb->data;

  cpu_hdr = (switch_cpu_header_t *)(data + BF_KNET_RX_FILETER_CPU_HEADER_OFFSET);

  match_filter = __bf_knet_match_rx_pkt_hash(skb, dev_port_hash_table, NULL,
                                             data, lsize, ntohs(cpu_hdr->ingress_port), HASH_BITS(dev_port_hash_table));
  match_filter = __bf_knet_match_rx_pkt_hash(skb, intf_hash_table, match_filter,
                                             data, lsize, ntohs(cpu_hdr->port_lag_index), HASH_BITS(intf_hash_table));
  match_filter = __bf_knet_match_rx_pkt_hash(skb, bd_hash_table, match_filter,
                                             data, lsize, ntohs(cpu_hdr->ingress_bd), HASH_BITS(bd_hash_table));
  match_filter = __bf_knet_match_rx_pkt_hash(skb, rcode_hash_table, match_filter,
                                             data, lsize, ntohs(cpu_hdr->reason_code), HASH_BITS(rcode_hash_table));
  return match_filter;
}

static void __bf_knet_rx_filter_add_hash(struct hlist_head *hash_table,
                                         bf_knet_rx_filter_info_t *new_filter,
                                         uint16_t key,
                                         int hash_bits)
{
  bf_knet_rx_filter_info_t *match_filter = NULL;
  bf_knet_rx_filter_info_t *filter = NULL;
  bf_knet_rx_filter_info_t *last_filter = NULL;
  bf_knet_rx_filter_spec_t *fspec = NULL;

  hlist_for_each_entry_rcu(filter, &hash_table[BF_KNET_GET_HASH(key, hash_bits)], hash_bucket) {
    last_filter = filter;
    fspec = &filter->rx_filter.spec;
    if (new_filter->rx_filter.spec.priority <  fspec->priority) {
      match_filter = filter;
      break;
    }
  }
  ...
  if (match_filter) {
    ...
    hlist_add_before_rcu(&new_filter->hash_bucket, &match_filter->hash_bucket);
  } else if (last_filter) {
    /*  no higher priority found */
    hlist_add_behind_rcu(&new_filter->hash_bucket, &last_filter->hash_bucket);
  } else {
    /* there was no filter */
    hlist_add_head_rcu(&new_filter->hash_bucket, &hash_table[BF_KNET_GET_HASH(key, hash_bits)]);
  }
}

static void bf_knet_rx_filter_add_hash(bf_knet_rx_filter_info_t *filter)
{
  switch_cpu_header_t *mask_cpu_hdr;
  switch_cpu_header_t *data_cpu_hdr;
  bf_knet_rx_filter_spec_t *fspec;

  fspec = &filter->rx_filter.spec;
  mask_cpu_hdr = (switch_cpu_header_t *)&fspec->mask[BF_KNET_RX_FILETER_CPU_HEADER_OFFSET];
  data_cpu_hdr = (switch_cpu_header_t *)&fspec->filter[BF_KNET_RX_FILETER_CPU_HEADER_OFFSET];

  if (mask_cpu_hdr->ingress_port) {
    __bf_knet_rx_filter_add_hash(dev_port_hash_table, filter, ntohs(data_cpu_hdr->ingress_port), HASH_BITS(dev_port_hash_table));
  } else if (mask_cpu_hdr->port_lag_index) {
    __bf_knet_rx_filter_add_hash(intf_hash_table, filter, ntohs(data_cpu_hdr->port_lag_index), HASH_BITS(intf_hash_table));
  } else if (mask_cpu_hdr->ingress_bd) {
    __bf_knet_rx_filter_add_hash(bd_hash_table, filter, ntohs(data_cpu_hdr->ingress_bd), HASH_BITS(bd_hash_table));
  } else if (mask_cpu_hdr->reason_code) {
    __bf_knet_rx_filter_add_hash(rcode_hash_table, filter, ntohs(data_cpu_hdr->reason_code), HASH_BITS(rcode_hash_table));
  }
}

static void bf_knet_rx_filter_delete_hash(bf_knet_rx_filter_info_t *filter)
{
  hash_del_rcu(&filter->hash_bucket);
}
		...
		struct packet_type *pt, struct net_device *dev2)
{
	...
	filter = bf_knet_match_rx_pkt_hash(skb);
	if (!filter) {
...
void bf_knet_cpuif_ndev_delete(bf_knet_msg_cpuif_ndev_delete_t *msg)
{
				 ...
				 &cpuif_info->bf_knet_rx_pf_list, list)
	{
		knet_debug(
		    ...
		    filter_info->rx_filter.spec.filter_id);
		bf_knet_rx_filter_delete_hash(filter_info);
...
void bf_knet_rx_filter_add(bf_knet_msg_rx_filter_add_t *msg)
{
	...
	bf_knet_rx_filter_add_hash(filter_info);
...
MODULE_DESCRIPTION("Intel KNET Packet Filtering");
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Intel");
