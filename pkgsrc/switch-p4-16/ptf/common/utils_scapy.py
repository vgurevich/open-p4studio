...
bind_layers(Ether, FabricHeader, type=0x9000)
...
def simple_cpu_packet(header_version=0,
    ...
    ether = Ether(dst=inner_pkt[Ether].dst, src=inner_pkt[Ether].src)
    ether.type = 0x9000
    ...
    fabric_payload_header = FabricPayloadHeader(ether_type=inner_pkt[Ether].type)
    ...
    if inner_pkt:
        inner_frame_bytes = bytearray(bytes(inner_pkt))
        pkt = pkt / inner_pkt[1:]
    else:
...
def packet_mask_do_not_care_header(mask, hdr_type, except_fields=None):
    except_fields = except_fields or []
    mask = copy.copy(mask)
    for f in hdr_type.fields_desc:
        if f.name not in except_fields:
            mask.set_do_not_care_scapy(hdr_type, f.name)
    return mask


def mask_set_do_not_care_packet(mask_obj, hdr_type, field_name):
    mask_obj.set_do_not_care_packet(hdr_type, field_name)
