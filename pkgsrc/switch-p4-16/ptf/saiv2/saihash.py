...
TEST_ECMP_OFFSET = 2  # test default ECMP offset value
TEST_ECMP_OFFSET1 = 4  # ecmp offset tests second value
TEST_LAG_OFFSET = 2  # test default LAG offset value
TEST_LAG_OFFSET1 = 4  # LAG offset tests second value
...
class SAIHashTestBase(SaiHelper):
    ...
    def setupFGECMPIPv4Hash(self, hash_fields_list=None, p_ipv4_mask=None):
        ...
        for hash_field in hash_fields_list:
            if hash_field in (SAI_NATIVE_HASH_FIELD_SRC_IP,
                              SAI_NATIVE_HASH_FIELD_DST_IP):
    ...
    def setupECMPOffset(self, offset=0):
        """
        Sets ECMP offset
        ...
        """
        sai_thrift_set_switch_attribute(
            self.client, ecmp_default_hash_offset=offset)

    def setupLagOffset(self, offset=0):
        """
        Sets Lag offset
        ...
        """
        sai_thrift_set_switch_attribute(
            self.client, lag_default_hash_offset=offset)
...
class SAIHashTest(SAIHashTestBase):
    ...
    def setUp(self):
        ...
        self.vlan200 = sai_thrift_create_vlan(
            self.client, vlan_id=200, learn_disable=True)
    ...
    def l3EcmpIPv4HashOffsetTest(self):
        """
        Verifies IPv4 ECMP with varing offset values
        """
        test_header("l3EcmpIPv4HashOffsetTest")
        ...
        try:
            ...
            self.setupECMPOffset(offset=TEST_ECMP_OFFSET)

            print("Verify load balancing equaly for all next_hops when"
                  "all packets fields changes")
            ecmp_count = self.l3IPv4EcmpPacketTest(hash_dict)
            print("ECMP count offset=%d:" % (TEST_ECMP_OFFSET), ecmp_count)
            equaly_balanced = verify_equaly_balanced(
                ecmp_count, expected_base=SEED_TEST_CHECK_BASE)
            self.assertTrue(
                equaly_balanced,
                "Ecmp paths are not equally balanced with offset=%d"
                % (TEST_ECMP_OFFSET))
            nbr_active_ports = verify_lb_active_ports(ecmp_count)
            self.assertTrue(nbr_active_ports == 4)

            # set the ecmp hash offset
            self.setupECMPOffset(offset=TEST_ECMP_OFFSET1)
            ecmp_count1 = self.l3IPv4EcmpPacketTest(hash_dict)
            print("ECMP count offset=%d:" % (TEST_ECMP_OFFSET1), ecmp_count1)
            equaly_balanced = verify_equaly_balanced(
                ecmp_count1, expected_base=SEED_TEST_CHECK_BASE)
            self.assertTrue(
                equaly_balanced,
                "Ecmp paths are not equally balanced with offset=%d"
                % (TEST_ECMP_OFFSET1))
            nbr_active_ports = verify_lb_active_ports(ecmp_count1)
            self.assertTrue(nbr_active_ports == 4)

            # todo vefiry if LB factor changed
            similary_balanced = verify_similary_balanced(
                ecmp_count, ecmp_count1)
            self.assertTrue(similary_balanced,
                            "ECMP offset change has no effect on LB")
    ...
    def l3LagIPv4HashOffsetTest(self):
        """
        Verifies basic L3 Lag configuration with varing offset values
        """
        ...
        try:
            ...
            self.setupLagOffset(offset=TEST_LAG_OFFSET)

            ecmp_count = self.l3IPv4LagPacketTest(hash_dict)

            print("ECMP count offset=%d:" % (TEST_LAG_OFFSET), ecmp_count)
            nbr_active_ports = verify_lb_active_ports(ecmp_count)
            self.assertTrue(nbr_active_ports == 3)
            equaly_balanced = verify_equaly_balanced(
                ecmp_count, expected_base=SEED_TEST_CHECK_BASE)
            ...
            self.setupLagOffset(offset=TEST_LAG_OFFSET1)
            print("Verify load balancing for IPv4 hash offset = %d"
                  % (TEST_LAG_OFFSET1))
            ecmp_count1 = self.l3IPv4LagPacketTest(hash_dict)
            print("ECMP count offset=%d:" % (TEST_LAG_OFFSET1), ecmp_count1)
            nbr_active_ports = verify_lb_active_ports(ecmp_count1)
            self.assertTrue(nbr_active_ports == 3)
            equaly_balanced = verify_equaly_balanced(
                ecmp_count1, expected_base=SEED_TEST_CHECK_BASE)
            ...
            similary_balanced = verify_similary_balanced(
                ecmp_count, ecmp_count1)
            self.assertTrue(similary_balanced,
                            "LAG Offset change has no effect on LB")
        finally:
            pass

    def ecmpIPv4HashRotateOffsetTest(self,
                                     hash_src_ip=None,
                                     hash_dst_ip=None,
                                     hash_udp_sport=None,
                                     hash_udp_dport=None):
        ...
        try:
            hash_fields = hash_to_hash_fields(hash_dict)
            print("Verify IPv4 ECMP rotate hash fields: %s"
                  % (hash_fields_to_hash_names(hash_fields)))
            ...
            status = sai_thrift_set_switch_attribute(
                self.client, ecmp_default_hash_offset=11)
            self.assertEqual(status, SAI_STATUS_SUCCESS)

            ecmp_count_1 = self.l3IPv4EcmpPacketTest(hash_dict)
            print("ECMP LB count:", ecmp_count_1)
        finally:
            pass
...
class SAIIPv6HashTest(SAIHashTestBase):
    ...
    def lagIPv6HashOffsetTest(self):
        """
        Verfies L3 IPv6 Lag basic case with varing offset values
        """
        ...
        try:
            ...
            print("Verify load balancing for IPv6 hash offset = %d"
                  % (TEST_LAG_OFFSET1))
            ecmp_count1 = self.lagIPv6PacketTest(hash_dict)
            print("ECMP count offset=%d:" % (TEST_LAG_OFFSET1), ecmp_count1)
            nbr_active_ports = verify_lb_active_ports(ecmp_count1)
            self.assertTrue(nbr_active_ports == 3)
            equaly_balanced = verify_equaly_balanced(
    ...
    def l3EcmpIPv6HashOffsetTest(self):
        """
        Verifies IPv6 ECMP with varing offset values
        """
        test_header("l3EcmpIPv6HashOffsetTest")
        ...
        try:
            ...
            # set the ecmp hash offset
            self.setupECMPOffset(offset=TEST_ECMP_OFFSET1)
            ecmp_count1 = self.l3IPv6EcmpPacketTest(hash_dict)
            print("ECMP count offset=%d:" % (TEST_ECMP_OFFSET1), ecmp_count1)
            equaly_balanced = verify_equaly_balanced(
                ecmp_count1, expected_base=SEED_TEST_CHECK_BASE)
            self.assertTrue(
    ...
    def ecmpIPv6HashRotateOffsetTest(self,
                                     hash_src_ip=None,
                                     hash_dst_ip=None,
                                     hash_udp_sport=None,
                                     hash_udp_dport=None):
        ...
        try:
            hash_fields = hash_to_hash_fields(hash_dict)
            print("Verify IPv6 ECMP rotate hash fields: %s"
                  % (hash_fields_to_hash_names(hash_fields)))
            ...
            ecmp_count_1 = self.l3IPv6EcmpPacketTest(hash_dict)
            print("ECMP LB count:", ecmp_count_1)
        finally:
            pass
...
class L3EcmpIPv4HashOffsetTest(SAIHashTest):
    """
    Verfies IPv4 ECMP offset case with varing values
    """

    def runTest(self):
        self.l3EcmpIPv4HashOffsetTest()
...
class L3LagIPv4HashOffsetTest(SAIHashTest):
    """
    Verfies L3 Lag basic case with varing seed values
    """

    def runTest(self):
        test_header("l3LagIPv4HashOffsetTest")
        self.l3LagIPv4HashOffsetTest()
...
class L3LagIPv6HashOffsetTest(SAIIPv6HashTest):
    """
    Verfies L3 IPv6 Lag basic case with varing offset values
    """

    def runTest(self):
        test_header("L3LagIPv6HashOffsetTest")
        self.lagIPv6HashOffsetTest()
...
class L3EcmpIPv6HashOffsetTest(SAIIPv6HashTest):
    """
    Verfies IPv6 ECMP offset case with varing values
    """

    def runTest(self):
        try:
            self.l3EcmpIPv6HashOffsetTest()
        finally:
            pass
...
@group('ecmp-rotatehash')
class EcmpIPv6RotateHashTest(SAIIPv6HashTest):
    """
    Verifies if IPv6 src hash is rotated by given offset and
    checks if packet distribuition is different before and after
    rotating hash.
    """
    def runTest(self):
        test_header("EcmpIPv6RotateHashTest")
        self.ecmpIPv6HashRotateOffsetTest(hash_src_ip=True)


@group('ecmp-rotatehash')
class EcmpIPv4RotateHashTest(SAIHashTest):
    """
    Verifies if IPv4 src hash is rotated by given offset and
    checks if packet distribuition is different before and after
    rotating hash.
    """
    def runTest(self):
        test_header("EcmpIPv4RotateHashTest")
        self.ecmpIPv4HashRotateOffsetTest(hash_src_ip=True)
