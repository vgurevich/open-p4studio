...
class MplsIpv6Test(SaiHelper):
    ...
    def setUp(self):
        ...
        sai_thrift_set_router_interface_attribute(
            self.client, self.ingress_rif, admin_mpls_state=True)
        ...
        self.mpls_counter = sai_thrift_create_counter(self.client)
        ...
        status = sai_thrift_create_inseg_entry(
            ...
            counter_id=self.mpls_counter)
        self.assertEqual(status, SAI_STATUS_SUCCESS)
        self.mpls1000_stats = 0
    ...
    def runTest(self):
        try:
            ...
            self.mplsCounterTest()
            self.mplsEgressLERTermUniformTest()
    ...
    def tearDown(self):
        ...
        sai_thrift_remove_counter(self.client, self.mpls_counter)
        ...
        sai_thrift_set_router_interface_attribute(
            self.client, self.ingress_rif, admin_mpls_state=False)
    ...
    def mplsCounterTest(self):
        '''
        Verify statistics of a counter attached to MPLS object
        Also verify getting counter_id of MPLS object and statistics clearing
        '''
        print("\nmplsCounterTest()")

        mpls_cntr = sai_thrift_get_inseg_entry_attribute(
            self.client, self.inseg_entry_1000, counter_id=True)
        self.assertEqual(mpls_cntr['counter_id'], self.mpls_counter)

        counter_stats = sai_thrift_get_counter_stats(
            self.client, self.mpls_counter)
        self.assertEqual(counter_stats['SAI_COUNTER_STAT_PACKETS'],
                         self.mpls1000_stats)
        self.assertNotEqual(counter_stats['SAI_COUNTER_STAT_BYTES'], 0)

        print("MPLS counter correct")

        sai_thrift_clear_counter_stats(self.client, self.mpls_counter)
        ...
        print("MPLS counter clear")

    def mplsEgressLERTermUniformTest(self):
        '''
        Verfies if MPLS label is popped in Egress LER and packet is forwarded
        based on IP lookup.
        '''
        print("\nmplsIngressLERTermUniformTest()")

        tc_val = 0
        orig_dscp_val = 18
        uniform_dscp_val = tc_val << 3
        ...
        send_pkt = simple_tcpv6_packet(
            ...
            ipv6_hlim=64)
        mpls_tag = {'label': 1000, 'ttl': 63, 'tc':  tc_val, 's':  1}
        mpls_tag_list = [mpls_tag]
        mpls_pkt = simple_mpls_packet(
            eth_dst=ROUTER_MAC,
            mpls_tags=mpls_tag_list,
        ...
        recv_pkt = simple_tcpv6_packet(
            ...
            eth_src=ROUTER_MAC,
            ipv6_dst='200::1',
            ipv6_dscp=uniform_dscp_val,
            ipv6_hlim=63)

        try:
            print("Set pop_qos_mode to UNIFORM")
            status = sai_thrift_set_inseg_entry_attribute(
                self.client,
                self.inseg_entry_1000,
            ...
            print("Set pop_qos_mode to PIPE")
            status = sai_thrift_set_inseg_entry_attribute(
                self.client,
                self.inseg_entry_1000,
                pop_qos_mode=SAI_INSEG_ENTRY_POP_QOS_MODE_PIPE)
    ...
    def mplsAdminStateTest(self):
        '''
        Verify SAI_ROUTER_INTERFACE_ATTR_ADMIN_MPLS_STATE setting during RIF
        creation and via set_attribute operation
        '''
        print("\nmplsAdminStateTest()")

        # both SAI_IN_DROP_REASON_L3_ANY and SAI_IN_DROP_REASON_IRIF_DISABLED
        # should be matched
        drop_reason = sai_thrift_s32_list_t(
            count=1, int32list=[SAI_IN_DROP_REASON_L3_ANY])
        l3_any_debug_cnt = sai_thrift_create_debug_counter(
            self.client,
        ...
        dc_attr = sai_thrift_get_debug_counter_attribute(
            self.client, l3_any_debug_cnt, index=True)
        l3_any_dc_index = dc_attr['index'] \
            + SAI_PORT_STAT_IN_DROP_REASON_RANGE_BASE

        drop_reason = sai_thrift_s32_list_t(
            count=1, int32list=[SAI_IN_DROP_REASON_IRIF_DISABLED])
        irif_disabled_debug_cnt = sai_thrift_create_debug_counter(
            self.client,
            type=SAI_DEBUG_COUNTER_TYPE_PORT_IN_DROP_REASONS,
            in_drop_reason_list=drop_reason)
        dc_attr = sai_thrift_get_debug_counter_attribute(
            self.client, irif_disabled_debug_cnt, index=True)
        irif_disabled_dc_index = dc_attr['index'] \
            + SAI_PORT_STAT_IN_DROP_REASON_RANGE_BASE
        ...
        try:
            ...
            print("Create another RIF with default MPLS state = Down")
            ingress_rif = sai_thrift_create_router_interface(
                self.client,
                type=SAI_ROUTER_INTERFACE_TYPE_PORT,
                port_id=self.port24,
                virtual_router_id=self.default_vrf)
            ...
            print("Send MPLS tag packet - drop")
            send_packet(self, self.dev_port24, mpls_pkt)
            verify_no_other_packets(self)

            print("Verify debug counters")
            for port, index in zip(
                    [self.port10, self.port24],
                    [l3_any_dc_index, irif_disabled_dc_index]):
                dc_stats = sai_thrift_get_debug_counter_port_stats(
                    self.client, port, [index])
                self.assertEqual(dc_stats[index], 1)

            print("Send MPLS packet with dst_mac != ROUTER_MAC")
            non_router_mac = "00:77:66:55:44:33:22"
            # mpls_pkt[Ether].eth_dst = non_router_mac
            mpls_pkt = simple_mpls_packet(
                eth_dst=non_router_mac,
                mpls_tags=mpls_tag_list,
                inner_frame=inner_pkt['IPv6'])
            send_packet(self, self.dev_port24, mpls_pkt)
            verify_no_other_packets(self)
            ...
            sai_thrift_remove_debug_counter(
                self.client, irif_disabled_debug_cnt)
            sai_thrift_remove_debug_counter(self.client, l3_any_debug_cnt)
            sai_thrift_remove_router_interface(self.client, ingress_rif)
...
class MplsIpv4Test(SaiHelper):
    ...
    def setUp(self):
        ...
        # Create Nexthop with IP=ipv4_3 PUSH label stack=[1000,2000,3000]
        self.nhop_label_3 = sai_thrift_create_next_hop(
            self.client,
        ...
        # Create Nexthop with IP=ipv4_3 and PUSH different label stack=[3333]
        self.nhop_label_3333 = sai_thrift_create_next_hop(
            self.client,
            ip=sai_ipaddress(ipv4_3),
            router_interface_id=self.egress_rif_1,
            type=SAI_NEXT_HOP_TYPE_MPLS,
        ...
        self.nhop_label_10 = sai_thrift_create_next_hop(
            self.client,
            ip=sai_ipaddress(ipv4_10),
            router_interface_id=self.egress_rif_9,
            type=SAI_NEXT_HOP_TYPE_MPLS,
    ...
    def tearDown(self):
        ...
        sai_thrift_remove_neighbor_entry(
            self.client, self.mpls_neighbor_entry_9)
        sai_thrift_remove_next_hop(self.client, self.nhop_label_9)
        sai_thrift_remove_next_hop(self.client, self.nhop_label_10)
        sai_thrift_remove_router_interface(self.client, self.egress_rif_9)
        ...
        sai_thrift_remove_next_hop(self.client, self.nhop_label_2)
        sai_thrift_remove_next_hop(self.client, self.nhop_label_3)
        sai_thrift_remove_next_hop(self.client, self.nhop_label_3333)
    ...
    def mplsIngressLERTest(self):
        ...
        mpls_tag_2 = {'label': 2000, 'ttl': 63, 'tc':  5, 's':  0}
        mpls_tag_3 = {'label': 3000, 'ttl': 63, 'tc':  5, 's':  0}
        mpls_tag_4 = {'label': 3333, 'ttl': 63, 'tc':  5, 's':  0}
        mpls_tag_1['s'] = 1
        mpls_tag_list.append(mpls_tag_1)
        mpls_pkt_label_1 = simple_mpls_packet(
            eth_src=ROUTER_MAC,
        ...
        del mpls_tag_list[:]
        mpls_tag_list.append(mpls_tag_4)
        ...
        try:
            ...
            sai_thrift_set_route_entry_attribute(
                self.client,
                self.mpls_encap_route_3,
                next_hop_id=self.nhop_label_3333)
            send_packet(self, self.dev_port10, send_pkt_3)
    ...
    def mplsEgressLERTermTest(self):
        ...
        recv_pkt = simple_tcp_packet(
            ...
            eth_src=ROUTER_MAC,
            ip_dst='20.20.20.1',
            ip_dscp=pipe_dscp_val,
            ip_ttl=63)
        recv_pkt_uniform = simple_tcp_packet(
            eth_dst='00:11:22:33:44:55',
            eth_src=ROUTER_MAC,
            ip_dst='20.20.20.1',
            ip_dscp=uniform_dscp_val,
            ip_ttl=63)
        ...
        try:
            ...
            print("Send MPLS tag packet with label 1000 - term and IP lookup")
            send_packet(self, self.dev_port10, mpls_pkt)
            verify_packet(self, recv_pkt_uniform, self.dev_port12)
            self.mpls_rif_in_stats += 1
            self.egress_rif_2_out_stats += 1
...
class MplsDropTrapTest(SaiHelper):
    ''' MPLS drop and trap test class '''
    def setUp(self):
        super(MplsDropTrapTest, self).setUp()
        ...
        # set ingress RIF MPLS admin state
        sai_thrift_set_router_interface_attribute(
            self.client, self.port10_rif, admin_mpls_state=True)
    ...
    def tearDown(self):
        ...
        sai_thrift_set_router_interface_attribute(
            self.client, self.port10_rif, admin_mpls_state=False)

        super(MplsDropTrapTest, self).tearDown()
    ...
    def mplsImplicitNullLabelTrapDropTest(self):
        ...
        try:
            ...
            print("Verifying MPLS debug counter")
            dc_stats = sai_thrift_get_debug_counter_port_stats(
                self.client, self.port10, [self.dc_index])
            self.assertEqual(dc_stats[self.dc_index], self.port10_disc_stats,
                             "MPLS debug counter value incorrect!")
