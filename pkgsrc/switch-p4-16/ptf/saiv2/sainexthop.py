...
class L3NexthopTest(SaiHelper):
    ...
    def customAttrTest(self):
        '''
            Test verifies nexthop custom attribute.
        '''
        print("customAttrTest")
        try:
            print("Creating nexthop with custom attribute")
            attr_value = sai_thrift_attribute_value_t(oid=self.port10_rif)
            attr = sai_thrift_attribute_t(id=(SAI_NEXT_HOP_ATTR_END + 0),
                                          value=attr_value)
            nhop = sai_thrift_create_next_hop(
                self.client, ip=sai_ipaddress('10.10.10.10'),
                router_interface_id=self.port10_rif,
                type=SAI_NEXT_HOP_TYPE_IP, custom_attribute=attr)

            print("Getting nexthop custom attribute")
            # print(sai_thrift_get_next_hop_attribute(
            #           self.client, nhop, custom_attribute=True))

            print("Setting nexthop custom attribute")
            attr_value = sai_thrift_attribute_value_t(oid=self.port11_rif)
            attr = sai_thrift_attribute_t(id=(SAI_NEXT_HOP_ATTR_END + 0),
                                          value=attr_value)
