...
class L3IPv4EcmpLpmTest(SaiHelper):
    ...
    # New test based on l3IPv4EcmpLpmTest
...
class L3IPv4EcmpLagTest(SaiHelper):
    ...
    # attribute marked as not implemented
...
class L3IPv6EcmpLagTest(SaiHelper):
    ...
    def l3IPv6EcmpHostTwoLagsDisabledLagMembersTest(self):
        ...
        try:
            ...
            # check LAG2 traffic
            self.assertTrue(count[3] == 0)  # LAG2 member 1 port7 disabled
        ...
        try:
            ...
            for i in range(0, MAX_ITRS):
                ...
                ports_to_verify = [
                    ...
                    self.dev_port7,  # LAG2 ports
                    self.dev_port8,
                    self.dev_port9,  # end of LAG2 ports
                    self.dev_port13
                ]
                rcv_idx = verify_any_packet_any_port(
...
@disabled
# Currently driver does not honor the order of members within the ecmp groups
class L3DynamicOrderedEcmpTest(SaiHelper):
    """
    ECMP tests for verifying DYNAMIC ORDERING of ECMP members
    """
    ordered_ecmp_enable = False

    def setUp(self):
        super(L3DynamicOrderedEcmpTest, self).setUp()

        enum_capability = sai_thrift_query_attribute_enum_values_capability(
            self.client,
            SAI_OBJECT_TYPE_NEXT_HOP_GROUP,
            SAI_NEXT_HOP_GROUP_ATTR_TYPE)
        if SAI_NEXT_HOP_GROUP_TYPE_DYNAMIC_UNORDERED_ECMP in enum_capability:
            self.ordered_ecmp_enable = True
        else:
            print("Skipping Test - Dynamic Unordered Ecmp is not supported.")
            return
        dmac1 = '00:11:22:33:44:55'
        dmac2 = '00:11:22:33:44:56'
        dmac3 = '00:11:22:33:44:57'
        ...
        self.nhop_group1 = sai_thrift_create_next_hop_group(
            self.client, type=SAI_NEXT_HOP_GROUP_TYPE_DYNAMIC_ORDERED_ECMP)
        self.nh_group_member1 = sai_thrift_create_next_hop_group_member(
            self.client,
            next_hop_group_id=self.nhop_group1,
            next_hop_id=self.nhop1)
    ...
    def tearDown(self):
        if self.ordered_ecmp_enable:
            sai_thrift_remove_neighbor_entry(
                self.client, self.neighbor_entry11)
            sai_thrift_remove_neighbor_entry(
                self.client, self.neighbor_entry12)
            sai_thrift_remove_neighbor_entry(
                self.client, self.neighbor_entry13)
            ...
            sai_thrift_remove_router_interface(self.client, self.port14_rif)
        super(L3DynamicOrderedEcmpTest, self).tearDown()
    ...
    def ecmpSequencingTest(self, priorities1, priorities2):
        ...
        Args:
            priorities1 (list):  List of sequence ids for ecmp members
            priorities2 (list): List of sequence ids for ecmp members
        """
        print("ECMP Dynamic Ordering Test for Priorties"
              "{} -> {}".format(priorities1, priorities2))

        try:
            print("Evaluating ECMP distrubition for Priorities"
                  "{}".format(priorities1))
            for mbr, priority in zip(self.ecmp_members, priorities1):
                status = sai_thrift_set_next_hop_group_member_attribute(
                    self.client, mbr, sequence_id=priority)
                self.assertEqual(status, SAI_STATUS_SUCCESS)
            ...
            self.assertTrue(dist_dict1 == dist_dict2,
                            "ECMP distribution {} with Priorities {}"
                            " and distribution {} with Priorities {}"
                            " are dissimilar".format(lb_count1, priorities1,
                                                     lb_count2, priorities2))
        finally:
            pass

    def hashDistributionTest(self):
        """
        Perform Traffic Distribution over ECMP members by sending
        traffic with varying dst ip and dst mac addresses

        Returns:
            List(int): ecmp member packet distribution counts
        """
...
class EcmpFGTest(SaiHelper):
    '''
    ECMP Fine Grain Test cases
    '''
    def setUp(self):
        super(EcmpFGTest, self).setUp()
        print('')
    ...
    def tearDown(self):
        ...
        sai_thrift_remove_next_hop(self.client, self.nhop3)
        sai_thrift_remove_router_interface(self.client, self.port14_rif)
        super(EcmpFGTest, self).tearDown()

    def runTest(self):
        self.ecmpFGBasicTest()
        self.ecmpFGAddDelMemTest()
        self.ecmpFGGrpMemAttrTest()
    ...
    def ecmpFGGrpMemAttrTest(self):
        ...
        try:
            ...
            sai_thrift_set_next_hop_group_member_attribute(
                self.client, self.nh_group_member2, next_hop_id=self.nhop1)
