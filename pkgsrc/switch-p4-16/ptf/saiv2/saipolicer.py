...
class BindPolicerToAclEntry(MinimalPortVlanConfig):
    ...
    def runTest(self):
        ...
        status = sai_thrift_set_policer_attribute(
            self.client, self.pol_id,
            green_packet_action=SAI_PACKET_ACTION_DROP,
            yellow_packet_action=SAI_PACKET_ACTION_DROP)
        self.assertEqual(status, SAI_STATUS_SUCCESS)

        print('\nSend traffic with all drop')
        pkt = simple_udp_packet(ip_dst='192.168.0.2')
        tx_cnt = 10
        for i in range(tx_cnt):
            print('Sending UDP packet', i)
        ...
        rx_cnt = count_matched_packets_all_ports(
            self, pkt, [self.dev_port1, self.dev_port2])
        print('Flooded packets:', rx_cnt)
        # green, yellow, red packet actions set to drop, all traffic dropped
        self.assertEqual(rx_cnt, 0)
        verify_no_other_packets(self)
        ...
        rx_cnt = count_matched_packets_all_ports(
            self, pkt, [self.dev_port1, self.dev_port2])
        print('Flooded packets:', rx_cnt)
        # green, yellow, red packet actions set to fwd, all traffic forwarded
        self.assertEqual(rx_cnt, tx_cnt * 2)
        verify_no_other_packets(self)
...
class PolicerTrafficTests(MinimalPortVlanConfig):
    ...
    def setUp(self):
        ...
        self.vlan_rif = sai_thrift_create_router_interface(
            self.client,
            type=SAI_ROUTER_INTERFACE_TYPE_VLAN,
            virtual_router_id=self.default_vrf,
            vlan_id=self.vlan)
        self.assertTrue(self.vlan_rif != 0)
    ...
    def tearDown(self):
        ...
        sai_thrift_remove_router_interface(self.client, self.vlan_rif)
...
class StormControlTests(MinimalPortVlanConfig):
    ...
    def setUp(self):
        ...
        self.bc_sc = sai_thrift_create_policer(
            self.client, meter_type=SAI_METER_TYPE_BYTES,
            mode=SAI_POLICER_MODE_STORM_CONTROL, cbs=250, cir=250)
    ...
    def tearDown(self):
        ...
        sai_thrift_clear_policer_stats(self.client, self.bc_sc)
        stats = sai_thrift_get_policer_stats(self.client, self.bc_sc)
        print("Statistics after broadcast storm control policer clear")
        print('SAI_POLICER_STAT_PACKETS:',
              stats['SAI_POLICER_STAT_PACKETS'])
        print('SAI_POLICER_STAT_YELLOW_PACKETS:',
              stats['SAI_POLICER_STAT_YELLOW_PACKETS'])
        ...
        sai_thrift_clear_policer_stats(self.client, self.mc_sc)
        stats = sai_thrift_get_policer_stats(self.client, self.mc_sc)
        print("Statistics after multicast storm control policer clear")
        print('SAI_POLICER_STAT_PACKETS:',
              stats['SAI_POLICER_STAT_PACKETS'])
        print('SAI_POLICER_STAT_YELLOW_PACKETS:',
              stats['SAI_POLICER_STAT_YELLOW_PACKETS'])
        ...
        stats = sai_thrift_clear_policer_stats(self.client, self.uc_sc)
        stats = sai_thrift_get_policer_stats(self.client, self.uc_sc)
        print("Statistics after flood storm control policer clear")
        print('SAI_POLICER_STAT_PACKETS:',
              stats['SAI_POLICER_STAT_PACKETS'])
        print('SAI_POLICER_STAT_YELLOW_PACKETS:',
              stats['SAI_POLICER_STAT_YELLOW_PACKETS'])
