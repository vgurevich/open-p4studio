...
class L3InterfaceTest(SaiHelper):
    ...
    def ipv4DisableTest(self):
        ...
        try:
            ...
            self.port10_rif_counter_out_octects += len(exp_pkt) + 4
    ...
    def macUpdateTest(self):
        ...
        try:
            ...
            self.port11_rif_counter_in += 1
            self.port10_rif_counter_out += 1
            self.port11_rif_counter_in_octects += len(pkt1) + 4
            self.port10_rif_counter_out_octects += len(exp_pkt1) + 4
    ...
    def ipv4FibTest(self):
        ...
        try:
            ...
            self.port11_rif_counter_in += 1
            self.port10_rif_counter_out += 1
            self.port11_rif_counter_in_octects += len(gre_pkt) + 4
            self.port10_rif_counter_out_octects += len(exp_gre_pkt) + 4

        finally:
            pass

    def ipv4ChecksumTest(self):
        """
        Verifies ipv4 checksum validation
        """
        print("\nipv4ChecksumTest()")

        pkt_rand = simple_tcp_packet(eth_dst=ROUTER_MAC,
                                     eth_src='00:22:22:22:22:22',
                                     ip_dst='11.11.11.1',
                                     ip_src='192.168.0.1',
                                     ip_id=105,
                                     ip_ttl=64)
        exp_pkt_rand = simple_tcp_packet(eth_dst='00:11:22:33:44:55',
                                         eth_src=ROUTER_MAC,
                                         ip_dst='11.11.11.1',
                                         ip_src='192.168.0.1',
                                         ip_id=105,
        ...
        pkt = simple_ip_packet(eth_dst=ROUTER_MAC,
                               ...
                               ip_tos=0,
                               ip_proto=0)
        exp_pkt = simple_ip_packet(eth_dst='00:11:22:33:44:55',
                                   eth_src=ROUTER_MAC,
                                   ip_dst='10.10.10.1',
                                   ip_src='192.168.0.1',
                                   ip_id=0xf4,
        ...
        try:
            ...
            # nothing change but ttl-1
            print("Sending packet port %d -> port %d (192.168.0.1 -> "
                  "11.11.11.0/24) LPM" % (self.dev_port11, self.dev_port10))
            send_packet(self, self.dev_port11, pkt_rand)
            verify_packet(self, exp_pkt_rand, self.dev_port10)
            self.port11_rif_counter_in += 1
            self.port10_rif_counter_out += 1
            self.port11_rif_counter_in_octects += len(pkt_rand) + 4
            self.port10_rif_counter_out_octects += len(exp_pkt_rand) + 4

            # GIVEN a random normal ip packet,
            # and manually modify checksum to 0xffff
            # WHEN send the packet to DUT
            # THEN DUT should drop it and add drop count
            pkt_rand.payload.chksum = 0xffff
            print("Sending packet port %d -> port %d (192.168.0.1 -> "
                  "11.11.11.0/24) LPM" % (self.dev_port11, self.dev_port10))
            send_packet(self, self.dev_port11, pkt_rand)
            ...
            # WHEN send the packet to DUT
            # THEN DUT should forward it as normal ip packet
            print("Sending packet with chsum=None - Forward\n"
                  "port %d -> port %d (192.168.0.1 -> "
                  "10.10.10.1)" % (self.dev_port11, self.dev_port10))
            send_packet(self, self.dev_port11, pkt)
            verify_packet(self, exp_pkt, self.dev_port10)
            self.port11_rif_counter_in += 1
            ...
            # THEN DUT should tolerant packet with 0xffff,
            # forward it as normal packet
            pkt.payload.chksum = 0xffff
            print("Sending packet with chsum=0xffff - Forward\n"
                  "port %d -> port %d (192.168.0.1 -> "
                  "10.10.10.1)" % (self.dev_port11, self.dev_port10))
            send_packet(self, self.dev_port11, pkt)
            verify_packet(self, exp_pkt, self.dev_port10)
            ...
            # the below represents forward_ip_packet_with_0xffff_chksum_drop
            # from sonic-mgmt test. This and above case are mutually exclusive.
            # So add it as comment in case smth changes or for debug purposes.
            ...
            #       "10.10.10.1)" % (self.dev_port11, self.dev_port10))
            # send_packet(self, self.dev_port11, pkt)
            # verify_no_other_packets(self, exp_pkt, self.dev_port10)
            # self.port11_rif_counter_in += 1
            # self.port11_rif_counter_in_octects += len(pkt) + 4

            # GIVEN a ip packet, after forwarded(ttl-1) by DUT,
            # it's checksum will be 0x0000 after recompute from scratch
            # WHEN send the packet to DUT
            # THEN DUT recompute new checksum as 0x0000
            # and forward packet as expected.
            pkt.payload.ttl = 230
            exp_pkt.payload.ttl = 229
    ...
    def rifStatsTest(self):
        ...
        self.assertTrue(self.lag1_rif_counter_out == lag1_rif_stats[
            'SAI_ROUTER_INTERFACE_STAT_OUT_PACKETS'])
        self.assertTrue(
            self.port11_rif_counter_in_octects == port11_rif_stats[
                'SAI_ROUTER_INTERFACE_STAT_IN_OCTETS'])
        self.assertTrue(
            self.port10_rif_counter_out_octects == port10_rif_stats[
    ...
    def customAttrTest(self):
        '''
            Test verifies router interface custom attribute.
        '''
        print("customAttrTest")
        try:
            print("Creating rif with custom attribute")
            ...
            print("Getting rif custom attribute")
            # print(sai_thrift_get_router_interface_attribute(
            #           self.client, rif, custom_attribute=True))

            print("Setting rif custom attribute")
...
@group('ig-bd-label')
class L3InterfaceIngressBdLabelTest(L3InterfaceTest):
    """
...
@group('eg-bd-label')
class L3InterfaceEgressBdLabelTest(L3InterfaceTest):
    """
...
@group('ig-bd-label')
class L3SubPortIngressBdLabelTest(L3SubPortTest):
    """
...
@group('eg-bd-label')
class L3SubPortEgressBdLabelTest(L3SubPortTest):
    """
...
class L3SviTest(SaiHelper):
    ...
    def runTest(self):
        try:
            ...
            # post routed flood not supported
...
class VirtualInterfaceTest(SaiHelper):
    """
    This class contains virtual router interface tests
    """
    def setUp(self):
        super(VirtualInterfaceTest, self).setUp()
        dmac1 = '00:11:22:33:44:55'
    ...
    def tearDown(self):
        ...
        sai_thrift_remove_next_hop(self.client, self.nhop1)
        sai_thrift_remove_neighbor_entry(self.client, self.neighbor_entry1)
        super(VirtualInterfaceTest, self).tearDown()

    def virtualInterfaceType(self):
        """
...
class EnumQueryCapability(SaiHelper):
    """
    ...
    def setUp(self):
        super(EnumQueryCapability, self).setUp()
    ...
    def runTest(self):
        try:
            self.rifTypePort()
            self.rifTypeVlan()
            self.rifTypeLoopback()
            self.rifTypeSubPort()
        finally:
            pass

    def rifTypePort(self):
        """
...
class SVIRifVlanTest(SaiHelper):
    """
    This class contains virtual router interface tests for SVIs
    """
    def setUp(self):
        super(SVIRifVlanTest, self).setUp()
        self.port24_bp = sai_thrift_create_bridge_port(
            self.client,
            bridge_id=self.default_1q_bridge,
            port_id=self.port24,
    ...
    def tearDown(self):
        ...
        super(SVIRifVlanTest, self).tearDown()

    def sviRifIpv4Test(self):
        """
        Verifies if packet is forwarded correctly before
