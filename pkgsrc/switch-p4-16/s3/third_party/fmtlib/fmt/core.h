...
class container_buffer : public basic_buffer<typename Container::value_type> {
 ...
 protected:
  void grow(std::size_t capacity_local) FMT_OVERRIDE {
    container_.resize(capacity_local);
    this->set(&container_[0], capacity_local);
  }
...
class context_base {
 ...
 protected:
  ...
  context_base(OutputIt out_local, basic_string_view<char_type> format_str,
               basic_format_args<Context> ctx_args,
               locale_ref loc = locale_ref())
  : parse_context_(format_str), out_(out_local), args_(ctx_args), loc_(loc) {}

  // Returns the argument with specified index.
  format_arg do_get_arg(unsigned arg_id) {
    format_arg arg_local = args_.get(arg_id);
    if (!arg_local)
...
class basic_format_context :
 ...
 public:
   ...
   */
  basic_format_context(OutputIt out_local, basic_string_view<char_type> format_str,
                       basic_format_args<basic_format_context> ctx_args,
                       internal::locale_ref loc = internal::locale_ref())
    : base(out_local, format_str, ctx_args, loc) {}
