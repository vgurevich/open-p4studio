...
# Parameters
my $sai_dir = catdir( dirname($script_dir), 'submodules', 'SAI' );
my $ptf_dir = catdir( dirname($script_dir), 'ptf',        'saiv2' );

#<<<
( my $args, my $usage ) = describe_options(
    "$PROGRAM_NAME %o",
    [ 'sai|s=s',         "SAI repo path, default is $sai_dir",                                                                    { default => $sai_dir } ],
    [ 'meta-build!',     'Build SAI meta, enabled by default, use --no-meta-build to disable',                                    { default => 1 }        ],
    [ 'ptf|p=s',         "PTF path, default is $ptf_dir",                                                                         { default => $ptf_dir } ],
    ...
    [ 'format!',         'Format source code after generation (enabled by default), use --no-format to disable. See also: --python-format',               ],
    [ 'python-format!',  'Use external tools to format the Python code with after generation, defaults to --[no-]format if specified, disabled otherwise' ],
...
my $clean         = $args->clean;
my $format        = $args->format // 1;
my $python_format = $args->python_format // $args->format // 0;
$ptf_dir = $args->ptf;
