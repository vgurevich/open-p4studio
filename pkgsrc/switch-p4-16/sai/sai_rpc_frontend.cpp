...
void convert_attr_thrift_to_sai(const sai_object_type_t sai_ot,
  ...
  switch (attr_md->attrvaluetype) {
    ...
    case SAI_ATTR_VALUE_TYPE_VLAN_LIST: {
      sai_attr->value.vlanlist.list = (sai_vlan_id_t *)malloc(
          sizeof(sai_vlan_id_t) * thrift_attr.value.vlanlist.count);
      int i = 0;
      for (auto vlan : thrift_attr.value.vlanlist.idlist) {
        sai_attr->value.vlanlist.list[i++] = vlan;
      }
      sai_attr->value.vlanlist.count = thrift_attr.value.vlanlist.count;
    } break;
    ...
    case SAI_ATTR_VALUE_TYPE_SEGMENT_LIST: {
      sai_attr->value.segmentlist.count = thrift_attr.value.segmentlist.count;
      sai_attr->value.segmentlist.list = (sai_ip6_t *)malloc(
          sizeof(sai_ip6_t) * thrift_attr.value.segmentlist.count);
      int i = 0;
      for (auto thrift_ip6 : thrift_attr.value.segmentlist.ip6list) {
        sai_thrift_ip6_t_parse(thrift_attr.value.segmentlist.ip6list[i],
                               sai_attr->value.segmentlist.list[i]);
        i++;
      }
    } break;
...
void sai_thrift_l2mc_entry_type_t_parse(
    const sai_thrift_l2mc_entry_type_t &thrift_l2mc_entry_type,
    sai_l2mc_entry_type_t *l2mc_entry_type) {
  *l2mc_entry_type = (sai_l2mc_entry_type_t)thrift_l2mc_entry_type;
}

void sai_thrift_ipmc_entry_type_t_parse(
    const sai_thrift_ipmc_entry_type_t &thrift_ipmc_entry_type,
    sai_ipmc_entry_type_t *ipmc_entry_type) {
  *ipmc_entry_type = (sai_ipmc_entry_type_t)thrift_ipmc_entry_type;
}
...
void convert_attr_sai_to_thrift(const sai_object_type_t sai_ot,
  ...
  switch (attr_md->attrvaluetype) {
    ...
    case SAI_ATTR_VALUE_TYPE_MAP_LIST:
    case SAI_ATTR_VALUE_TYPE_VLAN_LIST: {
      for (unsigned int i = 0; i < sai_attr.value.vlanlist.count; i++) {
        thrift_attr.value.vlanlist.idlist.push_back(
            sai_attr.value.vlanlist.list[i]);
      }
      thrift_attr.value.vlanlist.count = sai_attr.value.vlanlist.count;
      free(sai_attr.value.vlanlist.list);
    } break;
    case SAI_ATTR_VALUE_TYPE_QOS_MAP_LIST: {
    ...
    case SAI_ATTR_VALUE_TYPE_SEGMENT_LIST: {
      for (unsigned int i = 0; i < sai_attr.value.segmentlist.count; i++) {
        sai_thrift_ip6_t thrift_ip6;
        thrift_ip6 = sai_ip6_t_to_thrift(sai_attr.value.segmentlist.list[i]);
        thrift_attr.value.segmentlist.ip6list.push_back(thrift_ip6);
      }
      thrift_attr.value.segmentlist.count = sai_attr.value.segmentlist.count;
      free(sai_attr.value.segmentlist.list);
    } break;
    case SAI_ATTR_VALUE_TYPE_TLV_LIST:
    case SAI_ATTR_VALUE_TYPE_PORT_EYE_VALUES_LIST:
    case SAI_ATTR_VALUE_TYPE_TIMESPEC:
