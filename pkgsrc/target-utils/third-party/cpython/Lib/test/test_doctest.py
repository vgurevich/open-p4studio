...
Check doctest with a non-ascii filename:
    ...
    ...
    >>> parser = doctest.DocTestParser()
    >>> test = parser.get_doctest(doc, {}, "foo-b?r@baz", "foo-b?r@baz.py", 0)
    >>> test
    <DocTest foo-b?r@baz from foo-b?r@baz.py:0 (1 example)>
    >>> runner = doctest.DocTestRunner(verbose=False)
    >>> runner.run(test) # doctest: +ELLIPSIS
    **********************************************************************
    File "foo-b?r@baz.py", line 2, in foo-b?r@baz
    Failed example:
        raise Exception('cl?')
    Exception raised:
        Traceback (most recent call last):
          ...
          File "<doctest foo-b?r@baz[0]>", line 1, in <module>
            raise Exception('cl?')
        Exception: cl?
    TestResults(failed=1, attempted=1)
    """
