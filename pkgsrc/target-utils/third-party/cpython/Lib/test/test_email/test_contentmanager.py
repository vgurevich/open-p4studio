...
class TestRawDataManager(TestEmailBase):
    ...
    def test_get_text_plain_latin1(self):
        m = self._bytes_msg(textwrap.dedent("""\
            ...
            Bas?c t?xt.
            """).encode('latin1'))
        self.assertEqual(raw_data_manager.get_content(m), "Bas?c t?xt.\n")
    ...
    def test_set_text_short_line_minimal_non_ascii_heuristics(self):
        m = self._make_message()
        content = "et l? il est mont? sur moi et il commence ? m'?to.\n"
        raw_data_manager.set_content(m, content)
    ...
    def test_set_text_long_line_minimal_non_ascii_heuristics(self):
        m = self._make_message()
        content = ("j'ai un probl?me de python. il est sorti de son"
                   " vivarium.  et l? il est mont? sur moi et il commence"
                   " ? m'?to.\n")
        raw_data_manager.set_content(m, content)
        self.assertEqual(bytes(m), textwrap.dedent("""\
            Content-Type: text/plain; charset="utf-8"
    ...
    def test_set_text_11_lines_long_line_minimal_non_ascii_heuristics(self):
        m = self._make_message()
        content = '\n'*10 + (
                  "j'ai un probl?me de python. il est sorti de son"
                  " vivarium.  et l? il est mont? sur moi et il commence"
                  " ? m'?to.\n")
        raw_data_manager.set_content(m, content)
        self.assertEqual(bytes(m), textwrap.dedent("""\
    ...
    def test_set_message_with_non_ascii_and_coercion_to_7bit(self):
        ...
        content.set_content("j'ai un probl?me de python. il est sorti de son"
                            " vivarium.")
        raw_data_manager.set_content(m, content)
        self.assertEqual(bytes(m), textwrap.dedent("""\
            ...
            j'ai un probl?me de python. il est sorti de son vivarium.
            """).encode('utf-8'))
    ...
    def test_set_non_ascii_filename(self):
        m = self._make_message()
        m.set_content('foo', filename='?b?r?.txt')
        self.assertEqual(bytes(m), textwrap.dedent("""\
            Content-Type: text/plain; charset="utf-8"
            Content-Transfer-Encoding: 7bit
    ...
    def content_object_as_non_ascii_filename_receiver(self, obj, mimetype):
        m = self._make_message()
        m.set_content(obj, *mimetype, disposition='inline', filename='b?r.txt')
        self.assertEqual(m['Content-Disposition'], 'inline; filename="b?r.txt"')
        self.assertEqual(m.get_filename(), "b?r.txt")
        self.assertEqual(m['Content-Disposition'].params['filename'], "b?r.txt")
