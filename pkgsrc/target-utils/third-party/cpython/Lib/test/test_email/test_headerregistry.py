...
class TestContentDisposition(TestHeaderBase):
    ...
    content_disp_params = {
        ...
        'invalid_parameter_value_with_fws_between_ew': (
            ...
            '               =?UTF-8?Q?pdf?="',
            'attachment',
            {'filename': 'Schulbesuchsbest?ttigung.pdf'},
            [errors.InvalidHeaderDefect]*3,
            ('attachment; filename="Schulbesuchsbest?ttigung.pdf"'),
...
class TestAddressHeader(TestHeaderBase):
    ...
    example_params = {
        ...
        'rfc2047_atom_in_phrase_is_decoded':
            ...
            [],
            '"The ?ric, Himself" <foo@example.com>',
            'The ?ric, Himself',
            'foo@example.com',
            'foo',
            'example.com',
...
class TestAddressAndGroup(TestEmailBase):
    ...
    def test_il8n(self):
        a = Address('?ric', 'wok', 'ex?mple.com')
        self.assertEqual(a.display_name, '?ric')
        self.assertEqual(a.username, 'wok')
        self.assertEqual(a.domain, 'ex?mple.com')
        self.assertEqual(a.addr_spec, 'wok@ex?mple.com')
        self.assertEqual(str(a), '?ric <wok@ex?mple.com>')

    # XXX: there is an API design issue that needs to be solved here.
    #def test_non_ascii_username_raises(self):
    #    with self.assertRaises(ValueError):
    #        Address('foo', 'w?k', 'example.com')
    ...
    def test_non_ascii_username_in_addr_spec_raises(self):
        with self.assertRaises(ValueError):
            Address('foo', addr_spec='w?k@example.com')
...
class TestFolding(TestHeaderBase):
    ...
    def test_address_display_names(self):
        ...
        for name, result in (
                ...
                ('Foo Bar (France)', '"Foo Bar (France)"'),
                ('Foo Bar, Espa?a', 'Foo =?utf-8?q?Bar=2C_Espa=C3=B1a?='),
                ('Foo Bar (Espa?a)', 'Foo Bar =?utf-8?b?KEVzcGHDsWEp?='),
                ('Foo, Bar Espa?a', '=?utf-8?q?Foo=2C_Bar_Espa=C3=B1a?='),
                ('Foo, Bar [Espa?a]', '=?utf-8?q?Foo=2C_Bar_=5BEspa=C3=B1a=5D?='),
                ('Foo B?r, France', 'Foo =?utf-8?q?B=C3=A4r=2C?= France'),
                ('Foo B?r <France>', 'Foo =?utf-8?q?B=C3=A4r_=3CFrance=3E?='),
                (
                    'L?rem ipsum d?l?r sit amet, c?nsectetuer adipiscing. '
                    'Suspendisse p?tenti. Aliquam nibh. Suspendisse p?tenti.',
    ...
    def test_message_id_header_is_not_folded(self):
        ...
        h = self.make_header('Message-ID', '<????@w?k.com>')
        self.assertEqual(
        ...
        h = self.make_header('Message-ID', '<????fromMessage@w?k.com>')
        self.assertEqual(
            h.fold(policy=policy.default.clone(max_line_length=20)),
            'Message-ID:\n <????fromMessage@w?k.com>\n')
