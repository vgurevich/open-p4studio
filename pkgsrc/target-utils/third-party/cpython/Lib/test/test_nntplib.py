    ...
    def handle_XOVER(self, message_spec):
        if message_spec == "57-59":
            self.push_lit(
                ...
                "59\tRe: Message d'erreur incompr?hensible (par moi)"
...
class NNTPv1v2TestsMixin:
    ...
    def _check_article_body(self, lines):
        ...
        self.assertEqual(lines[-1].decode('utf-8'), "-- Signed by Andr?.")
    ...
    def check_over_xover_resp(self, resp, overviews):
        ...
        self.assertEqual(over["subject"],
                         "Re: Message d'erreur incompr?hensible (par moi)")
...
class MiscTests(unittest.TestCase):
    ...
    def test_decode_header(self):
        ...
        gives("=?ISO-8859-15?Q?D=E9buter_en_Python?=", "D?buter en Python")
        gives("=?utf-8?q?Re=3A_=5Bsqlite=5D_probl=C3=A8me_avec_ORDER_BY_sur_des_cha?="
              ...
              "Re: [sqlite] probl?me avec ORDER BY sur des cha?nes de caract?res accentu?es")
        gives("Re: =?UTF-8?B?cHJvYmzDqG1lIGRlIG1hdHJpY2U=?=",
              "Re: probl?me de matrice")
        # A natively utf-8 header (found in the real world!)
        gives("Re: Message d'erreur incompr?hensible (par moi)",
              "Re: Message d'erreur incompr?hensible (par moi)")
