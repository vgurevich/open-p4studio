...
class DefaultArgumentsTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.msg['From'] = 'P?olo <f?o@bar.com>'
...
class SMTPSimTests(unittest.TestCase):
    ...
    def test_send_unicode_without_SMTPUTF8(self):
        smtp = smtplib.SMTP(
            ...
            timeout=support.LOOPBACK_TIMEOUT)
        self.addCleanup(smtp.close)
        self.assertRaises(UnicodeEncodeError, smtp.sendmail, 'Alice', 'B?b', '')
        self.assertRaises(UnicodeEncodeError, smtp.mail, '?lice')
    ...
    def test_name_field_not_included_in_envelop_addresses(self):
        ...
        message['From'] = email.utils.formataddr(('Micha?l', 'michael@example.com'))
        message['To'] = email.utils.formataddr(('Ren?', 'rene@example.com'))
...
class SMTPUTF8SimTests(unittest.TestCase):
    ...
    def test_send_unicode_with_SMTPUTF8_via_sendmail(self):
        ...
        smtp = smtplib.SMTP(
            ...
            timeout=support.LOOPBACK_TIMEOUT)
        self.addCleanup(smtp.close)
        smtp.sendmail('J?hn', 'S?lly', m,
                      mail_options=['BODY=8BITMIME', 'SMTPUTF8'])
        self.assertEqual(self.serv.last_mailfrom, 'J?hn')
        self.assertEqual(self.serv.last_rcpttos, ['S?lly'])
    ...
    def test_send_unicode_with_SMTPUTF8_via_low_level_API(self):
        ...
        smtp.ehlo()
        self.assertEqual(
            smtp.mail('J?', options=['BODY=8BITMIME', 'SMTPUTF8']),
            (250, b'OK'))
        self.assertEqual(smtp.rcpt('J?nos'), (250, b'OK'))
        self.assertEqual(smtp.data(m), (250, b'OK'))
        self.assertEqual(self.serv.last_mailfrom, 'J?')
        self.assertEqual(self.serv.last_rcpttos, ['J?nos'])
        self.assertEqual(self.serv.last_message, m)
        self.assertIn('BODY=8BITMIME', self.serv.last_mail_options)
        self.assertIn('SMTPUTF8', self.serv.last_mail_options)
        self.assertEqual(self.serv.last_rcpt_options, [])
    ...
    def test_send_message_uses_smtputf8_if_addrs_non_ascii(self):
        ...
        msg.set_content("oh l? l?, know what I mean, know what I mean?\n\n")
        # XXX smtpd converts received /r/n to /n, so we can't easily test that
        # we are successfully sending /r/n :(.
        expected = textwrap.dedent("""\
            From: P?olo <f?o@bar.com>
            To: Dinsdale
        ...
        self.assertEqual(self.serv.last_mailfrom, 'f?o@bar.com')
