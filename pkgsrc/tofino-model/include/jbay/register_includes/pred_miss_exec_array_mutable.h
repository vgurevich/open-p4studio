...
#ifndef __REGISTER_INCLUDES_PRED_MISS_EXEC_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_PRED_MISS_EXEC_ARRAY_MUTABLE_H__
...
class PredMissExecArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  PredMissExecArrayMutable(
    ...
    : RegisterBlock(chipNumber, StartOffset(index_pipe_addrmap, index_mau_addrmap), 4 * ArraySize(index_pipe_addrmap, index_mau_addrmap, -1), true, write_callback, read_callback, std::string("PredMissExecArrayMutable")+":"+boost::lexical_cast<std::string>(index_pipe_addrmap) + "," + boost::lexical_cast<std::string>(index_mau_addrmap)),
...
public:
  uint16_t pred_miss_glob_exec(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].pred_miss_glob_exec();
  }
  void pred_miss_glob_exec(uint32_t a0,const uint16_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].pred_miss_glob_exec(v);
  }
  uint16_t pred_miss_loca_exec(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].pred_miss_loca_exec();
  }
  void pred_miss_loca_exec(uint32_t a0,const uint16_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].pred_miss_loca_exec(v);
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("PredMissExecArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<PredMissExecMutable> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_PRED_MISS_EXEC_ARRAY_MUTABLE_H__
