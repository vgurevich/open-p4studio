...
#ifndef __REGISTER_INCLUDES_CBUS_PARITY_ERR_LOG_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_CBUS_PARITY_ERR_LOG_ARRAY_MUTABLE_H__
...
class CbusParityErrLogArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  CbusParityErrLogArrayMutable(
      int chipNumber, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(), 4 * ArraySize(-1), true, write_callback, read_callback, std::string("CbusParityErrLogArrayMutable")),
...
public:
  ...
  uint32_t cbus(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].cbus();
  }
  void cbus(uint32_t a0,const uint32_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].cbus(v);
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("CbusParityErrLogArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<CbusParityErrLogMutable> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_CBUS_PARITY_ERR_LOG_ARRAY_MUTABLE_H__
