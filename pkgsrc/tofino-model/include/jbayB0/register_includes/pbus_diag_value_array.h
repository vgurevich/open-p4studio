...
#ifndef __REGISTER_INCLUDES_PBUS_DIAG_VALUE_ARRAY_H__
#define __REGISTER_INCLUDES_PBUS_DIAG_VALUE_ARRAY_H__
...
class PbusDiagValueArray : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  PbusDiagValueArray(
      int chipNumber, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(), 4 * ArraySize(-1), false, write_callback, read_callback, std::string("PbusDiagValueArray")),
...
public:
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("PbusDiagValueArray") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<PbusDiagValue> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_PBUS_DIAG_VALUE_ARRAY_H__
