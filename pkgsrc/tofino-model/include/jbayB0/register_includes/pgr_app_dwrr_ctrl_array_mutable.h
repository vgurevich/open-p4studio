...
#ifndef __REGISTER_INCLUDES_PGR_APP_DWRR_CTRL_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_PGR_APP_DWRR_CTRL_ARRAY_MUTABLE_H__
...
class PgrAppDwrrCtrlArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  PgrAppDwrrCtrlArrayMutable(
      int chipNumber, int index_pipe_addrmap, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(index_pipe_addrmap), 4 * ArraySize(index_pipe_addrmap, -1), true, write_callback, read_callback, std::string("PgrAppDwrrCtrlArrayMutable")+":"+boost::lexical_cast<std::string>(index_pipe_addrmap)),
...
public:
  ...
  uint16_t max(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].max();
  }
  void max(uint32_t a0,const uint16_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].max(v);
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("PgrAppDwrrCtrlArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<PgrAppDwrrCtrlMutable> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_PGR_APP_DWRR_CTRL_ARRAY_MUTABLE_H__
