...
#ifndef __REGISTER_INCLUDES_PGR_PGEN_PORT_DOWN_LOG_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_PGR_PGEN_PORT_DOWN_LOG_ARRAY_MUTABLE_H__
...
class PgrPgenPortDownLogArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  PgrPgenPortDownLogArrayMutable(
      int chipNumber, int index_pipe_addrmap, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(index_pipe_addrmap), 12 * ArraySize(index_pipe_addrmap, -1), true, write_callback, read_callback, std::string("PgrPgenPortDownLogArrayMutable")+":"+boost::lexical_cast<std::string>(index_pipe_addrmap)),
...
public:
  ...
  BitVector<72> &port_down_vec(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].port_down_vec();
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("PgrPgenPortDownLogArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<PgrPgenPortDownLogMutable> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_PGR_PGEN_PORT_DOWN_LOG_ARRAY_MUTABLE_H__
