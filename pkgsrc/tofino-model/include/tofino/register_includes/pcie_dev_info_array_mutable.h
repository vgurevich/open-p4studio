...
#ifndef __REGISTER_INCLUDES_PCIE_DEV_INFO_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_PCIE_DEV_INFO_ARRAY_MUTABLE_H__
...
class PcieDevInfoArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  PcieDevInfoArrayMutable(
      int chipNumber, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(), 4 * ArraySize(-1), true, write_callback, read_callback, std::string("PcieDevInfoArrayMutable")),
...
public:
  ...
  uint32_t dev_info(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].dev_info();
  }
  void dev_info(uint32_t a0,const uint32_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].dev_info(v);
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("PcieDevInfoArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<PcieDevInfoMutable> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_PCIE_DEV_INFO_ARRAY_MUTABLE_H__
