/*******************************************************************************
 *  Copyright (C) 2024 Intel Corporation
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *  http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing,
 *  software distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions
 *  and limitations under the License.
 *
 *
 *  SPDX-License-Identifier: Apache-2.0
 ******************************************************************************/

// generated by split_classes.pl
#ifndef __REGISTER_INCLUDES_QPEX_PIPE_CTRL_H__
#define __REGISTER_INCLUDES_QPEX_PIPE_CTRL_H__


#include <cstdint>
#include <cassert>
#include <array>
#include <vector>
#include <string>
#include <boost/lexical_cast.hpp>
#include <boost/format.hpp>
#include <model_core/register_block.h>
#include <shared/bitvector.h>












namespace tofino {
  namespace register_classes {

class QpexPipeCtrl : public model_core::RegisterBlock<RegisterCallback> {
public:
  QpexPipeCtrl(
      int chipNumber, int index_tm_pex_pipe_rspec, RegisterCallback& write_callback = 0, RegisterCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(index_tm_pex_pipe_rspec), 4, false, write_callback, read_callback, std::string("QpexPipeCtrl")+":"+boost::lexical_cast<std::string>(index_tm_pex_pipe_rspec))
    {
    }
  QpexPipeCtrl(
      
  )
    : RegisterBlock(0, 0, 0, false, 0, 0, "QpexPipeCtrl")
    {
    }
public:





  uint8_t &cnt_port() { return cnt_port_; }





  uint8_t &cnt_port_en() { return cnt_port_en_; }





  uint8_t &cnt_en() { return cnt_en_; }





  uint8_t &pack_sop_dis() { return pack_sop_dis_; }






  uint8_t &ct_sop_wait() { return ct_sop_wait_; }





  uint8_t &freeze_on_error() { return freeze_on_error_; }





  uint8_t &ecc_dis() { return ecc_dis_; }

  bool read(
      uint32_t offset, uint32_t* data
      ) const {
    if (read_callback_) read_callback_();
    *data = (cnt_port_ & 0x7f);
    *data |= ((cnt_port_en_ & 0x1) << 7);
    *data |= ((cnt_en_ & 0x1) << 8);
    *data |= ((pack_sop_dis_ & 0x1) << 9);
    *data |= ((ct_sop_wait_ & 0x1f) << 12);
    *data |= ((freeze_on_error_ & 0x1) << 30);
    *data |= ((ecc_dis_ & 0x1) << 31);
    return true;
  }


  bool write(
      uint32_t offset, uint32_t data
      ) {
    cnt_port_ = (data & 0x7f);
    cnt_port_en_ = ((data >> 7) & 0x1);
    cnt_en_ = ((data >> 8) & 0x1);
    pack_sop_dis_ = ((data >> 9) & 0x1);
    ct_sop_wait_ = ((data >> 12) & 0x1f);
    freeze_on_error_ = ((data >> 30) & 0x1);
    ecc_dis_ = ((data >> 31) & 0x1);
    if (write_callback_) write_callback_();
    return true;
  }

  void reset(
      
      ) {
    cnt_port_ = 0x0;
    cnt_port_en_ = 0x0;
    cnt_en_ = 0x0;
    pack_sop_dis_ = 0x0;
    ct_sop_wait_ = 0xa;
    freeze_on_error_ = 0x0;
    ecc_dis_ = 0x0;
    if (write_callback_) write_callback_();
  }

  std::string to_string(
      uint32_t offset, bool print_zeros = false, std::string indent_string = ""
      ) const {
    std::string r("");
    bool all_zeros=true;
    r += indent_string + std::string("QpexPipeCtrl") + ":\n";
    r += indent_string + "  " + std::string("cnt_port") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(cnt_port_) ) + "\n";
    all_zeros &= (0 == cnt_port_);
    r += indent_string + "  " + std::string("cnt_port_en") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(cnt_port_en_) ) + "\n";
    all_zeros &= (0 == cnt_port_en_);
    r += indent_string + "  " + std::string("cnt_en") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(cnt_en_) ) + "\n";
    all_zeros &= (0 == cnt_en_);
    r += indent_string + "  " + std::string("pack_sop_dis") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(pack_sop_dis_) ) + "\n";
    all_zeros &= (0 == pack_sop_dis_);
    r += indent_string + "  " + std::string("ct_sop_wait") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(ct_sop_wait_) ) + "\n";
    all_zeros &= (0 == ct_sop_wait_);
    r += indent_string + "  " + std::string("freeze_on_error") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(freeze_on_error_) ) + "\n";
    all_zeros &= (0 == freeze_on_error_);
    r += indent_string + "  " + std::string("ecc_dis") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(ecc_dis_) ) + "\n";
    all_zeros &= (0 == ecc_dis_);
    if (all_zeros && !print_zeros) {
      return("");
    }
    else {
      return r;
    }
  }

  std::string to_string(
      bool print_zeros = false, std::string indent_string = ""
      ) const {
    std::string r("");
    bool all_zeros=true;
    r += indent_string + std::string("QpexPipeCtrl") + ":\n";
    r += indent_string + "  " + std::string("cnt_port") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(cnt_port_) ) + "\n";
    all_zeros &= (0 == cnt_port_);
    r += indent_string + "  " + std::string("cnt_port_en") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(cnt_port_en_) ) + "\n";
    all_zeros &= (0 == cnt_port_en_);
    r += indent_string + "  " + std::string("cnt_en") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(cnt_en_) ) + "\n";
    all_zeros &= (0 == cnt_en_);
    r += indent_string + "  " + std::string("pack_sop_dis") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(pack_sop_dis_) ) + "\n";
    all_zeros &= (0 == pack_sop_dis_);
    r += indent_string + "  " + std::string("ct_sop_wait") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(ct_sop_wait_) ) + "\n";
    all_zeros &= (0 == ct_sop_wait_);
    r += indent_string + "  " + std::string("freeze_on_error") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(freeze_on_error_) ) + "\n";
    all_zeros &= (0 == freeze_on_error_);
    r += indent_string + "  " + std::string("ecc_dis") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(ecc_dis_) ) + "\n";
    all_zeros &= (0 == ecc_dis_);
    if (all_zeros && !print_zeros) {
      return("");
    }
    else {
      return r;
    }
  }

private:
  uint8_t cnt_port_;
  uint8_t cnt_port_en_;
  uint8_t cnt_en_;
  uint8_t pack_sop_dis_;
  uint8_t ct_sop_wait_;
  uint8_t freeze_on_error_;
  uint8_t ecc_dis_;
private:
  static int StartOffset(
      int index_tm_pex_pipe_rspec
      ) {
    int offset=0;
    offset += 0x400000; // to get to tm_top
    offset += 0x100000; // to get to tm_clc_top
    offset += 0x10000; // to get to pex
    assert(index_tm_pex_pipe_rspec < 4);
    offset += index_tm_pex_pipe_rspec * 0x4000; // tm_pex_pipe_rspec[]
    return offset;
  }

};









  }; // namespace register_classes
}; // namespace tofino

#endif // __REGISTER_INCLUDES_QPEX_PIPE_CTRL_H__
