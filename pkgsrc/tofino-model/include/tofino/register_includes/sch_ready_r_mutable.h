/*******************************************************************************
 *  Copyright (C) 2024 Intel Corporation
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *  http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing,
 *  software distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions
 *  and limitations under the License.
 *
 *
 *  SPDX-License-Identifier: Apache-2.0
 ******************************************************************************/

// generated by split_classes.pl
#ifndef __REGISTER_INCLUDES_SCH_READY_R_MUTABLE_H__
#define __REGISTER_INCLUDES_SCH_READY_R_MUTABLE_H__


#include <cstdint>
#include <cassert>
#include <array>
#include <vector>
#include <string>
#include <boost/lexical_cast.hpp>
#include <boost/format.hpp>
#include <model_core/register_block.h>
#include <shared/bitvector.h>












namespace tofino {
  namespace register_classes {

class SchReadyRMutable : public model_core::RegisterBlock<RegisterCallback> {
public:
  SchReadyRMutable(
      int chipNumber, int index_tm_sch_pipe_rspec, RegisterCallback& write_callback = 0, RegisterCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(index_tm_sch_pipe_rspec), 4, true, write_callback, read_callback, std::string("SchReadyRMutable")+":"+boost::lexical_cast<std::string>(index_tm_sch_pipe_rspec))
    {
    }
  SchReadyRMutable(
      
  )
    : RegisterBlock(0, 0, 0, true, 0, 0, "SchReadyRMutable")
    {
    }
public:




  uint8_t p_max_dynamic() { return p_max_dynamic_; }
  void p_max_dynamic(const uint8_t &v) { p_max_dynamic_=v; }




  uint8_t p_max_static() { return p_max_static_; }
  void p_max_static(const uint8_t &v) { p_max_static_=v; }




  uint8_t q_max_dynamic() { return q_max_dynamic_; }
  void q_max_dynamic(const uint8_t &v) { q_max_dynamic_=v; }




  uint8_t q_max_static() { return q_max_static_; }
  void q_max_static(const uint8_t &v) { q_max_static_=v; }




  uint8_t q_exc_dynamic() { return q_exc_dynamic_; }
  void q_exc_dynamic(const uint8_t &v) { q_exc_dynamic_=v; }




  uint8_t q_exc_static() { return q_exc_static_; }
  void q_exc_static(const uint8_t &v) { q_exc_static_=v; }




  uint8_t q_min_dynamic() { return q_min_dynamic_; }
  void q_min_dynamic(const uint8_t &v) { q_min_dynamic_=v; }




  uint8_t q_min_static() { return q_min_static_; }
  void q_min_static(const uint8_t &v) { q_min_static_=v; }




  uint8_t tdm() { return tdm_; }
  void tdm(const uint8_t &v) { tdm_=v; }




  uint8_t pexcr() { return pexcr_; }
  void pexcr(const uint8_t &v) { pexcr_=v; }

  bool read(
      uint32_t offset, uint32_t* data
      ) const {
    if (read_callback_) read_callback_();
    *data = (p_max_dynamic_ & 0x1);
    *data |= ((p_max_static_ & 0x1) << 1);
    *data |= ((q_max_dynamic_ & 0x1) << 2);
    *data |= ((q_max_static_ & 0x1) << 3);
    *data |= ((q_exc_dynamic_ & 0x1) << 4);
    *data |= ((q_exc_static_ & 0x1) << 5);
    *data |= ((q_min_dynamic_ & 0x1) << 6);
    *data |= ((q_min_static_ & 0x1) << 7);
    *data |= ((tdm_ & 0x1) << 8);
    *data |= ((pexcr_ & 0x1) << 9);
    return true;
  }


  bool write(
      uint32_t offset, uint32_t data
      ) {
    p_max_dynamic_ = (data & 0x1);
    p_max_static_ = ((data >> 1) & 0x1);
    q_max_dynamic_ = ((data >> 2) & 0x1);
    q_max_static_ = ((data >> 3) & 0x1);
    q_exc_dynamic_ = ((data >> 4) & 0x1);
    q_exc_static_ = ((data >> 5) & 0x1);
    q_min_dynamic_ = ((data >> 6) & 0x1);
    q_min_static_ = ((data >> 7) & 0x1);
    tdm_ = ((data >> 8) & 0x1);
    pexcr_ = ((data >> 9) & 0x1);
    if (write_callback_) write_callback_();
    return true;
  }

  void reset(
      
      ) {
    p_max_dynamic_ = 0x0;
    p_max_static_ = 0x0;
    q_max_dynamic_ = 0x0;
    q_max_static_ = 0x0;
    q_exc_dynamic_ = 0x0;
    q_exc_static_ = 0x0;
    q_min_dynamic_ = 0x0;
    q_min_static_ = 0x0;
    tdm_ = 0x0;
    pexcr_ = 0x0;
    if (write_callback_) write_callback_();
  }

  std::string to_string(
      uint32_t offset, bool print_zeros = false, std::string indent_string = ""
      ) const {
    std::string r("");
    bool all_zeros=true;
    r += indent_string + std::string("SchReadyRMutable") + ":\n";
    r += indent_string + "  " + std::string("p_max_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(p_max_dynamic_) ) + "\n";
    all_zeros &= (0 == p_max_dynamic_);
    r += indent_string + "  " + std::string("p_max_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(p_max_static_) ) + "\n";
    all_zeros &= (0 == p_max_static_);
    r += indent_string + "  " + std::string("q_max_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_max_dynamic_) ) + "\n";
    all_zeros &= (0 == q_max_dynamic_);
    r += indent_string + "  " + std::string("q_max_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_max_static_) ) + "\n";
    all_zeros &= (0 == q_max_static_);
    r += indent_string + "  " + std::string("q_exc_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_exc_dynamic_) ) + "\n";
    all_zeros &= (0 == q_exc_dynamic_);
    r += indent_string + "  " + std::string("q_exc_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_exc_static_) ) + "\n";
    all_zeros &= (0 == q_exc_static_);
    r += indent_string + "  " + std::string("q_min_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_min_dynamic_) ) + "\n";
    all_zeros &= (0 == q_min_dynamic_);
    r += indent_string + "  " + std::string("q_min_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_min_static_) ) + "\n";
    all_zeros &= (0 == q_min_static_);
    r += indent_string + "  " + std::string("tdm") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(tdm_) ) + "\n";
    all_zeros &= (0 == tdm_);
    r += indent_string + "  " + std::string("pexcr") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(pexcr_) ) + "\n";
    all_zeros &= (0 == pexcr_);
    if (all_zeros && !print_zeros) {
      return("");
    }
    else {
      return r;
    }
  }

  std::string to_string(
      bool print_zeros = false, std::string indent_string = ""
      ) const {
    std::string r("");
    bool all_zeros=true;
    r += indent_string + std::string("SchReadyRMutable") + ":\n";
    r += indent_string + "  " + std::string("p_max_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(p_max_dynamic_) ) + "\n";
    all_zeros &= (0 == p_max_dynamic_);
    r += indent_string + "  " + std::string("p_max_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(p_max_static_) ) + "\n";
    all_zeros &= (0 == p_max_static_);
    r += indent_string + "  " + std::string("q_max_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_max_dynamic_) ) + "\n";
    all_zeros &= (0 == q_max_dynamic_);
    r += indent_string + "  " + std::string("q_max_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_max_static_) ) + "\n";
    all_zeros &= (0 == q_max_static_);
    r += indent_string + "  " + std::string("q_exc_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_exc_dynamic_) ) + "\n";
    all_zeros &= (0 == q_exc_dynamic_);
    r += indent_string + "  " + std::string("q_exc_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_exc_static_) ) + "\n";
    all_zeros &= (0 == q_exc_static_);
    r += indent_string + "  " + std::string("q_min_dynamic") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_min_dynamic_) ) + "\n";
    all_zeros &= (0 == q_min_dynamic_);
    r += indent_string + "  " + std::string("q_min_static") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(q_min_static_) ) + "\n";
    all_zeros &= (0 == q_min_static_);
    r += indent_string + "  " + std::string("tdm") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(tdm_) ) + "\n";
    all_zeros &= (0 == tdm_);
    r += indent_string + "  " + std::string("pexcr") +  ": 0x" + boost::str( boost::format("%x") % static_cast<uint>(pexcr_) ) + "\n";
    all_zeros &= (0 == pexcr_);
    if (all_zeros && !print_zeros) {
      return("");
    }
    else {
      return r;
    }
  }

private:
  uint8_t p_max_dynamic_;
  uint8_t p_max_static_;
  uint8_t q_max_dynamic_;
  uint8_t q_max_static_;
  uint8_t q_exc_dynamic_;
  uint8_t q_exc_static_;
  uint8_t q_min_dynamic_;
  uint8_t q_min_static_;
  uint8_t tdm_;
  uint8_t pexcr_;
private:
  static int StartOffset(
      int index_tm_sch_pipe_rspec
      ) {
    int offset=0;
    offset += 0x400000; // to get to tm_top
    offset += 0xc0000; // to get to tm_sch_top
    assert(index_tm_sch_pipe_rspec < 4);
    offset += index_tm_sch_pipe_rspec * 0x10000; // tm_sch_pipe_rspec[]
    offset += 0x104; // to get to ready
    return offset;
  }

};










  }; // namespace register_classes
}; // namespace tofino

#endif // __REGISTER_INCLUDES_SCH_READY_R_MUTABLE_H__
