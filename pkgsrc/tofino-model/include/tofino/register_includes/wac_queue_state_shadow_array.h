...
#ifndef __REGISTER_INCLUDES_WAC_QUEUE_STATE_SHADOW_ARRAY_H__
#define __REGISTER_INCLUDES_WAC_QUEUE_STATE_SHADOW_ARRAY_H__
...
class WacQueueStateShadowArray : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  WacQueueStateShadowArray(
      int chipNumber, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(), 4 * ArraySize(-1), false, write_callback, read_callback, std::string("WacQueueStateShadowArray")),
...
public:
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("WacQueueStateShadowArray") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<WacQueueStateShadow> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_WAC_QUEUE_STATE_SHADOW_ARRAY_H__
