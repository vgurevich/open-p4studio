...
#ifndef __REGISTER_INCLUDES_GATEWAY_INHIBIT_LUT_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_GATEWAY_INHIBIT_LUT_ARRAY_MUTABLE_H__
...
class GatewayInhibitLutArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  GatewayInhibitLutArrayMutable(
    ...
    : RegisterBlock(chipNumber, StartOffset(index_pipe_addrmap, index_mau_addrmap), 4 * ArraySize(index_pipe_addrmap, index_mau_addrmap, -1), true, write_callback, read_callback, std::string("GatewayInhibitLutArrayMutable")+":"+boost::lexical_cast<std::string>(index_pipe_addrmap) + "," + boost::lexical_cast<std::string>(index_mau_addrmap)),
...
public:
  uint8_t gateway_inhibit_lut(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].gateway_inhibit_lut();
  }
  void gateway_inhibit_lut(uint32_t a0,const uint8_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].gateway_inhibit_lut(v);
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("GatewayInhibitLutArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<GatewayInhibitLutMutable> array;
  uint32_t size0_;
...
private:
  static int StartOffset(
    ...
    offset += 0x1300; // to get to gateway_inhibit_lut
    return offset;
  }
...
#endif // __REGISTER_INCLUDES_GATEWAY_INHIBIT_LUT_ARRAY_MUTABLE_H__
