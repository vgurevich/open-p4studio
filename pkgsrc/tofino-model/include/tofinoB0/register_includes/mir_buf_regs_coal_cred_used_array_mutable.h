...
#ifndef __REGISTER_INCLUDES_MIR_BUF_REGS_COAL_CRED_USED_ARRAY_MUTABLE_H__
#define __REGISTER_INCLUDES_MIR_BUF_REGS_COAL_CRED_USED_ARRAY_MUTABLE_H__
...
class MirBufRegsCoalCredUsedArrayMutable : public model_core::RegisterBlock<RegisterArrayCallback> {
public:
  MirBufRegsCoalCredUsedArrayMutable(
      int chipNumber, int index_pipe_addrmap, RegisterArrayCallback& write_callback = 0, RegisterArrayCallback& read_callback = 0
  )
    : RegisterBlock(chipNumber, StartOffset(index_pipe_addrmap), 4 * ArraySize(index_pipe_addrmap, -1), true, write_callback, read_callback, std::string("MirBufRegsCoalCredUsedArrayMutable")+":"+boost::lexical_cast<std::string>(index_pipe_addrmap)),
...
public:
  ...
  uint16_t coal_entries_used(uint32_t a0)
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    return array[a0].coal_entries_used();
  }
  void coal_entries_used(uint32_t a0,const uint16_t &v) 
  {
    CheckArrayBounds(a0,real_size0_,size0_,0,0,false);
    array[a0].coal_entries_used(v);
  }
  ...
  std::string to_string(
    ...
    if (! r_s.empty()) {
      r += indent_string + std::string("MirBufRegsCoalCredUsedArrayMutable") + "["+boost::lexical_cast<std::string>(a0)+"]"+ ":\n" + r_s ;
    }
...
private:
  std::vector<MirBufRegsCoalCredUsedMutable> array;
  uint32_t size0_;
...
#endif // __REGISTER_INCLUDES_MIR_BUF_REGS_COAL_CRED_USED_ARRAY_MUTABLE_H__
