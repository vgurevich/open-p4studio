...
#ifndef _TOFINOXX_MEM_UTILS_
#define _TOFINOXX_MEM_UTILS_

#include <mem_utils_shared.h>


#define BFN_MEM_TM(y)                     BFN_CONCAT(bfn_mem::pipe_top_level_tm_,y)

#define BFN_MEM_PIPE_PKTGEN_ADDR(p)       (((p)*BFN_MEM_PIPE_ESZ) + BFN_MEM_PIPE(party_pgr_address))
#define BFN_MEM_PIPE_PKTGEN_BUFMEM_ESZ(p) BFN_MEM_PIPE(party_pgr_buffer_mem_word_array_element_size)
#define BFN_MEM_PIPE_PKTGEN_BUFMEM_CNT(p) BFN_MEM_PIPE(party_pgr_buffer_mem_word_array_count)

#define BFN_MEM_TM_PRE_FIFO_ADDR          BFN_MEM_TM_PRE(fifo_mem_word_address)
#define BFN_MEM_TM_PRE_FIFO_ESZ           BFN_MEM_TM_PRE(fifo_mem_word_array_element_size)
#define BFN_MEM_TM_PRE_FIFO_CNT           BFN_MEM_TM_PRE(fifo_mem_word_array_count)

#define BFN_MEM_TM_PRE_RDM_SZ             (BFN_MEM_TM_PRE(rdm_mem_word_array_element_size) * \
                                           BFN_MEM_TM_PRE(rdm_mem_word_array_count) / 16)
...
namespace MODEL_CHIP_NAMESPACE {
 ...
 public:
    static uint64_t pre_mit_mem_address(int i) {
      switch (i) {
        case 0: return BFN_MEM_TM_PRE(mit0_mem_word_address);
        case 1: return BFN_MEM_TM_PRE(mit1_mem_word_address);
        case 2: return BFN_MEM_TM_PRE(mit2_mem_word_address);
        case 3: return BFN_MEM_TM_PRE(mit3_mem_word_address);
        default: return BFN_MEM_INVAL_MEM_ADDRESS;
      }
    }
    static constexpr uint64_t pre_mit_mem_size() {
      return BFN_MEM_TM_PRE(mit0_mem_word_array_element_size) *
             BFN_MEM_TM_PRE(mit0_mem_word_array_count) / 16;
    }
    static constexpr uint64_t pre_pbt_mem_size() {
      return BFN_MEM_TM_PRE(pbt0_mem_word_array_element_size) *
             BFN_MEM_TM_PRE(pbt0_mem_word_array_count) / 16;
    }
    static constexpr uint64_t pre_lit_np_mem_size() {
      return BFN_MEM_TM_PRE(lit0_np_mem_word_array_element_size) *
             BFN_MEM_TM_PRE(lit0_np_mem_word_array_count) / 16;
    }
    static constexpr uint64_t pre_lit_mem_size() {
      return BFN_MEM_TM_PRE(lit0_bm_mem_word0_array_element_size) *
             BFN_MEM_TM_PRE(lit0_bm_mem_word0_array_count) / 16;
    }
    ...
    static constexpr uint64_t pre_pmt_mem_size() {
      return BFN_MEM_TM_PRE(pmt0_mem_word0_array_element_size) *
             BFN_MEM_TM_PRE(pmt0_mem_word0_array_count) / 16;
    }
    ...
    static uint64_t pktgen_mem_address(const unsigned int p) {
      return pipe_mem_address(p, BFN_MEM_PIPE(party_pgr_address));
    }
    static uint64_t pktgen_buffer_mem_word_array_count(unsigned int p) {
      return BFN_MEM_PIPE(party_pgr_buffer_mem_word_array_count);
    }
    static uint64_t pktgen_buffer_mem_word_array_element_size(unsigned int p) {
      return BFN_MEM_PIPE(party_pgr_buffer_mem_word_array_element_size);
    }
