...
uint8_t TcamRowVhWithReg::CalculateTopNibble(int bus,
  ...
  if ( enable ) {
    switch ( half_byte_mux_ctl_[bus].tcam_row_halfbyte_mux_ctl_select() ) {
      ...
      case 2:
      case 3:
        // In JBay codes 2 and 3 select the extra_bits on bits 1:0 and version bits on bits 3:2.

        // extra bit mux selects bits from byte[5]:
        //  Code 0 selects bits [1:0]
        //  Code 1 selects bits [3:2]
        //  Code 2 selects bits [5:4]
        //  Code 3 selects bits [7:6]

        byte = 0x3 & ( input.get_byte( output_byte_source_[5] ) >> ( 2 * extra_bit_mux_ctl_[bus].tcam_extra_bit_mux_ctl()) );
