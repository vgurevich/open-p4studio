...
uint8_t get8(std::FILE *in) {
    char data;
    if (fread(&data, sizeof(data), 1, in)) {}
    return data & 0xffU;
}

int32_t get32(std::FILE *in) {
    char data[4];
    if (fread(data, sizeof(data), 1, in)) {}
    return (data[0] & 0xffU) | ((data[1] & 0xffU) << 8) |
           ((data[2] & 0xffU) << 16) | ((data[3] & 0xffU) << 24);
...
std::FILE *getline(std::FILE *in, std::string &str, char delim = '\n') {
    str.clear();
    int ch;
    while ((ch = fgetc(in)) != EOF && ch != delim)
...
std::FILE *operator>>(std::FILE *in, bson_wrap<json::vector> o) {
    json::vector &out = o.o;
    long start = ftell(in);
    long end = start + get32(in);
    bool error = false;
    out.clear();
    while (uint8_t type = get8(in)) {
        if (error || feof(in) || ferror(in)) break;
        if (ftell(in) >= end) {
            std::cerr << "truncated array" << std::endl;
            // in.setstate(std::ios::failbit);  FIXME -- no way to set error in stdio?
            error = true;
            break; }
        std::string key;
        getline(in, key, '\0');
        ...
        default:
            std::cerr << "unhandled bson tag " << hex(type) << std::endl;
            break; } }
    if (start != -1 && in && ftell(in) != end) {
        std::cerr << "incorrect length for object" << std::endl; }
    return in;
}

std::FILE *operator>>(std::FILE *in, bson_wrap<json::map> o) {
    json::map &out = o.o;
    long start = ftell(in);
    long end = start + get32(in);
    bool error = false;
...
std::FILE *operator>>(std::FILE *in, bson_wrap<std::unique_ptr<obj>> json) {
    json::map rv;
    in >> binary(rv);
    if (auto asvec = map_is_vector(rv))
        json.o = std::move(asvec);
