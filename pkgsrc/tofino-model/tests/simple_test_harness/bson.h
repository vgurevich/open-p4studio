...
std::FILE *operator>>(std::FILE *in, bson_wrap<json::vector> json);
std::FILE *operator>>(std::FILE *in, bson_wrap<json::map> json);
inline std::FILE *operator>>(std::FILE *in, bson_wrap<obj *> json) {
    std::unique_ptr<obj> p;
    in >> binary(p);
    if (feof(in) || ferror(in)) json.o = p.release();
    return in;
}
