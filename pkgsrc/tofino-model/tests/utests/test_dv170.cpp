...
// XXX -> test_dv170.cpp
// Mismatch on hash distribution output on dataplane test
//
...
namespace MODEL_CHIP_TEST_NAMESPACE {
  ...
  bool dv170_print = false;

  using namespace std;
  using namespace MODEL_CHIP_NAMESPACE;


TEST(BFN_TEST_NAME(Dv170Test),Packet1) {
    GLOBAL_MODEL->Reset();
    if (dv170_print) RMT_UT_LOG_INFO("test_dv170_packet1()\n");
    ...
    ASSERT_TRUE(tu.read_test_file("extracted_tests/test_dv170.test"));
