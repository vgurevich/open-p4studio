...
namespace MODEL_CHIP_TEST_NAMESPACE {
  ...
  bool tc2_print = false;

  using namespace std;
  using namespace MODEL_CHIP_NAMESPACE;

  TEST(BFN_TEST_NAME(Tcam2Test),Basic) {
    if (tc2_print) RMT_UT_LOG_INFO("test_tcam2_basic()\n");    
    Tcam2<256,40> t;
    ...
    if (tc2_print) bv1.print();
    if (tc2_print) bv2.print();

    t.reset();
    t.set_value(99, *bvP);
    t.set_value(0, bv1);
    if (tc2_print) t.print(0);
    t.get(0, &bv2, &bv3);
    if (tc2_print) bv1.print();
    if (tc2_print) bv2.print();
    if (tc2_print) bv3.print();

    // Test lookup
    t.set_value(124, bv4);
    t.set_value_mask(224, bv4, bvM);
    t.set_value(125, bv5);
    t.set_value_mask(225, bv5, bvM);
    int l0 = t.tcam_lookup(bvL4);
    if (tc2_print) RMT_UT_LOG_INFO("Found bvL4 at index %d\n", l0);
    ASSERT_EQ(124, l0);
    int l1 = t.tcam_lookup(bvL4b);
    ASSERT_EQ(224, l1);
    if (tc2_print) RMT_UT_LOG_INFO("Found bvL4b at index %d\n", l1);

    // Test masked lookup
    int l2 = t.tcam_lookup(bvL5);
    ASSERT_EQ(125, l2);
    if (tc2_print) RMT_UT_LOG_INFO("Found bvL5 at index %d\n", l2);
    int l3 = t.tcam_lookup(bvL5b);
    ASSERT_EQ(225, l3);
    if (tc2_print) RMT_UT_LOG_INFO("Found bvL5b at index %d\n", l3);
  }

  TEST(BFN_TEST_NAME(Tcam2Test),SetGet) {
    if (tc2_print) RMT_UT_LOG_INFO("test_tcam2_set_get()\n");    
    Tcam2<256,40> t;
    BitVector<40> bv4(std::array<uint8_t,5>({{4,4,4,4,4}}));
    BitVector<40> bv5(std::array<uint8_t,5>({{5,5,5,5,5}}));
    uint8_t M = 0xFFu;
  ...
  TEST(BFN_TEST_NAME(Tcam2Test),Reset) {
    if (tc2_print) RMT_UT_LOG_INFO("test_tcam2_reset()\n");    
    Tcam2<256,40> t;
    BitVector<40> bv4(std::array<uint8_t,5>({{4,4,4,4,4}}));
    BitVector<40> bv5(std::array<uint8_t,5>({{5,5,5,5,5}}));
    uint8_t M = 0xFFu;
  ...
  TEST(BFN_TEST_NAME(Tcam2Test),StartOffset) {
    if (tc2_print) RMT_UT_LOG_INFO("test_tcam2_start_offset()\n");    
    Tcam2<256,40> t;
    BitVector<40> bv4(std::array<uint8_t,5>({{4,4,4,4,4}}));
    BitVector<40> bv5(std::array<uint8_t,5>({{5,5,5,5,5}}));
    uint8_t M = 0xFFu;
  ...
  uint64_t tcam2_compare_func_local(uint64_t w0, uint64_t w1,
                                   uint64_t s0, uint64_t s1) {
    uint64_t val = ~(s0|s1) | (w0&w1) | (s0&w0) | (s1&w1);
    //uint64_t msk = UINT64_C(0xFFFFFFFFFFFFFFFF); FAIL_1
    //uint64_t msk = (w0&w1); FAIL_2
    //uint64_t msk = ~(s0&s1); FAIL_3
    uint64_t msk = ~(s0&s1) | (w0&w1);
    return ~(val & msk);
  }

  TEST(BFN_TEST_NAME(Tcam2Test),TcamCompare) {
    if (tc2_print) RMT_UT_LOG_INFO("test_tcam2_tcam_compare()\n");
  ...
  TEST(BFN_TEST_NAME(Tcam2Test),TcamLookup) {
    if (tc2_print) RMT_UT_LOG_INFO("test_tcam2_tcam_lookup)\n");
    Tcam2<256,44> tcam2;
    tcam2.reset();
    ...
    uint64_t      cmp = tcam2_compare_func_local(w0, w1, s0, s1);
    ...
    tcam2.set_word1_word0(11, bv_new_word1, bv_new_word0);
    tcam2.set_value_mask(22, bv_value, bv_mask);
    tcam2.get(22, &bv_word1, &bv_word0);
    ...
    int r1 = tcam2.tcam_lookup(bv_search0,bv_search1);
    int r2 = tcam2.tcam_lookup(bv_search1);
